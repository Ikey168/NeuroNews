...
from src.knowledge_graph.nlp_populator import KnowledgeGraphPopulator, get_entity_relationships
...
async def get_related_entities_endpoint(
    entity_name: str = Query(..., description="Name of the entity to find relationships for"),
    max_results: int = Query(10, ge=1, le=100, description="Maximum number of related entities to return"),
    include_confidence: bool = Query(True, description="Include confidence scores in results"),
    min_confidence: float = Query(0.0, ge=0.0, le=1.0, description="Minimum confidence threshold for relationships"),
    ...
    try:
        ...
        response = {
            ...
            'min_confidence_threshold': min_confidence if min_confidence > 0.0 else None,
            'related_entities': related_entities,
            'timestamp': datetime.utcnow().isoformat(),
            'status': 'success'
        }
        
        logger.info(f"Successfully returned {len(related_entities)} related entities for {entity_name}")
        return response
        
    except Exception as e:
        logger.error(f"Error processing related entities request for {entity_name}: {str(e)}")
        raise HTTPException(
            status_code=500,
    ...
    try:
        ...
        if 'published_date' in article_data and article_data['published_date']:
            try:
                ...
                logger.warning(f"Invalid published_date format: {article_data['published_date']}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to populate article: {str(e)}"
        )
    finally:
        await populator.close()
...
async def batch_populate_articles_endpoint(
    ...
    try:
        ...
        if not articles:
            raise HTTPException(
                status_code=400,
                detail="No articles provided for batch processing"
            )
