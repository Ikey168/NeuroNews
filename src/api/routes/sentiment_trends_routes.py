...
async def generate_sentiment_alerts(
    ...
    topic: Optional[str] = Query(
        None, description="Specific topic to check for alerts"
    ),
    lookback_days: int = Query(
        7, ge=1, le=30, description="Days to look back for change detection"
    ),
    analyzer: SentimentTrendAnalyzer = Depends(get_sentiment_analyzer),
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        alerts = await analyzer.generate_sentiment_alerts(
            topic=topic, lookback_days=lookback_days
        )
        ...
        response = {
            "generation_parameters": {"topic": topic, "lookback_days": lookback_days},
...
async def get_sentiment_alerts(
    ...
    try:
        ...
        if include_resolved:
            ...
            logger.warning(
                "Resolved alerts not yet supported, returning active alerts only"
            )

        alerts = await analyzer.get_active_alerts(
            topic=topic, severity=severity, limit=limit
        )
    ...
    try:
        ...
        response = {
            ...
            "data_quality": {
                ...
                "total_articles_analyzed": trend_summary.statistical_summary.get(
                    "total_articles", 0
                ),
...
async def get_sentiment_trends_summary(
    limit_topics: int = Query(
        20, ge=1, le=100, description="Maximum number of topics to include"
    ),
    analyzer: SentimentTrendAnalyzer = Depends(get_sentiment_analyzer),
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        trend_summaries = sorted(
            ...
            key=lambda x: (
                len(x.data_points),
                x.statistical_summary.get("total_articles", 0),
            ),
            reverse=True,
        )[:limit_topics]
        ...
        if trend_summaries:
            overall_sentiment = sum(s.average_sentiment for s in trend_summaries) / len(
                trend_summaries
            )
            overall_volatility = sum(
                s.sentiment_volatility for s in trend_summaries
            ) / len(trend_summaries)
        else:
        ...
        for alert in all_alerts:
            ...
            alert_breakdown["by_type"][alert.alert_type] = (
                alert_breakdown["by_type"].get(alert.alert_type, 0) + 1
            )
        ...
        response = {
            ...
            "trending_topics": {
                "most_positive": sorted(
                    topic_summaries, key=lambda x: x["current_sentiment"], reverse=True
                )[:5],
                "most_negative": sorted(
                    topic_summaries, key=lambda x: x["current_sentiment"]
                )[:5],
                "most_volatile": sorted(
                    topic_summaries, key=lambda x: x["volatility"], reverse=True
                )[:5],
            },
            "analysis_period": {
                "start_date": start_date.isoformat(),
    ...
    try:
        ...
        with psycopg2.connect(**analyzer.conn_params) as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM sentiment_alerts WHERE resolved_at IS NULL"
                )
