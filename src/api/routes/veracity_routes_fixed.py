...
@router.post("/analyze", response_model=VeracityResponse)
async def analyze_news_veracity(article: Article) -> VeracityResponse:
    """
    Analyze the veracity of a single news article.

    Args:
        article: Article data containing ID and text
...
@router.post("/batch", response_model=BatchVeracityResponse)
    ...
    try:
        if not request.articles:
            return BatchVeracityResponse(
                results=[], total_processed=0, status="success"
            )
