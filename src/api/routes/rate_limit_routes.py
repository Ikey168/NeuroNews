...
from src.api.middleware.rate_limit_middleware import RateLimitStore, RateLimitConfig, SuspiciousActivityDetector
...
async def get_api_limits(
    ...
    try:
        ...
        if rate_limit_store.use_redis:
            try:
                concurrent_requests = await rate_limit_store._get_concurrent_count_redis(user_id)
            except:
                pass
        else:
...
@router.get("/api_limits/suspicious_activity", response_model=List[SuspiciousActivityResponse])
async def get_suspicious_activity(
    ...
    try:
        ...
        recent_alerts = [
            alert for alert in suspicious_detector.alerts
...
async def reset_user_limits(
    ...
    try:
        ...
        logger.info(f"Rate limits reset for user {user_id} by admin {current_user.get('user_id')}")
