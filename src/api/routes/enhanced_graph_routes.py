...
class RelatedEntitiesRequest(BaseModel):
    entity: str = Field(..., min_length=1, description="Entity name to find relationships for")
    entity_type: Optional[str] = Field(None, description="Type of entity (Organization, Person, Event)")
...
class CacheManagementRequest(BaseModel):
    pattern: Optional[str] = Field(None, description="Pattern to match for cache clearing")
...
async def get_optimized_graph() -> OptimizedGraphAPI:
    ...
    if optimized_graph_api is None:
        ...
        raise HTTPException(
            status_code=503, 
            detail="Knowledge Graph API service not available"
        )
...
async def get_related_entities_v2(
    ...
    max_depth: int = Query(2, ge=1, le=5, description="Maximum relationship depth to traverse"),
    ...
    try:
        ...
        if relationship_types:
            parsed_relationship_types = [rt.strip() for rt in relationship_types.split(',')]
...
async def search_entities_v2(
    ...
    types: Optional[str] = Query(None, description="Comma-separated entity types to filter by"),
