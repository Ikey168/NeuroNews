...
from src.api.auth.api_key_manager import api_key_manager, APIKeyStatus
...
class APIKeyGenerateRequest(BaseModel):
    ...
    expires_in_days: Optional[int] = Field(None, description="Days until expiration (default: 365)", ge=1, le=3650)
    permissions: Optional[List[str]] = Field(None, description="List of permissions for this key")
    rate_limit: Optional[int] = Field(None, description="Rate limit in requests per minute", ge=1, le=10000)
...
class APIKeyRenewRequest(BaseModel):
    ...
    extends_days: Optional[int] = Field(None, description="Days to extend (default: 365)", ge=1, le=3650)
...
async def generate_api_key_query_param(
    ...
    if user_id != requesting_user_id and user_role not in ['admin', 'administrator']:
        raise HTTPException(
            status_code=403,
            detail="Can only generate API keys for your own account"
        )
...
async def renew_api_key(
    ...
    try:
        result = await api_key_manager.renew_api_key(
            user_id=user_id,
            key_id=request.key_id,
            extends_days=request.extends_days
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to renew API key: {str(e)}")
...
async def get_api_key_usage_stats(user_id: str = Depends(get_user_id)):
    ...
    try:
        ...
        user_requests = sum(
            count for key_id, count in metrics["request_counts"].items()
...
async def cleanup_expired_keys(current_user: dict = Depends(require_auth)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to cleanup expired keys: {str(e)}")
