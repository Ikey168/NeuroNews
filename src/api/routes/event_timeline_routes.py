...
from src.api.event_timeline_service import EventTimelineService, HistoricalEvent, TimelineVisualizationData
...
class EventTrackingRequest(BaseModel):
    ...
    topic: str = Field(..., min_length=2, max_length=200, description="Topic to track events for")
    start_date: Optional[datetime] = Field(None, description="Start date for event tracking")
    end_date: Optional[datetime] = Field(None, description="End date for event tracking")
    event_types: Optional[List[str]] = Field(None, description="Specific event types to track")
    max_events: int = Field(default=100, ge=1, le=500, description="Maximum number of events to track")
    include_related: bool = Field(default=True, description="Include related event relationships")
    store_in_neptune: bool = Field(default=True, description="Store events in Neptune database")
...
class TimelineVisualizationRequest(BaseModel):
    ...
    include_impact_analysis: bool = Field(default=True, description="Include impact analysis")
    include_entity_chart: bool = Field(default=True, description="Include entity involvement chart")
...
async def get_event_timeline_service() -> EventTimelineService:
    ...
    if _event_timeline_service is None:
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail="Event timeline service not available"
            )
...
async def get_enhanced_event_timeline(
    ...
    include_visualizations: bool = Query(True, description="Include visualization data"),
                ...
                for e in response_data['events']:
                    try:
                        event = HistoricalEvent(
                            ...
                            topic=e.get('topic', topic),  # Use endpoint topic as fallback
                        ...
                        logger.warning(f"Failed to create HistoricalEvent from data: {event_error}")
                        continue
...
async def export_timeline_data(
    ...
    try:
        ...
        if format not in ['json', 'csv', 'html']:
            raise HTTPException(
                status_code=400,
                detail="Format must be one of: json, csv, html"
            )
        ...
        if format == 'json':
            return JSONResponse(
                ...
                headers={'Content-Disposition': f'attachment; filename="{topic}_timeline.json"'}
            )
            ...
            # Write header
            writer.writerow([
                'Event ID', 'Title', 'Description', 'Timestamp', 'Event Type',
                'Confidence', 'Impact Score', 'Entities Involved'
            ])
            ...
            return Response(
                content=csv_content,
                media_type='text/csv',
                headers={'Content-Disposition': f'attachment; filename="{topic}_timeline.csv"'}
            )
            ...
            return Response(
                ...
                headers={'Content-Disposition': f'attachment; filename="{topic}_timeline.html"'}
            )
...
async def get_timeline_analytics(
    time_window_days: int = Query(30, ge=1, le=365, description="Analysis time window in days"),
    top_topics: int = Query(10, ge=1, le=50, description="Number of top topics to include"),
    service: EventTimelineService = Depends(get_event_timeline_service),
    user=Depends(get_optional_auth)
):
    ...
    try:
        ...
        return {
            ...
            "components": {
                ...
                "graph_populator": service.graph_populator is not None if service else False,
                "entity_extractor": service.entity_extractor is not None if service else False
            }
        }
