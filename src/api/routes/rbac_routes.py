...
from src.api.rbac.rbac_system import rbac_manager, UserRole, Permission
...
async def get_role_info(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve role information: {str(e)}"
        )
...
async def get_user_permissions(
    ...
    if (current_user.get('sub') != user_id and 
        current_user.get('role', '').lower() not in ['admin', 'administrator']):
        raise HTTPException(
            status_code=403,
...
async def check_access(
    ...
    try:
        ...
        for role in [UserRole.FREE, UserRole.PREMIUM, UserRole.ADMIN]:
            if rbac_manager.has_access(role, access_request.method, access_request.path):
...
async def get_endpoint_permissions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get endpoint permissions: {str(e)}"
        )
...
async def get_rbac_metrics(admin_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve RBAC metrics: {str(e)}"
        )
...
async def delete_user_permissions(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500,
                detail="Failed to delete user permissions"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to delete user permissions: {str(e)}"
        )
