...
class EntityRelationshipQuery(BaseModel):
    ...
    max_depth: int = Field(2, ge=1, le=5, description="Maximum relationship depth to traverse")
    max_results: int = Field(50, ge=1, le=200, description="Maximum number of results to return")
    relationship_types: Optional[List[str]] = Field(None, description="Filter by specific relationship types")
    min_confidence: float = Field(0.0, ge=0.0, le=1.0, description="Minimum confidence threshold")
    include_context: bool = Field(True, description="Include relationship context information")
...
class EventTimelineQuery(BaseModel):
    ...
    max_events: int = Field(50, ge=1, le=200, description="Maximum number of events to return")
    event_types: Optional[List[str]] = Field(None, description="Filter by specific event types")
    include_articles: bool = Field(True, description="Include source articles in timeline")

class GraphSearchQuery(BaseModel):
    """Request model for advanced graph search."""
    query_type: str = Field(..., description="Type of search: 'entity', 'relationship', 'path'")
    search_terms: List[str] = Field(..., description="Search terms or entity names")
    filters: Optional[Dict[str, Any]] = Field({}, description="Additional search filters")
    sort_by: str = Field("relevance", description="Sort results by: relevance, date, confidence")
...
async def get_enhanced_graph_populator() -> EnhancedKnowledgeGraphPopulator:
    ...
    if not ENHANCED_KG_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Enhanced knowledge graph components not available"
        )
    ...
    try:
        ...
        if CONFIG_AVAILABLE:
            ...
            import os
            neptune_endpoint = os.getenv('NEPTUNE_ENDPOINT', 'wss://demo-cluster.neptune.amazonaws.com:8182/gremlin')
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Knowledge graph service not available: {str(e)}"
        )
...
async def get_related_entities(
    ...
    relationship_types: Optional[str] = Query(None, description="Comma-separated relationship types to filter by"),
    ...
    try:
        ...
        if not entity or len(entity.strip()) < 2:
            raise HTTPException(
                status_code=400,
                detail="Entity name must be at least 2 characters long"
            )
        ...
        logger.info(f"Successfully returned {len(related_entities)} related entities for {entity}")
        return response
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error processing related entities request for {entity}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve related entities: {str(e)}"
        )
...
async def get_event_timeline(
    ...
    try:
        ...
        logger.info(f"Successfully returned {len(timeline_events)} timeline events for {topic}")
        return response
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve event timeline: {str(e)}"
        )
...
async def get_entity_details(
    ...
    include_relationships: bool = Query(True, description="Include relationship information"),
    include_articles: bool = Query(True, description="Include source articles"),
...
async def advanced_graph_search(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to execute graph search: {str(e)}"
        )


@router.get("/graph_analytics")
async def get_graph_analytics(
    metric_type: str = Query("overview", description="Type of analytics: overview, centrality, clustering"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve graph analytics: {str(e)}"
        )
...
async def _query_timeline_events(
    ...
    try:
        ...
        gremlin_query += f".order().by('published_date', desc).limit({max_events}).valueMap(true)"
        
        # Execute query using enhanced populator's graph builder
        results = await populator.graph_builder._execute_traversal(
            populator.graph_builder.g.V().hasLabel('Article')
            .has('title', containing(topic))
        ...
        for i, article in enumerate(results):
            ...
            entities_involved = await _get_article_entities(populator, article.get('id', [''])[0])
            ...
            timeline_event = TimelineEvent(
                ...
                description=article.get('content', [''])[0][:500] + "..." if article.get('content') else "",
...
async def _get_article_entities(populator: EnhancedKnowledgeGraphPopulator, article_id: str) -> List[str]:
    """Get entities mentioned in a specific article."""
    try:
        # Query entities connected to the article
        results = await populator.graph_builder._execute_traversal(
            populator.graph_builder.g.V().has('id', article_id)
...
async def _get_entity_relationships(populator: EnhancedKnowledgeGraphPopulator, entity_id: str) -> List[Dict[str, Any]]:
    """Get relationships for a specific entity."""
    try:
        # Query outgoing relationships
        out_results = await populator.graph_builder._execute_traversal(
            populator.graph_builder.g.V().has('id', entity_id)
            .outE()
...
async def _get_entity_articles(populator: EnhancedKnowledgeGraphPopulator, entity_id: str) -> List[Dict[str, Any]]:
...
async def _execute_advanced_search(
    ...
    try:
        if query_type == 'entity':
            return await _search_entities(populator, search_terms, filters, sort_by, limit)
        elif query_type == 'relationship':
            return await _search_relationships(populator, search_terms, filters, sort_by, limit)
    ...
    try:
        ...
        if search_terms:
            ...
            for i, term in enumerate(search_terms):
                if i == 0:
                    ...
                    search_query = search_query.or_(has('normalized_form', containing(term)))
        else:
            search_query = base_query
        ...
        for entity in results:
            entities.append({
                ...
                'properties': {k: v[0] if v else None for k, v in entity.items() 
...
async def _get_overview_analytics(populator: EnhancedKnowledgeGraphPopulator, entity_type: Optional[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        vertex_count = await populator.graph_builder._execute_traversal(vertex_count_query.count())
        edge_count = await populator.graph_builder._execute_traversal(populator.graph_builder.g.E().count())
        ...
        return {
            ...
            'entity_type_distribution': type_distribution[0] if type_distribution else {},
...
async def _get_centrality_analytics(populator: EnhancedKnowledgeGraphPopulator, entity_type: Optional[str], top_n: int) -> Dict[str, Any]:
...
async def _get_clustering_analytics(populator: EnhancedKnowledgeGraphPopulator, entity_type: Optional[str], top_n: int) -> Dict[str, Any]:
    ...
    try:
        ...
        results = await populator.graph_builder._execute_traversal(
            populator.graph_builder.g.V()
            .has('entity_type', entity_type) if entity_type else populator.graph_builder.g.V()
            .bothE()
            .groupCount().by(otherV().values('entity_type'))
