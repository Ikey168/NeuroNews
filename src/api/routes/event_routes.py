...
class EventDetectionRequest(BaseModel):
    ...
    max_articles: int = Field(100, ge=10, le=1000, description="Maximum articles to process")
    clustering_method: str = Field("kmeans", pattern="^(kmeans|dbscan)$", description="Clustering algorithm")
    min_cluster_size: int = Field(3, ge=2, le=20, description="Minimum articles per cluster")
...
async def get_breaking_news(
    category: Optional[str] = Query(None, description="Filter by category (e.g., Technology, Politics)"),
    hours_back: int = Query(24, ge=1, le=168, description="Hours to look back for events"),
    ...
    **Example:**
    ...
    """
    try:
        logger.info(f"Getting breaking news: category={category}, hours_back={hours_back}, limit={limit}")
        ...
        response_events = []
        for event in events:
            response_events.append(BreakingNewsResponse(
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving breaking news: {str(e)}")


@router.get("/events/clusters", response_model=List[EventClusterResponse])
async def get_event_clusters(
    category: Optional[str] = Query(None, description="Filter by category"),
    event_type: Optional[str] = Query(None, description="Filter by event type (breaking, trending, developing)"),
    ...
    min_significance: float = Query(0.0, ge=0.0, le=100.0, description="Minimum significance score")
):
    """
    ...
    **Query Parameters:**
    ...
    """
    try:
        logger.info(f"Getting event clusters: category={category}, event_type={event_type}")
        ...
        for row in rows:
            ...
            # Parse JSON fields
            primary_sources = json.loads(row['primary_sources']) if row['primary_sources'] else []
            geographic_focus = json.loads(row['geographic_focus']) if row['geographic_focus'] else []
            key_entities = json.loads(row['key_entities']) if row['key_entities'] else []
            
            clusters.append(EventClusterResponse(
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving event clusters: {str(e)}")


@router.get("/events/{cluster_id}/articles", response_model=List[ArticleInClusterResponse])
async def get_articles_in_cluster(
    ...
    **Query Parameters:**
    ...
    try:
        ...
        if not rows:
            raise HTTPException(status_code=404, detail=f"Cluster {cluster_id} not found or has no articles")
        
        # Convert to response model
        articles = []
        for row in rows:
            articles.append(ArticleInClusterResponse(
...
async def trigger_event_detection(
    ...
    **Request Body:**
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error in event detection: {str(e)}")
    ...
    try:
        ...
        for row in rows:
            categories.append({
                ...
                'top_events': row['top_events'].split(' | ') if row['top_events'] else []
            })
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving trending categories: {str(e)}")
    ...
    try:
        ...
        stats = {
            ...
            'embedding_statistics': {
                ...
                'avg_processing_time': float(embedding_stats['avg_processing_time'] or 0)
            },
            'assignment_statistics': {
                ...
                'representative_articles': int(assignment_stats['representative_articles'] or 0)
            },
            'system_info': {
