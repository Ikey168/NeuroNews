...
from src.api.security.aws_waf_manager import waf_manager, ThreatType, ActionType
...
class WAFConfigRequest(BaseModel):
    ...
    rate_limit: Optional[int] = Field(None, description="Rate limit per 5 minutes", ge=100, le=10000)
    enable_sql_protection: Optional[bool] = Field(None, description="Enable SQL injection protection")
    enable_xss_protection: Optional[bool] = Field(None, description="Enable XSS protection")
...
async def require_admin(user: dict = Depends(require_auth)) -> dict:
    ...
    if user.get('role', '').lower() not in ['admin', 'administrator']:
        raise HTTPException(
            status_code=403,
            detail="Admin privileges required for WAF management"
        )
    return user
...
async def deploy_waf(admin_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500,
                detail="Failed to create WAF Web ACL"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to deploy WAF: {str(e)}"
        )

@router.post("/waf/associate/{api_gateway_arn}")
async def associate_waf_with_gateway(
    api_gateway_arn: str = Path(..., description="API Gateway ARN to associate with WAF"),
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500,
                detail="Failed to associate WAF with API Gateway"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to associate WAF: {str(e)}"
        )
...
async def get_blocked_requests(
    limit: int = Query(100, description="Maximum number of blocked requests to return", ge=1, le=1000),
    admin_user: dict = Depends(require_admin)
):
    """
...
async def get_waf_status(_: dict = Depends(require_auth)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get WAF status: {str(e)}"
        )
...
async def get_real_time_threats(
    ...
    try:
        ...
        threat_analysis = {
            ...
            "recommendations": _generate_security_recommendations(metrics, middleware_metrics)
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get real-time threats: {str(e)}"
        )

@router.post("/waf/configure")
async def configure_waf(
    config: WAFConfigRequest,
    admin_user: dict = Depends(require_admin)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to configure WAF: {str(e)}"
        )
...
async def get_sql_injection_attempts(
    ...
    try:
        ...
        blocked_requests = waf_manager.get_blocked_requests(limit * 2)  # Get more to filter
        
        sql_injection_attempts = [
            req for req in blocked_requests
            if 'sql' in req.get('action', '').lower() or 'sql' in str(req.get('uri', '')).lower()
        ][:limit]
        ...
        return {
            ...
            "analysis": {
                "common_injection_patterns": _analyze_sql_patterns(sql_injection_attempts),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get SQL injection attempts: {str(e)}"
        )
...
async def get_xss_attempts(
    ...
    try:
        ...
        xss_attempts = [
            req for req in blocked_requests
            if 'xss' in req.get('action', '').lower() or 'script' in str(req.get('uri', '')).lower()
        ][:limit]
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get XSS attempts: {str(e)}"
        )
...
async def get_geofencing_status(admin_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        geo_blocked = [req for req in blocked_requests if 'geo' in req.get('action', '').lower()]
        
        return {
            "timestamp": datetime.utcnow().isoformat(),
            "geofencing_enabled": True,
            "allowed_countries": waf_manager.allowed_countries,
            "blocked_countries_sample": list(set(req.get('country', 'unknown') for req in geo_blocked))[:10],
            "statistics": {
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get geofencing status: {str(e)}"
        )
...
def _analyze_attack_patterns(blocked_requests: List[Dict]) -> List[str]:
    ...
    for req in blocked_requests:
        ...
        elif any(sql_word in uri.lower() for sql_word in ['union', 'select', 'drop', 'insert']):
            patterns.append('SQL injection')
        elif len(uri) > 200:
            patterns.append('Long URI (potential buffer overflow)')
    return list(set(patterns))

def _calculate_active_threats(waf_metrics: Dict, middleware_metrics: Dict) -> Dict[str, Any]:
    """Calculate active threats based on metrics."""
    return {
        "sql_injection_attempts": _extract_metric_value(waf_metrics, 'SQLInjectionBlocked'),
...
def _calculate_risk_level(waf_metrics: Dict, middleware_metrics: Dict) -> str:
    ...
    total_blocked = sum(_extract_metric_value(waf_metrics, metric) for metric in 
                       ['SQLInjectionBlocked', 'XSSBlocked', 'GeoBlocked', 'RateLimitExceeded'])
...
def _generate_security_recommendations(waf_metrics: Dict, middleware_metrics: Dict) -> List[str]:
    ...
    if middleware_metrics.get('block_rate', 0) > 5:
        recommendations.append("High block rate detected - review legitimate traffic patterns")
    
    if not recommendations:
        recommendations.append("Security posture is good - maintain current configuration")
...
def _extract_metric_value(metrics: Dict, metric_name: str) -> int:
    ...
    try:
        return metrics.get('metrics', {}).get(metric_name, {}).get('blocked_requests', 0)
...
def _get_top_attacking_ips(attempts: List[Dict]) -> List[str]:
    ...
    for attempt in attempts:
        ...
        ip_counts[ip] = ip_counts.get(ip, 0) + 1
    return [ip for ip, _ in sorted(ip_counts.items(), key=lambda x: x[1], reverse=True)[:5]]
...
def _get_top_blocked_countries(geo_blocked: List[Dict]) -> List[str]:
    ...
    return [country for country, _ in sorted(country_counts.items(), key=lambda x: x[1], reverse=True)[:5]]
