...
async def get_articles_by_topic(
    ...
    min_probability: float = Query(0.2, ge=0.0, le=1.0, description="Minimum topic probability"),
...
async def get_articles_by_keyword(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving articles by keyword: {str(e)}"
        )
...
async def advanced_search(
    search_term: Optional[str] = Query(None, description="Text to search in title and content"),
    topic: Optional[str] = Query(None, description="Filter by topic name"),
    keyword: Optional[str] = Query(None, description="Filter by keyword"),
    min_topic_probability: float = Query(0.1, ge=0.0, le=1.0, description="Minimum topic probability"),
    limit: int = Query(50, ge=1, le=200, description="Maximum number of articles to return"),
    offset: int = Query(0, ge=0, description="Number of articles to skip"),
    db: KeywordTopicDatabase = Depends(get_keyword_topic_db)
) -> Dict[str, Any]:
...
async def get_trending_topics(
    ...
    min_articles: int = Query(5, ge=1, description="Minimum articles for trending topics"),
    db: KeywordTopicDatabase = Depends(get_keyword_topic_db)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        trending_topics = [
            ...
            if topic["article_count"] >= min_articles
        ]
        
        # Sort by article count (frequency) and average probability
        trending_topics.sort(
            key=lambda x: (x["article_count"], x["avg_probability"]), 
            reverse=True
        )
...
async def get_trending_keywords(
    days: int = Query(7, ge=1, le=30, description="Number of days for trend analysis"),
    min_frequency: int = Query(10, ge=1, description="Minimum frequency for trending keywords"),
    db: KeywordTopicDatabase = Depends(get_keyword_topic_db)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        trending_keywords = sorted(
            keywords,
            key=lambda x: (x["frequency"], x["avg_score"]),
            reverse=True
        )[:50]  # Top 50 trending keywords
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving trending keywords: {str(e)}"
        )
