...
from src.api.security.aws_waf_manager import waf_manager, ThreatType, SecurityEvent, ActionType
...
class WAFSecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            await self._log_request_metrics(client_ip, request_path, processing_time, response.status_code)
    ...
    async def _log_security_event(self, threat_type: ThreatType, source_ip: str, 
                                 user_agent: str, request_path: str, 
                                 action_taken: ActionType, details: Dict[str, Any]):
        """Log security event."""
        event = SecurityEvent(
    ...
    async def _log_request_metrics(self, client_ip: str, path: str, 
...
class WAFMetricsMiddleware(BaseHTTPMiddleware):
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'top_blocked_ips': dict(sorted(self.metrics['top_blocked_ips'].items(), key=lambda x: x[1], reverse=True)[:10]),
