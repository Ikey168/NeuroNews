...
class AWSWAFManager:
    ...
    def __init__(self):
        ...
        self.allowed_countries = os.getenv("WAF_ALLOWED_COUNTRIES", "US,CA,GB,AU,DE,FR,JP").split(",")
        
        # Rate limiting configuration
        self.rate_limit_requests = int(os.getenv("WAF_RATE_LIMIT", "2000"))  # requests per 5 minutes
    ...
    def create_web_acl(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Created Web ACL: {self.web_acl_name} with ARN: {self.web_acl_arn}")
            return True
        ...
        try:
            ...
            for metric_name in metric_names:
                try:
                    ...
                    total_blocked = sum(point['Sum'] for point in response['Datapoints'])
                    metrics[metric_name] = {
    ...
    def get_blocked_requests(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.wafv2_client.get_sampled_requests(
                ...
                Scope='REGIONAL',
                TimeWindow={
                    'StartTime': datetime.now(timezone.utc).replace(hour=datetime.now(timezone.utc).hour - 1),
                ...
                "widgets": [
                    {
                        ...
                        "properties": {
                            "metrics": [
                                ["AWS/WAFV2", "BlockedRequests", "WebACL", self.web_acl_name, "Region", self.region],
                                ["AWS/WAFV2", "AllowedRequests", "WebACL", self.web_acl_name, "Region", self.region]
                            ],
                            "period": 300,
                            "stat": "Sum",
                            "region": self.region,
                    ...
                    },
                    {
                        ...
                        "properties": {
                            "metrics": [
                                ["AWS/WAFV2", "SQLInjectionBlocked", "WebACL", self.web_acl_name, "Region", self.region],
                                ["AWS/WAFV2", "XSSBlocked", "WebACL", self.web_acl_name, "Region", self.region]
                            ],
                            "period": 300,
                            "stat": "Sum",
                            "region": self.region,
                    ...
                    },
                    {
                        ...
                        "properties": {
                            "metrics": [
                                ["AWS/WAFV2", "GeoBlocked", "WebACL", self.web_acl_name, "Region", self.region],
                                ["AWS/WAFV2", "RateLimitExceeded", "WebACL", self.web_acl_name, "Region", self.region]
                            ],
                            "period": 300,
                            "stat": "Sum",
                            "region": self.region,
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        # Check AWS clients
        health_status['components']['waf_client'] = 'operational' if self.wafv2_client else 'unavailable'
        health_status['components']['cloudwatch_client'] = 'operational' if self.cloudwatch_client else 'unavailable'
        health_status['components']['logs_client'] = 'operational' if self.logs_client else 'unavailable'
