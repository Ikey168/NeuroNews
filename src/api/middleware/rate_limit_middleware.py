...
class SuspiciousActivityDetector:
    ...
    async def _check_error_rate(self, metrics: List[RequestMetrics]) -> bool:
        ...
        now = datetime.now()
        recent = [m for m in metrics if (now - m.timestamp).total_seconds() < 300]  # Last 5 minutes
    ...
    async def _check_endpoint_abuse(self, metrics: List[RequestMetrics], endpoint: str) -> bool:
        ...
        return len(recent_same_endpoint) > self.config.SUSPICIOUS_PATTERNS['endpoint_abuse']
    
    def _log_suspicious_activity(self, user_id: str, metrics: RequestMetrics, alerts: List[str]):
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def _check_rate_limits(self, user_id: str, user_tier: UserTier) -> Optional[JSONResponse]:
    ...
    def _create_rate_limit_response(self, message: str, limit: int, reset_time: int = 60) -> JSONResponse:
    ...
    def _add_rate_limit_headers(self, response, user_id: str, user_tier: UserTier):
        ...
        response.headers["X-RateLimit-Limit-Minute"] = str(user_tier.requests_per_minute)
