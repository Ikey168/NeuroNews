...
class UserBehaviorProfile:
    ...
    user_id: str
    typical_hours: List[int] = field(default_factory=list)  # Hours they usually access API
    typical_endpoints: Dict[str, int] = field(default_factory=dict)  # Endpoint usage frequency
...
class AdvancedSuspiciousActivityDetector:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            'bot_user_agents': [
                ...
                'automated', 'headless', 'phantom', 'selenium'
            ]
        }
    ...
    async def analyze_user_activity(self, user_id: str, recent_requests: List[Dict]) -> List[SuspiciousActivity]:
    ...
    async def _update_user_profile(self, user_id: str, requests: List[Dict]):
        ...
        profile.typical_hours = list(set(profile.typical_hours + hours))[-24:]  # Keep last 24 unique hours
        ...
        if len(requests) > 0:
            time_span = max(1, (max(req.get('timestamp', datetime.now()) for req in requests) - 
                              min(req.get('timestamp', datetime.now()) for req in requests)).total_seconds() / 3600)
            profile.average_requests_per_hour = len(requests) / time_span
        
        # Update IP addresses
        ips = [req.get('ip_address', '') for req in requests if req.get('ip_address')]
        profile.typical_ip_addresses = list(set(profile.typical_ip_addresses + ips))[-10:]  # Keep last 10 IPs
        
        # Update user agents
        user_agents = [req.get('user_agent', '') for req in requests if req.get('user_agent')]
        profile.user_agent_patterns = list(set(profile.user_agent_patterns + user_agents))[-5:]  # Keep last 5
        
        profile.last_updated = datetime.now()
    
    async def _detect_rapid_requests(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
        ...
        if len(recent_requests) >= self.config['rapid_requests_threshold']:
            return SuspiciousActivity(
                ...
                ip_addresses=list(set(req.get('ip_address', '') for req in recent_requests)),
                endpoints_accessed=list(set(req.get('endpoint', '') for req in recent_requests)),
                request_count=len(recent_requests),
                confidence_score=min(1.0, len(recent_requests) / self.config['rapid_requests_threshold'])
            )
        
        return None
    
    async def _detect_unusual_hours(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
        ...
        if len(unusual_requests) > 5:  # More than 5 requests in unusual hours
            return SuspiciousActivity(
                ...
                details={
                    ...
                    'hours_accessed': list(set(req.get('timestamp', datetime.now()).hour for req in unusual_requests)),
                    'typical_hours': profile.typical_hours
                },
                ip_addresses=list(set(req.get('ip_address', '') for req in unusual_requests)),
                endpoints_accessed=list(set(req.get('endpoint', '') for req in unusual_requests)),
    ...
    async def _detect_multiple_ips(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
        ...
        ips = set(req.get('ip_address', '') for req in recent_requests if req.get('ip_address'))
        ...
        if len(ips) >= self.config['multiple_ip_threshold']:
            return SuspiciousActivity(
                ...
                endpoints_accessed=list(set(req.get('endpoint', '') for req in recent_requests)),
                request_count=len(recent_requests),
                confidence_score=min(1.0, len(ips) / (self.config['multiple_ip_threshold'] * 2))
            )
        
        return None
    
    async def _detect_high_error_rate(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
        ...
        error_count = sum(1 for req in recent_requests if req.get('response_code', 200) >= 400)
    ...
    async def _detect_endpoint_abuse(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
        ...
        if abused_endpoints:
            total_abuse_requests = sum(endpoint_counts[endpoint] for endpoint in abused_endpoints)
            ...
            return SuspiciousActivity(
                ...
                details={
                    ...
                    'requests_per_endpoint': {ep: endpoint_counts[ep] for ep in abused_endpoints},
                    'threshold': self.config['endpoint_abuse_threshold']
                },
                ip_addresses=list(set(req.get('ip_address', '') for req in requests[-50:])),
                endpoints_accessed=abused_endpoints,
                request_count=total_abuse_requests,
                confidence_score=min(1.0, total_abuse_requests / (self.config['endpoint_abuse_threshold'] * 2))
            )
        
        return None
    
    async def _detect_bot_behavior(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
        ...
        for ua in user_agents:
            if any(bot_term in ua.lower() for bot_term in self.config['bot_user_agents']):
                bot_indicators += 1
                break
        total_indicators += 1
        ...
        if confidence >= 0.6:  # 60% confidence threshold
            return SuspiciousActivity(
                ...
                details={
                    ...
                    'patterns_detected': {
                        'regular_timing': len(intervals) > 5 and statistics.stdev(intervals) < 1.0,
                        'bot_user_agent': any(bot_term in ua.lower() for ua in user_agents for bot_term in self.config['bot_user_agents']),
                        'sequential_access': len(set(endpoints)) == 1 and len(endpoints) > 5
                    }
                },
                ip_addresses=list(set(req.get('ip_address', '') for req in requests[-20:])),
                endpoints_accessed=list(set(req.get('endpoint', '') for req in requests[-20:])),
    ...
    async def _detect_credential_stuffing(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
        """Detect credential stuffing attacks."""
        # Look for multiple failed login attempts
        auth_requests = [
            req for req in requests 
            if '/auth/' in req.get('endpoint', '') and req.get('response_code', 200) == 401
        ]
        
        if len(auth_requests) >= 10:  # 10+ failed login attempts
            now = datetime.now()
            recent_auth_failures = [
                req for req in auth_requests
                if (now - req.get('timestamp', now)).total_seconds() < 300  # Last 5 minutes
            ]
            ...
            if len(recent_auth_failures) >= 5:
                return SuspiciousActivity(
                    ...
                    ip_addresses=list(set(req.get('ip_address', '') for req in recent_auth_failures)),
                    endpoints_accessed=list(set(req.get('endpoint', '') for req in recent_auth_failures)),
    ...
    async def _detect_data_scraping(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
        ...
        data_endpoints = [req for req in recent_requests if any(
        ...
        if len(data_endpoints) >= 30:  # High volume of data requests
            ...
            typical_data_requests = sum(
                count for endpoint, count in profile.typical_endpoints.items()
                if any(pattern in endpoint for pattern in ['/articles', '/news', '/graph', '/entities'])
            )
            ...
            if current_rate > typical_data_requests * 3:  # 3x normal rate
                return SuspiciousActivity(
                    ...
                    ip_addresses=list(set(req.get('ip_address', '') for req in data_endpoints)),
                    endpoints_accessed=list(set(req.get('endpoint', '') for req in data_endpoints)),
                    request_count=len(data_endpoints),
                    confidence_score=min(1.0, current_rate / (typical_data_requests * 5))
                )
        
        return None
    
    async def _detect_ddos_pattern(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
    ...
    async def _detect_unusual_user_agent(self, user_id: str, requests: List[Dict]) -> Optional[SuspiciousActivity]:
        ...
        for ua in recent_user_agents:
            if ua and ua not in profile.user_agent_patterns:
                ...
                suspicious_terms = ['bot', 'crawler', 'scraper', 'hack', 'test', 'curl', 'wget']
        ...
        if unusual_agents:
            return SuspiciousActivity(
                ...
                ip_addresses=list(set(req.get('ip_address', '') for req in requests[-10:])),
                endpoints_accessed=list(set(req.get('endpoint', '') for req in requests[-10:])),
    ...
    async def _log_suspicious_activity(self, activity: SuspiciousActivity):
        ...
        if activity.alert_level in [AlertLevel.HIGH, AlertLevel.CRITICAL]:
            logger.warning(f"Suspicious activity detected: {activity.pattern_type.value}", extra=log_data)
        else:
            logger.info(f"Potential suspicious activity: {activity.pattern_type.value}", extra=log_data)
    ...
    def get_recent_alerts(self, hours: int = 24, min_alert_level: AlertLevel = AlertLevel.LOW) -> List[SuspiciousActivity]:
        ...
        return [
            ...
            alert.alert_level.value >= min_alert_level.value
        ]
