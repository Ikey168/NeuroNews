...
from .language_processor import LanguageDetector, AWSTranslateService, TranslationQualityChecker
...
class MultiLanguageArticleProcessor(ArticleProcessor):
    ...
    def __init__(self,
        ...
        super().__init__(
            ...
            redshift_user, redshift_password, **kwargs
        )
    ...
    def process_article(self, article_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            detected_language, confidence = self.language_detector.detect_language(text_to_analyze)
            
            # Store language detection result
            self._store_language_detection(
                article_id, detected_language, confidence, 
                'local_heuristic', len(text_to_analyze)
            )
            ...
            processed_article.update({
                ...
                'processed_language': self.target_language if result['translation_performed'] else detected_language
            })
    ...
    def _translate_article(self, 
        ...
        try:
            ...
            if title_translation['error'] or content_translation['error']:
                logger.warning(f"Translation errors: title={title_translation['error']}, content={content_translation['error']}")
                return result
            ...
            overall_quality = (title_quality['quality_score'] + content_quality['quality_score']) / 2
            ...
            if overall_quality >= self.quality_threshold:
                ...
                self._store_translation(
                    article_id, source_language, self.target_language,
                    title, title_translation['translated_text'],
                    content, content_translation['translated_text'],
                    overall_quality, title_translation['confidence'],
                    title_quality['issues'] + content_quality['issues'],
                    title_quality['recommendations'] + content_quality['recommendations']
                )
                
                logger.info(f"Translation completed with quality score: {overall_quality:.2f}")
                
            else:
                logger.warning(f"Translation quality too low: {overall_quality:.2f} < {self.quality_threshold}")
    ...
    def _store_language_detection(self,
                                 article_id: str,
        ...
        try:
            with psycopg2.connect(**self.conn_params) as conn:
                with conn.cursor() as cursor:
                    ...
                    """, (article_id, detected_language, confidence, method, text_length))
        ...
        try:
            with psycopg2.connect(**self.conn_params) as conn:
                with conn.cursor() as cursor:
                    ...
                    """, (
                        article_id, original_language, target_language,
                        original_title, translated_title,
                        original_content, translated_content,
                        quality_score, confidence,
                        json.dumps(issues), json.dumps(recommendations)
                    ))
            ...
            with psycopg2.connect(**self.conn_params) as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                    ...
                    return {
                        ...
                        'total_translations': sum(row['translation_count'] for row in translation_stats)
                    }
    ...
    def store_language_detection(self, detection_data: Dict[str, Any]) -> bool:
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                self.connection.commit()
                logger.debug(f"Language detection stored for article {detection_data.get('article_id')}")
                return True
        except Exception as e:
            logger.error(f"Error storing language detection: {e}")
            return False
    ...
    def store_translation(self, translation_data: Dict[str, Any]) -> bool:
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                self.connection.commit()
                logger.debug(f"Translation stored for article {translation_data.get('article_id')}")
                return True
        except Exception as e:
            logger.error(f"Error storing translation: {e}")
            return False
