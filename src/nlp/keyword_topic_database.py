...
from src.database.redshift_loader import RedshiftLoader, ArticleRecord
...
class KeywordTopicDatabase:
    ...
    async def store_extraction_results(self, results: List[ExtractionResult]) -> Dict[str, Any]:
        ...
        for result in results:
            try:
                ...
                logger.error(f"Error storing extraction result for {result.article_id}: {e}")
    ...
    async def get_articles_by_topic(self, 
                                   topic_name: str,
    ...
    async def get_articles_by_keyword(self,
                                     keyword: str,
                                     min_score: float = 0.1,
                                     limit: int = 50,
                                     offset: int = 0) -> List[Dict[str, Any]]:
        ...
        for row in results:
            ...
            filtered_keywords = [
                ...
                if keyword.lower() in kw.get('keyword', '').lower() and kw.get('score', 0) >= min_score
            ]
            
            if filtered_keywords:  # Only include if keyword matches with sufficient score
                article = {
                    "id": row[0],
                    "url": row[1],
                    "title": row[2], 
    ...
    async def get_topic_statistics(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        for topic in topics:
            topic["percentage"] = (topic["article_count"] / total_articles * 100) if total_articles > 0 else 0.0
        
        return topics
    
    async def get_keyword_statistics(self, days: int = 30, min_frequency: int = 3) -> List[Dict[str, Any]]:
    ...
    async def search_articles_by_content_and_topics(self,
                                                   search_term: str,
        ...
        # Topic filter
        if topic_filter:
            conditions.append("JSON_EXTRACT_PATH_TEXT(dominant_topic, 'topic_name') ILIKE %s")
            conditions.append("CAST(JSON_EXTRACT_PATH_TEXT(dominant_topic, 'probability') AS FLOAT) >= %s")
            params.extend([f"%{topic_filter}%", min_topic_probability])
        
        # Keyword filter  
        if keyword_filter:
            conditions.append("JSON_EXTRACT_PATH_TEXT(keywords, '$[*].keyword') ILIKE %s")
        ...
        for row in results:
            article = {
                ...
                "url": row[1],
                "title": row[2],
                "content": row[3][:500] + "..." if len(row[3]) > 500 else row[3],  # Truncate content
                "source": row[4],
...
async def create_keyword_topic_db(redshift_config: Optional[Dict[str, str]] = None) -> KeywordTopicDatabase:
