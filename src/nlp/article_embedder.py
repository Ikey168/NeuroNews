...
class ArticleEmbedder:
    ...
    async def generate_embedding(self, 
                                text: str, 
                                title: str = "",
                                article_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.conn_params and article_id:
                existing_embedding = await self._get_existing_embedding(article_id, text_hash)
            ...
            quality_score = self._calculate_embedding_quality(embedding_vector, preprocessed_text)
            ...
            logger.debug(f"Generated embedding for article {article_id} in {processing_time:.2f}s")
    ...
    async def generate_embeddings_batch(self, 
        ...
        try:
            ...
            if self.conn_params:
                preprocessed_data = await self._filter_existing_embeddings(preprocessed_data)
            ...
            logger.info(f"Generated {len(results)} embeddings in {total_time:.2f}s "
                       f"({avg_time_per_article:.2f}s per article)")
    ...
    def _calculate_embedding_quality(self, embedding_vector: np.ndarray, text: str) -> float:
        """
    ...
    async def _get_existing_embedding(self, article_id: str, text_hash: str) -> Optional[Dict]:
                ...
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    if row:
                        ...
                        return {
                            ...
                            'embedding_quality_score': float(row['embedding_quality_score']) if row['embedding_quality_score'] else 0.5,
    ...
    async def _filter_existing_embeddings(self, articles_data: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Filtered out {len(articles_data) - len(filtered_data)} existing embeddings")
    ...
    async def get_embeddings_for_clustering(self, 
        ...
        try:
            with psycopg2.connect(**self.conn_params) as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    logger.info(f"Retrieved {len(embeddings)} embeddings for clustering")
                    return embeddings
