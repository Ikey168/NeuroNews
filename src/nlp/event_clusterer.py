...
class EventClusterer:
    ...
    async def detect_events(self, 
        ...
        try:
            if len(embeddings_data) < self.min_cluster_size:
                logger.warning(f"Not enough articles ({len(embeddings_data)}) for clustering")
                return []
            ...
            self.stats.update({
                ...
                'clusters_created': len(set(cluster_labels)) - (1 if -1 in cluster_labels else 0),
    ...
    def _find_optimal_clusters(self, embeddings: np.ndarray) -> int:
        ...
        try:
            ...
            logger.info(f"Best silhouette score: {max(silhouette_scores):.3f} for k={optimal_k}")
    ...
    async def _process_clusters_to_events(self,
                                         embeddings_data: List[Dict[str, Any]],
    ...
    async def _create_event_from_cluster(self,
                                        cluster_id: int,
        ...
        try:
            ...
            embeddings_matrix = np.array([article['embedding_vector'] for article in articles])
            ...
            event = {
                ...
                'cohesion_score': 1.0 / (1.0 + avg_distance),  # Inverse of average distance
                'separation_score': cluster_metrics.get('silhouette_score', 0.0),  # Use silhouette as proxy
                ...
                'articles': [
                    {
                        ...
                        'contribution_score': 1.0 / len(articles),  # Equal contribution for now
                        'novelty_score': 1.0 - (i / len(articles))  # Earlier articles are more novel
                    }
    ...
    def _determine_event_type(self, articles: List[Dict[str, Any]], duration_hours: float) -> str:
    ...
    def _calculate_trending_score(self, articles: List[Dict[str, Any]], duration_hours: float) -> float:
        ...
        try:
            ...
            hours_since_latest = (datetime.now() - latest_article['published_date']).total_seconds() / 3600
    ...
    def _calculate_impact_score(self, articles: List[Dict[str, Any]], source_counts: Counter) -> float:
        """Calculate the estimated impact/importance of the event."""
        try:
            # Factors: source credibility, article volume, sentiment
            
            # Source credibility factor
            credible_sources = sum(1 for article in articles 
    ...
    def _calculate_velocity_score(self, articles: List[Dict[str, Any]], duration_hours: float) -> float:
    ...
    def _find_peak_activity(self, articles: List[Dict[str, Any]]) -> datetime:
        ...
        try:
            ...
            for article in articles:
                hour_key = article['published_date'].replace(minute=0, second=0, microsecond=0)
    ...
    def _infer_category(self, articles: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            category_keywords = {
                'Technology': ['tech', 'ai', 'artificial intelligence', 'software', 'computer', 'digital', 'app', 'google', 'apple', 'microsoft'],
                'Politics': ['government', 'election', 'president', 'congress', 'senate', 'policy', 'law', 'vote'],
                'Health': ['health', 'medical', 'doctor', 'hospital', 'disease', 'covid', 'vaccine', 'medicine'],
                'Business': ['business', 'company', 'market', 'stock', 'economy', 'financial', 'revenue', 'profit'],
    ...
    def _calculate_event_significance(self, events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                    ...
                    for event in events:
                        try:
                            ...
                            """, (
                                ...
                                event['embedding_model'], event['clustering_method'],
                                event['cluster_size'], event['silhouette_score'],
                                event['cohesion_score'], event['separation_score'],
                                event['first_article_date'], event['last_article_date'],
                                event['peak_activity_date'], event['event_duration_hours'],
                                json.dumps(event['primary_sources']),
                                json.dumps(event['geographic_focus']),
                                json.dumps(event['key_entities']),
                                event['trending_score'], event['impact_score'],
                                event['velocity_score'], event['status']
                            ))
                            ...
                            for article in event['articles']:
                                cur.execute("""
                                    ...
                                    ON CONFLICT (article_id, cluster_id) DO UPDATE SET
                                        ...
                                        updated_at = CURRENT_TIMESTAMP
                                """, (
                                    article['article_id'], event['cluster_id'],
                                    ...
                                    self.clustering_method, '1.0'
                                ))
                            
                            stored_count += 1
                            
                        except Exception as e:
                            logger.warning(f"Error storing event {event['cluster_id']}: {e}")
                            continue
            ...
            with psycopg2.connect(**self.conn_params) as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    for row in rows:
                        ...
                        for date_field in ['first_article_date', 'last_article_date', 'peak_activity_date']:
    ...
    async def test_clusterer():
        ...
        if embeddings_data:
            ...
            for event in events[:3]:  # Show first 3
                print(f"- {event['cluster_name']} ({event['event_type']}) - {event['cluster_size']} articles")
                print(f"  Trending: {event['trending_score']:.2f}, Impact: {event['impact_score']:.2f}")
        else:
