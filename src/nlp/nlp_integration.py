...
class OptimizedSentimentAnalyzer:
    ...
    async def _fallback_sentiment_analysis(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for article in articles:
            try:
                ...
                logger.error(f"Legacy sentiment analysis failed for article {article.get('id')}: {e}")
                results.append({
                    'article_id': article.get('id'),
                    'error': str(e),
                    'fallback': True
...
class OptimizedArticleEmbedder:
    ...
    async def generate_embeddings_batch(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
            ...
            for result in results['results']:
                if 'embedding' in result:
                    ...
                    embedding_results.append({
                        ...
                        'embedding_quality': embedding_data.get('quality_score', 0.0),
                        'processing_time': result.get('processing_time', 0.0),
    ...
    async def _fallback_embedding_generation(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Fallback to legacy article embedder."""
        logger.info("Using legacy article embedder as fallback")
        
        try:
            # Use legacy embedder
            embedding_results = await self.legacy_embedder.generate_embeddings_batch(articles)
...
class OptimizedEventClusterer:
    ...
    async def cluster_articles_async(self, articles: List[Dict[str, Any]], 
                                   embeddings: List[np.ndarray] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.legacy_clusterer:
                ...
                clustering_result = self._simple_clustering_fallback(articles, embeddings)
    ...
    def _simple_clustering_fallback(self, articles: List[Dict[str, Any]], 
...
class IntegratedNLPProcessor:
    ...
    async def process_articles_comprehensive(self, articles: List[Dict[str, Any]], 
        ...
        logger.info(f"Starting comprehensive NLP processing for {len(articles)} articles")
        ...
        try:
            ...
            if 'clustering' in operations:
                clustering_results = await self._process_clustering(articles, results['articles'])
            ...
            results['performance_metrics'] = {
                ...
                'memory_usage_mb': pipeline_stats.get('memory_stats', {}).get('current_usage_mb', 0),
                'batch_count': pipeline_stats.get('pipeline_stats', {}).get('batch_count', 0),
    ...
    async def _process_clustering(self, articles: List[Dict[str, Any]], 
                                processed_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if None in embeddings:
                ...
                embedding_results = await self.article_embedder.generate_embeddings_batch(articles)
                embeddings = [np.array(r.get('embedding', [])) for r in embedding_results]
            
            # Perform clustering
            clustering_results = await self.event_clusterer.cluster_articles_async(articles, embeddings)
    ...
    async def analyze_sentiment_optimized(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Optimized sentiment analysis with caching."""
        return await self.sentiment_analyzer.analyze_batch(articles)
    
    async def generate_embeddings_optimized(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Optimized embedding generation with caching."""
        return await self.article_embedder.generate_embeddings_batch(articles)
    
    async def cluster_events_optimized(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def get_comprehensive_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'component_stats': {
                ...
                'article_embedder': getattr(self.article_embedder, 'get_performance_stats', lambda: {})(),
                'event_clusterer': getattr(self.event_clusterer, 'get_performance_stats', lambda: {})()
            },
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        sample_articles = [
            {
                ...
                'content': f'This is a sample news article about technology and innovation. ' * 20
            }
            for i in range(10)
        ]
        ...
        try:
            ...
            print(f"Throughput: {results['performance_metrics']['articles_per_second']:.2f} articles/sec")
            print(f"Cache hit rate: {results['performance_metrics']['cache_hit_rate']:.2%}")
            print(f"Memory usage: {results['performance_metrics']['memory_usage_mb']:.1f}MB")
            
            if results['clustering']:
                print(f"Event clusters found: {results['clustering'].get('cluster_count', 0)}")
            
            # Get comprehensive stats
            stats = processor.get_comprehensive_stats()
            print(f"Total sessions: {stats['total_stats']['sessions']}")
            print(f"Total articles processed: {stats['total_stats']['articles_processed']}")
