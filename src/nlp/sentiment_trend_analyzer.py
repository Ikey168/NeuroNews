...
class SentimentTrendAnalyzer:
    ...
    def __init__(self,
        ...
        self.config = {
            ...
            'topics_of_interest': [
                'politics', 'technology', 'economy', 'healthcare', 
                'climate', 'education', 'security', 'international'
            ]
        }
    ...
    async def analyze_historical_trends(self, 
                                      ...
                                      time_granularity: str = 'daily') -> List[TopicTrendSummary]:
        """
        ...
        """
        try:
            logger.info(f"Analyzing historical sentiment trends for topic: {topic or 'all'}")
            ...
            sentiment_data = await self._fetch_sentiment_data(topic, start_date, end_date)
            ...
            for topic_name, topic_data in topics_data.items():
                if len(topic_data) < self.config['min_articles_for_trend']:
                    logger.warning(f"Insufficient data for topic {topic_name}: {len(topic_data)} articles")
                    continue
                
                # Aggregate data by time granularity
                aggregated_data = self._aggregate_by_time_granularity(topic_data, time_granularity)
    ...
    async def generate_sentiment_alerts(self, 
                                      topic: Optional[str] = None,
                                      lookback_days: int = 7) -> List[TrendAlert]:
        """
        ...
        try:
            ...
            end_date = datetime.now(timezone.utc)
            start_date = end_date - timedelta(days=lookback_days * 2)  # Get more data for comparison
    ...
    def _group_data_by_topic(self, sentiment_data: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _aggregate_by_time_granularity(self, 
                                     topic_data: List[Dict[str, Any]], 
                                     granularity: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _create_trend_points(self, 
                           topic: str, 
                           aggregated_data: Dict[str, List[Dict[str, Any]]]) -> List[SentimentTrendPoint]:
    ...
    def _calculate_trend_summary(self, 
                               topic: str, 
                               trend_points: List[SentimentTrendPoint],
                               time_range: Tuple[datetime, datetime]) -> TopicTrendSummary:
        ...
        if len(trend_points) >= 2:
            ...
            slope, intercept, r_value, p_value, std_err = stats.linregress(x_values, sentiment_scores)
    ...
    async def _detect_sentiment_alerts(self, 
                                     topic: str, 
                                     topic_data: List[Dict[str, Any]],
                                     lookback_days: int) -> List[TrendAlert]:
        ...
        try:
            ...
            current_period = [item for item in topic_data if item['publish_date'] >= cutoff_date]
            previous_period = [item for item in topic_data if item['publish_date'] < cutoff_date]
            
            if not current_period or not previous_period:
                return alerts
            
            # Calculate sentiment averages for both periods
            current_sentiment = statistics.mean([item['sentiment_score'] for item in current_period])
            previous_sentiment = statistics.mean([item['sentiment_score'] for item in previous_period])
            
            change_magnitude = abs(current_sentiment - previous_sentiment)
            change_percentage = (change_magnitude / (abs(previous_sentiment) + 0.001)) * 100
            ...
            if (previous_sentiment > 0.1 and current_sentiment < -0.1) or \
               (previous_sentiment < -0.1 and current_sentiment > 0.1):
                if change_magnitude >= self.config['alert_thresholds']['trend_reversal']:
                    alert = self._create_alert(
                        topic=topic,
            ...
            current_volatility = statistics.stdev([item['sentiment_score'] for item in current_period]) \
                               if len(current_period) > 1 else 0
            previous_volatility = statistics.stdev([item['sentiment_score'] for item in previous_period]) \
                                if len(previous_period) > 1 else 0
            
            volatility_increase = current_volatility - previous_volatility
            if volatility_increase >= self.config['alert_thresholds']['volatility_spike']:
                alert = self._create_alert(
                    topic=topic,
                    ...
                    change_percentage=(volatility_increase / (previous_volatility + 0.001)) * 100,
                    time_window=f"{lookback_days}d",
                    affected_articles=[item['id'] for item in current_period]
                )
                alerts.append(alert)
    ...
    def _create_alert(self, 
                     ...
                     change_percentage: float,
                     time_window: str,
                     affected_articles: List[str]) -> TrendAlert:
        ...
        description = f"Sentiment for '{topic}' has {direction} by {change_percentage:.1f}% " \
        ...
        confidence = min(0.95, max(0.3, change_magnitude + (len(affected_articles) / 100)))
    ...
    async def get_topic_trend_summary(self, topic: str, days: int = 30) -> Optional[TopicTrendSummary]:
...
async def analyze_sentiment_trends_for_topic(topic: str, 
                                           redshift_config: Dict[str, Any],
                                           days: int = 30) -> Optional[TopicTrendSummary]:
    """
...
async def generate_daily_sentiment_alerts(redshift_config: Dict[str, Any]) -> List[TrendAlert]:
    """
