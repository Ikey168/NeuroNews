...
class NERProcessor:
    ...
    TECHNOLOGY_KEYWORDS = {
        ...
        'api', 'rest api', 'graphql', 'microservices', 'devops', 'ci/cd',
        'cybersecurity', 'data science', 'big data', 'analytics', 'iot',
        'internet of things', '5g', 'quantum computing', 'edge computing'
    }
    
    # Policy-related keywords for enhanced detection
    POLICY_KEYWORDS = {
        'gdpr', 'ccpa', 'privacy policy', 'data protection', 'regulation',
        'compliance', 'security policy', 'terms of service', 'user agreement',
        'copyright', 'intellectual property', 'patent', 'trademark',
        'open source', 'license', 'mit license', 'apache license'
    }
    ...
    def __init__(self, 
        ...
        logger.info(f"Initializing NER processor with model: {model_name} on device: {self.device}")
    ...
    def extract_entities(self, text: str, article_id: str = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(f"Extracted {len(processed_entities)} entities from article {article_id}")
    ...
    def _enhance_entity_type(self, entity_text: str, original_type: str, context: str) -> str:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_entities_per_text': (
                self.stats['total_entities_extracted'] / max(1, self.stats['total_texts_processed'])
            ),
