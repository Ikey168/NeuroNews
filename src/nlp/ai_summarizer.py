...
class AIArticleSummarizer:
    ...
    def _load_model(self, model_type: SummarizationModel) -> Tuple:
        ...
        try:
            if model_type == SummarizationModel.BART:
                ...
                model = PegasusForConditionalGeneration.from_pretrained(model_type.value)
    ...
    def _calculate_metrics(self, original_text: str, summary_text: str, 
        ...
        confidence_score = min(0.95, max(0.1, 
            0.8 - abs(compression_ratio - 0.2) * 2  # Prefer ~20% compression
        ))
    ...
    async def summarize_article(self, 
                              ...
                              model: Optional[SummarizationModel] = None) -> Summary:
        """
        ...
        try:
            ...
            logger.info(f"Summary generated: {length.value} ({metrics['word_count']} words, "
    ...
    async def summarize_article_all_lengths(self, 
                                           text: str,
                                           model: Optional[SummarizationModel] = None) -> Dict[SummaryLength, Summary]:
        """
...
async def demo_summarization():
    ...
    for length, summary in summaries.items():
        ...
        print(f"Words: {summary.word_count}, Compression: {summary.compression_ratio:.2%}")
