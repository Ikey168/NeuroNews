...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/tmp/ner_job.log')
    ]
)
...
class KubernetesNERProcessor:
    ...
    def process_article_batch(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Processed batch in {batch_processing_time:.2f}s, extracted {len(results)} entities")
            return results
    ...
    async def store_results_in_redshift(self, results: List[Dict[str, Any]]):
        ...
        try:
            ...
            with self.redshift_processor as processor:
                ...
                logger.info(f"Successfully stored {success_count} entity results in Redshift")
                
        except Exception as e:
            logger.error(f"Failed to store results in Redshift: {e}")
            # Save results to file as backup
            backup_file = os.path.join(self.output_dir, f"entity_results_backup_{int(time.time())}.json")
    ...
    async def update_postgres_processing_status(self, article_ids: List[str]):
        ...
        try:
            with self.postgres_conn.cursor() as cur:
                ...
                logger.info(f"Updated NER processing status for {len(article_ids)} articles")
    ...
    def save_processing_stats(self):
        ...
        try:
            ...
            stats_file = os.path.join(self.output_dir, f"ner_job_stats_{int(time.time())}.json")
            ...
            if self.stats['articles_processed'] > 0:
                ...
                logger.info(f"  Avg Entities/Article: {self.stats.get('average_entities_per_article', 0):.2f}")
                ...
                for future in as_completed(future_to_batch):
                    ...
                    try:
                        ...
                        if batch_results:
                            ...
                            article_ids = list(set([r['article_id'] for r in batch_results]))
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Kubernetes Entity Extraction Processor')
    ...
    parser.add_argument('--model-name', default='dbmdz/bert-large-cased-finetuned-conll03-english', help='Model name')
    ...
    confidence_threshold = float(os.getenv('CONFIDENCE_THRESHOLD', args.confidence_threshold))
