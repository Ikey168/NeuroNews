...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/tmp/sentiment_job.log')
    ]
)
...
class KubernetesSentimentProcessor:
            ...
            for i, (article, sentiment_result) in enumerate(zip(articles, sentiment_results)):
                try:
                    result = {
                        ...
                        'sentiment_confidence': float(sentiment_result.get('confidence', sentiment_result['score'])),
    ...
    async def store_results_in_redshift(self, results: List[Dict[str, Any]]):
        ...
        try:
            ...
            with self.redshift_processor as processor:
                ...
                # Insert results
                success_count = await processor.batch_insert_sentiment_results(redshift_records)
                logger.info(f"Successfully stored {success_count} sentiment results in Redshift")
                
        except Exception as e:
            logger.error(f"Failed to store results in Redshift: {e}")
            # Save results to file as backup
            backup_file = os.path.join(self.output_dir, f"sentiment_results_backup_{int(time.time())}.json")
    ...
    async def update_postgres_processing_status(self, article_ids: List[str]):
        ...
        try:
            with self.postgres_conn.cursor() as cur:
                ...
                logger.info(f"Updated processing status for {len(article_ids)} articles")
    ...
    def save_processing_stats(self):
        ...
        try:
            ...
            stats_file = os.path.join(self.output_dir, f"sentiment_job_stats_{int(time.time())}.json")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Kubernetes Sentiment Analysis Processor')
    ...
    parser.add_argument('--model-name', default='cardiffnlp/twitter-roberta-base-sentiment-latest', help='Model name')
