...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/tmp/ai_job.log')
    ]
)
...
class KubernetesAIProcessor:
    ...
    async def fetch_articles_to_process(self, limit: int = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            with self.postgres_conn.cursor() as cur:
                ...
                params = [
                    processing_window,
                    int(os.getenv('MINIMUM_ARTICLE_LENGTH', '200')),  # Longer minimum for topic modeling
                    datetime.now(timezone.utc) - timedelta(days=14)  # Last 14 days for topic analysis
                ]
    ...
    def preprocess_texts(self, articles: List[Dict[str, Any]]) -> List[str]:
        ...
        for article in articles:
            try:
                ...
                logger.error(f"Failed to preprocess article {article.get('article_id', 'unknown')}: {e}")
    ...
    def perform_topic_modeling(self, texts: List[str], embeddings: np.ndarray) -> Tuple[List[int], List[Dict[str, Any]]]:
        """
    ...
    def perform_lda_topic_modeling(self, texts: List[str], embeddings: np.ndarray) -> Tuple[List[int], List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            for topic_idx in range(self.num_topics):
                ...
                top_weights = [lda_model.components_[topic_idx][i] for i in top_word_indices]
    ...
    def perform_umap_topic_modeling(self, texts: List[str], embeddings: np.ndarray) -> Tuple[List[int], List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            for topic_idx in range(self.num_topics):
                ...
                topic_texts = [texts[i] for i, t in enumerate(topic_assignments) if t == topic_idx]
                ...
                if topic_texts:
                    ...
                    try:
                        ...
                        topic_desc = {
                            ...
                            'topic_words': top_words,
                            'topic_weights': top_weights,
                            'topic_label': f"Topic_{topic_idx}_{top_words[0]}_{top_words[1]}" if top_words else f"Topic_{topic_idx}",
                        ...
                        logger.warning(f"Failed to generate description for topic {topic_idx}: {e}")
                        topic_desc = {
                            'topic_id': topic_idx,
                            'topic_words': [],
                            'topic_weights': [],
    ...
    def process_article_batch(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            logger.info(f"Processing batch of {len(articles)} articles for topic modeling")
            ...
            topic_assignments, topic_descriptions = self.perform_topic_modeling(texts, embeddings)
            ...
            for i, (article, topic_id) in enumerate(zip(articles, topic_assignments)):
                try:
                    ...
                    if topic_desc is None:
                        logger.warning(f"No topic description found for topic {topic_id}")
                        continue
            ...
            logger.info(f"Processed batch in {batch_processing_time:.2f}s, assigned topics to {len(results)} articles")
            return results
    ...
    async def store_results_in_redshift(self, results: List[Dict[str, Any]]):
        ...
        try:
            ...
            with self.redshift_processor as processor:
                ...
                logger.info(f"Successfully stored {success_count} topic results in Redshift")
                
        except Exception as e:
            logger.error(f"Failed to store results in Redshift: {e}")
            # Save results to file as backup
            backup_file = os.path.join(self.output_dir, f"topic_results_backup_{int(time.time())}.json")
    ...
    async def update_postgres_processing_status(self, article_ids: List[str]):
        ...
        try:
            with self.postgres_conn.cursor() as cur:
                ...
                logger.info(f"Updated topic processing status for {len(article_ids)} articles")
        ...
        try:
            ...
            if self.stats['articles_processed'] > 0:
                ...
                self.stats['throughput_articles_per_second'] = (
                    self.stats['articles_processed'] / self.stats['total_duration_seconds']
                )
            
            stats_file = os.path.join(self.output_dir, f"topic_job_stats_{int(time.time())}.json")
            ...
            if self.stats['articles_processed'] > 0:
                logger.info(f"  Throughput: {self.stats.get('throughput_articles_per_second', 0):.2f} articles/sec")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Kubernetes AI Topic Modeling Processor')
    ...
    parser.add_argument('--topic-model-type', default='LDA', choices=['LDA', 'UMAP'], help='Topic model type')
    ...
    parser.add_argument('--model-cache-dir', default='/app/models', help='Model cache directory')
    parser.add_argument('--gpu-cache-dir', default='/app/gpu-cache', help='GPU cache directory')
