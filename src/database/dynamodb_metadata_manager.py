...
class ArticleMetadataIndex:
    ...
    def to_dynamodb_item(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "source_date": "{0}#{1}".format(self.source, self.published_date),
            "date_source": "{0}#{1}".format(self.published_date, self.source),
            "category_date": "{0}#{1}".format(self.category, self.published_date),
            "year_month": (
                self.published_date[:7] if len(self.published_date) >= 7 else ""
            ),
...
class SearchQuery:
    ...
    date_range: Optional[Dict[str, str]] = (
        None  # {'start': 'YYYY-MM-DD', 'end': 'YYYY-MM-DD'}
    )
...
class DynamoDBMetadataManager:
    ...
    def _initialize_table(self):
        ...
        try:
            ...
            self.logger.info("DynamoDB table exists: {0}".format(self.table_name))
        except ClientError as e:
            if e.response["Error"]["Code"] == "ResourceNotFoundException":
                self.logger.info("Creating DynamoDB table: {0}".format(self.table_name))
                self._create_table()
            else:
                raise
    ...
    def _create_table(self):
        ...
        try:
            ...
            self.logger.error("Failed to create DynamoDB table: {0}".format(e))
            raise
    ...
    async def batch_index_articles(
        ...
        try:
            ...
            result = {
                ...
                "indexing_rate": (
                    len(articles) / (execution_time / 1000) if execution_time > 0 else 0
                ),
            }
            ...
            self.logger.error("Batch indexing failed: {0}".format(e))
            raise
            ...
            with self.table.batch_writer() as batch_writer:
                for article in batch:
                    try:
                        ...
                        self.logger.warning("Failed to index article: {0}".format(e))

        except Exception as e:
            self.logger.error("Batch write failed: {0}".format(e))
            failed = len(batch)
            failed_items = [
                {"article_id": article.get("id", "unknown"), "error": str(e)}
                for article in batch
            ]

        return {"indexed": indexed, "failed": failed, "failed_items": failed_items}
    ...
    async def get_articles_by_source(
        ...
        try:
            ...
            items = [
                ArticleMetadataIndex.from_dynamodb_item(item)
    ...
    async def get_articles_by_date_range(
        ...
        try:
            ...
            self.logger.error("Failed to query articles by date range: {0}".format(e))
            raise
        ...
        try:
            ...
            if match_all:
                ...
                for tag in tags:
                    ...
                    filter_expr = (
                        tag_expr if filter_expr is None else filter_expr & tag_expr
                    )
            else:
                # Any tag matches
                filter_expr = None
            ...
            self.logger.error("Failed to query articles by tags: {0}".format(e))
            raise

    async def get_articles_by_category(
        self, category: str, limit: int = 50, start_date: Optional[str] = None
    ) -> QueryResult:
        ...
        try:
            ...
            if start_date:
                ...
                ] & Key("published_date").gte(start_date)
            ...
            self.logger.error("Failed to query articles by category: {0}".format(e))
            raise
    ...
    async def search_articles(self, search_query: SearchQuery) -> QueryResult:
        ...
        try:
            ...
            self.logger.error("Full-text search failed: {0}".format(e))
            raise
    ...
    def _build_date_range_filter(self, date_range: Dict[str, str]):
        ...
        if "end" in date_range:
            ...
            filter_expr = (
                end_filter if filter_expr is None else filter_expr & end_filter
            )
    ...
    ) -> List[ArticleMetadataIndex]:
        """Score and sort search results by relevance."""
        scored_items = []

        for item in items:
            score = self._calculate_relevance_score(
                item, search_tokens, search_query.fields
            )
        ...
        for field in search_fields:
            ...
            for token in search_tokens:
                if token in field_text:
                    ...
                    if " {0} ".format(token) in " {0} ".format(field_text):
                        score += weight * 0.5
        ...
        try:
            ...
            return {
                ...
                "monthly_distribution": dict(
                    sorted(monthly_counts.items(), reverse=True)[:12]
                ),
                "execution_time_ms": execution_time,
                "table_info": {
            ...
            self.logger.error("Failed to get metadata statistics: {0}".format(e))
            raise
    ...
    async def update_article_metadata(
        ...
        try:
            ...
            self.logger.debug("Updated metadata for article: {0}".format(article_id))
            return True
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": (
                    "healthy"
                    if table_status == "ACTIVE" and read_success
                    else "unhealthy"
                ),
                "table_status": table_status,
                "table_name": self.table_name,
                "region": self.config.region,
                "read_capacity": response["Table"]["ProvisionedThroughput"][
                    "ReadCapacityUnits"
                ],
...
async def integrate_with_s3_storage(
    s3_metadata: Dict[str, Any], dynamodb_manager: DynamoDBMetadataManager
) -> ArticleMetadataIndex:
    """
...
async def integrate_with_redshift_etl(
    redshift_record: Dict[str, Any], dynamodb_manager: DynamoDBMetadataManager
) -> bool:
    """
    ...
    updates = {"redshift_loaded": True, "processing_status": "processed"}

    return await dynamodb_manager.update_article_metadata(article_id, updates)


async def sync_metadata_from_scraper(
    articles: List[Dict[str, Any]], dynamodb_manager: DynamoDBMetadataManager
) -> Dict[str, Any]:
    """
