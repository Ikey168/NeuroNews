...
class DynamoDBMetadataPipeline:
    ...
    def from_crawler(cls, crawler):
        ...
        config = DynamoDBMetadataConfig(
            table_name=settings.get('DYNAMODB_METADATA_TABLE', 'neuronews-article-metadata'),
    ...
    async def spider_closed(self, spider):
        ...
        self.logger.info(f"  - Average indexing time: {self.stats['total_processing_time']/max(1, self.stats['indexed_articles']):.3f}s")
    
    async def process_item(self, item, spider):
        """Process scraped item and index metadata."""
        if not self.manager:
            self.logger.warning("DynamoDB manager not initialized, skipping metadata indexing")
            return item
        ...
        try:
            ...
            self.logger.debug(f"Indexed metadata for article: {metadata.article_id} ({processing_time:.3f}s)")
...
class S3MetadataSync:
    ...
    async def sync_s3_storage_event(self, s3_metadata: Dict[str, Any]) -> Optional[ArticleMetadataIndex]:
        """
        ...
        try:
            metadata = await integrate_with_s3_storage(s3_metadata, self.dynamodb_manager)
    ...
    async def sync_multiple_s3_articles(self, s3_metadata_list: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            batch_result = await self.dynamodb_manager.batch_index_articles(article_data_list)
        ...
        self.logger.info(f"S3 metadata sync: {synced_count}/{len(s3_metadata_list)} articles ({sync_time:.2f}ms)")
        return result
...
class RedshiftMetadataSync:
    ...
    async def sync_redshift_load_event(self, redshift_record: Dict[str, Any]) -> bool:
        ...
        try:
            success = await integrate_with_redshift_etl(redshift_record, self.dynamodb_manager)
            if success:
                self.logger.debug(f"Updated metadata for Redshift load: {redshift_record.get('article_id')}")
            return success
        except Exception as e:
            self.logger.error(f"Failed to sync Redshift metadata: {e}")
            return False
    
    async def sync_batch_redshift_loads(self, redshift_records: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        self.logger.info(f"Redshift metadata sync: {updated_count}/{len(redshift_records)} updates ({sync_time:.2f}ms)")
        return result
...
class DataValidationMetadataSync:
    ...
    async def sync_validation_results(self, 
                                    article_id: str, 
        ...
        try:
            ...
            updates = {
                ...
                'source_reputation': validation_result.get('source_reputation', 'unknown'),
                'last_validated': datetime.now(timezone.utc).isoformat()
            }
            
            success = await self.dynamodb_manager.update_article_metadata(article_id, updates)
            if success:
                self.logger.debug(f"Updated validation metadata for article: {article_id}")
            return success
...
class MetadataIntegrationOrchestrator:
    ...
    def __init__(self, 
        ...
        # Initialize managers
        self.dynamodb_manager = DynamoDBMetadataManager(dynamodb_config, aws_credentials)
    ...
    async def process_scraped_article(self, article_data: Dict[str, Any]) -> Dict[str, Any]:
        """
...
def create_scrapy_pipeline_config(settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'DYNAMODB_METADATA_TABLE': settings.get('table_name', 'neuronews-article-metadata'),
...
async def sync_existing_s3_articles(s3_storage, dynamodb_manager: DynamoDBMetadataManager) -> Dict[str, Any]:
    """
