...
class S3ArticleStorage:
    ...
    async def store_raw_article(
        ...
        try:
            ...
            article_metadata = ArticleMetadata(
                ...
                url=article['url'],
                title=article['title'],
                published_date=article.get('published_date', enhanced_article['scraped_date'][:10]),
    ...
    async def store_processed_article(
        ...
        try:
            ...
            article_metadata = ArticleMetadata(
                ...
                published_date=article.get('published_date', enhanced_article['processed_date'][:10]),
        ...
        for i, article in enumerate(articles):
            try:
                ...
                error_metadata = ArticleMetadata(
                    ...
                    url=article.get('url', ''),
                    title=article.get('title', ''),
                    published_date=article.get('published_date', datetime.now(timezone.utc).strftime('%Y-%m-%d')),
                    scraped_date=datetime.now(timezone.utc).isoformat(),
                    content_hash='',
                    file_size=0,
                    s3_key='',
    ...
    async def get_storage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            processed_keys = await self.list_articles_by_prefix(ArticleType.PROCESSED.value)
            stats["processed_articles"]["count"] = len(processed_keys)
            
            # Calculate total
            stats["total_count"] = stats["raw_articles"]["count"] + stats["processed_articles"]["count"]
            ...
            if sample_keys:
                ...
                stats["raw_articles"]["total_size"] = int(avg_size * stats["raw_articles"]["count"])
                stats["processed_articles"]["total_size"] = int(avg_size * stats["processed_articles"]["count"])
...
async def ingest_scraped_articles_to_s3(
    ...
    """
    if not articles:
        return {"status": "success", "total_articles": 0, "stored_articles": 0, "errors": []}
    
    # Initialize S3 storage
    aws_key = aws_credentials.get('aws_access_key_id') if aws_credentials else None
    aws_secret = aws_credentials.get('aws_secret_access_key') if aws_credentials else None
...
async def verify_s3_data_consistency(
    ...
    try:
        ...
        raw_keys = await storage.list_articles_by_prefix(ArticleType.RAW.value, limit=sample_size//2)
        processed_keys = await storage.list_articles_by_prefix(ArticleType.PROCESSED.value, limit=sample_size//2)
