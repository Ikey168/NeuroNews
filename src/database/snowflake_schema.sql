-- Snowflake Schema for NeuroNews Articles
-- Migrated from Redshift to Snowflake-compatible syntax
-- Updated to use VARIANT for JSON fields and Snowflake best practices
...
CREATE TABLE news_articles (
    ...
    scraped_at TIMESTAMP_NTZ NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    ...
    dominant_topic VARIANT, -- JSON object of the most probable topic
    extraction_method VARCHAR(50), -- Method used for extraction (tfidf_lda, bert, etc.)
    extraction_processed_at TIMESTAMP_NTZ, -- When extraction was performed
    extraction_processing_time NUMBER(10,3) -- Processing time in seconds
)
...
CREATE TABLE article_summaries (
    ...
    confidence_score NUMBER(5,4), -- Quality confidence score
    processing_time NUMBER(10,4), -- Time taken to generate (seconds)
    
    -- Summary metrics
    word_count INTEGER,
    sentence_count INTEGER,
    compression_ratio NUMBER(5,4), -- Summary words / Original words
...
CLUSTER BY (created_at, summary_length, article_id);

-- Create staging table for batch loads
DROP TABLE IF EXISTS news_articles_staging;

CREATE TABLE news_articles_staging AS
SELECT * FROM news_articles WHERE 1=0; -- Create empty table with same structure
...
CREATE TABLE event_clusters (
    ...
    -- Quality metrics
    silhouette_score NUMBER(5,4), -- Clustering quality score
    cohesion_score NUMBER(5,4), -- How similar articles are within cluster
    separation_score NUMBER(5,4), -- How different cluster is from others
    
    -- Event detection metadata
    first_article_date TIMESTAMP_NTZ, -- When first article in cluster was published
    last_article_date TIMESTAMP_NTZ, -- When last article in cluster was published
    peak_activity_date TIMESTAMP_NTZ, -- When most articles were published
    event_duration_hours NUMBER(8,2), -- Duration of event in hours
    
    -- Geographic and source information
    primary_sources VARIANT, -- JSON array of main sources covering the event
    geographic_focus VARIANT, -- JSON array of locations mentioned
    key_entities VARIANT, -- JSON array of main entities (people, orgs) involved
    
    -- Event significance
    trending_score NUMBER(8,4), -- How trending/viral the event is
    impact_score NUMBER(5,2), -- Estimated impact/importance (1-100)
    velocity_score NUMBER(8,4), -- How fast the story is developing
...
CLUSTER BY (created_at, category, event_type);
...
CREATE TABLE article_cluster_assignments (
    ...
    -- Assignment metadata
    assignment_confidence NUMBER(5,4), -- How confident the clustering is
    distance_to_centroid NUMBER(10,6), -- Distance from cluster center
    is_cluster_representative BOOLEAN DEFAULT FALSE, -- Is this a key article for the cluster?
    
    -- Article contribution to cluster
    contribution_score NUMBER(5,4), -- How much this article contributes to cluster definition
    novelty_score NUMBER(5,4), -- How novel/unique this article is within cluster
    
    -- Processing metadata
    embedding_vector VARIANT, -- JSON array of the article's embedding vector (optional)
...
CLUSTER BY (assigned_at, cluster_id, assignment_confidence);
...
CREATE TABLE article_embeddings (
    ...
    embedding_vector VARIANT NOT NULL, -- JSON array of embedding values
    ...
    -- Quality metrics
    embedding_quality_score NUMBER(5,4), -- Quality assessment of embedding
    processing_time NUMBER(8,4), -- Time taken to generate embedding (seconds)
...
CLUSTER BY (created_at, embedding_model);

-- Create views for high-quality articles
...
SELECT 
    ...
    LISTAGG(DISTINCT a.source, ', ') as sources,
...
SELECT 
    ...
    ec.event_duration_hours,
    LISTAGG(DISTINCT a.title, ' | ') as sample_headlines,
...
SELECT 
    ...
    LISTAGG(DISTINCT ec.cluster_name, ' | ') as top_events
FROM event_clusters ec
WHERE 
    ec.status = 'active'
...
-- Snowflake automatically creates clustering keys above, but we can add search optimization
ALTER TABLE news_articles ADD SEARCH OPTIMIZATION ON EQUALITY(source, content_quality, source_credibility);
ALTER TABLE event_clusters ADD SEARCH OPTIMIZATION ON EQUALITY(category, event_type, status);
ALTER TABLE article_cluster_assignments ADD SEARCH OPTIMIZATION ON EQUALITY(cluster_id, article_id);

-- Enable automatic clustering
ALTER TABLE news_articles RESUME RECLUSTER;
ALTER TABLE article_summaries RESUME RECLUSTER;
ALTER TABLE event_clusters RESUME RECLUSTER;
ALTER TABLE article_cluster_assignments RESUME RECLUSTER;
ALTER TABLE article_embeddings RESUME RECLUSTER;
