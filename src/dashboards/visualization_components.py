...
class NewsVisualization:
    ...
    @staticmethod
    def create_timeline_chart(articles: List[Dict], title: str = "News Timeline") -> go.Figure:
        ...
        if 'category' in df.columns:
            daily_counts = df.groupby([df['published_date'].dt.date, 'category']).size().reset_index(name='count')
            daily_counts['date'] = pd.to_datetime(daily_counts['published_date'])
            
            fig = px.line(daily_counts, x='date', y='count', color='category',
                         title=title,
                         labels={'count': 'Number of Articles', 'date': 'Date'})
        else:
            daily_counts = df.groupby(df['published_date'].dt.date).size().reset_index(name='count')
            daily_counts['date'] = pd.to_datetime(daily_counts['published_date'])
    ...
    def create_sentiment_heatmap(articles: List[Dict]) -> go.Figure:
        ...
        sentiment_matrix = df.groupby(['date', 'hour'])['sentiment_score'].mean().reset_index()
        
        # Pivot for heatmap
        heatmap_data = sentiment_matrix.pivot(index='date', columns='hour', values='sentiment_score')
    ...
    def create_word_frequency_chart(articles: List[Dict], top_n: int = 20) -> go.Figure:
        ...
        for article in articles:
            ...
            if 'keywords' in article and article['keywords']:
                words = article['keywords'] if isinstance(article['keywords'], list) else [article['keywords']]
            elif 'topics' in article and article['topics']:
                words = article['topics'] if isinstance(article['topics'], list) else [article['topics']]
...
class NetworkVisualization:
    ...
    @staticmethod
    def create_entity_network(entities: List[Dict], relationships: Dict,
                ...
                for rel in rels.get('relationships', []):
                    ...
                    if target and target in G.nodes():
                        G.add_edge(entity_id, target,
                                  relationship=rel.get('type', rel.get('relation', 'related')),
        ...
        for edge in G.edges(data=True):
            ...
            edge_info.append(f"{edge[0]} → {edge[1]}<br>Type: {edge[2].get('relationship', 'related')}")
        
        edge_trace = go.Scatter(x=edge_x, y=edge_y,
                               line=dict(width=VIZ_CONFIG["network_graph"]["edge_width"], 
        ...
        for node in G.nodes(data=True):
            ...
            # Node info
            node_info = node[1]
            node_text.append(f"{node_info.get('label', node[0])}<br>"
                           f"Type: {node_info.get('type', 'unknown')}<br>"
                           f"Connections: {G.degree(node[0])}")
            
            # Color by type
            entity_type = node_info.get('type', 'unknown')
            node_color.append(VIZ_CONFIG["color_schemes"]["entities"].get(entity_type, '#FFEAA7'))
        ...
        for entity_id, rels in relationships.items():
            if entity_id in G.nodes():
                for rel in rels.get('relationships', []):
                    ...
                    if target in G.nodes() and rel_type in ['parent', 'contains', 'owns']:
