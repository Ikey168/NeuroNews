...
class DashboardAPI:
    ...
    def get_articles_by_topic(self, topic: str, limit: int = 50) -> List[Dict]:
        ...
        try:
            response = requests.get(f"{self.base_url}/news/articles/topic/{topic}?limit={limit}")
    ...
    def get_breaking_news(self, hours: int = 24, limit: int = 10) -> List[Dict]:
        ...
        try:
            response = requests.get(f"{self.base_url}/api/v1/breaking-news?hours={hours}&limit={limit}")
            if response.status_code == 200:
                return response.json()
            return []
        except Exception as e:
    ...
    def get_entity_relationships(self, entity_id: str) -> Dict:
        ...
        try:
            response = requests.get(f"{self.base_url}/graph/entity/{entity_id}/relationships")
            if response.status_code == 200:
                return response.json()
            return {}
        except Exception as e:
...
def create_entity_network_graph(entities: List[Dict], relationships: Dict) -> go.Figure:
    ...
    for entity_id, rels in relationships.items():
        for rel in rels.get('relationships', []):
            if 'target' in rel:
                G.add_edge(entity_id, rel['target'], 
    ...
    for node in G.nodes():
        ...
        node_text.append(f"{node_info.get('label', node)}<br>Type: {node_info.get('type', 'unknown')}")
        # Color by type
        type_colors = {'PERSON': 'red', 'ORG': 'blue', 'GPE': 'green', 'unknown': 'gray'}
...
def create_news_trends_chart(articles: List[Dict]) -> go.Figure:
    ...
    if 'published_date' in df.columns:
        ...
        fig = px.line(daily_counts, x='date', y='article_count',
...
def main():
        ...
        if articles:
            ...
            for i, article in enumerate(articles[:5]):  # Show first 5
                with st.expander(f"📄 {article.get('title', 'No title')[:100]}..."):
                    ...
                    st.write(f"**Summary:** {article.get('summary', 'No summary available')[:200]}...")
        else:
    ...
    if events:
        ...
        with col1:
            ...
            sorted_events = sorted(events, key=lambda x: x.get('impact_score', 0), reverse=True)[:5]
            for event in sorted_events:
                with st.expander(f"🎯 {event.get('cluster_name', 'Unknown Event')}"):
                    ...
                    st.write(f"**Duration:** {event.get('event_duration_hours', 0):.1f} hours")
        
        with col2:
            st.write("**Most Trending Events:**")
            trending_events = sorted(events, key=lambda x: x.get('trending_score', 0), reverse=True)[:5]
            for event in trending_events:
                with st.expander(f"📈 {event.get('cluster_name', 'Unknown Event')}"):
                    st.write(f"**Type:** {event.get('event_type', 'Unknown')}")
                    st.write(f"**Trending Score:** {event.get('trending_score', 0):.2f}")
                    st.write(f"**Velocity Score:** {event.get('velocity_score', 0):.2f}")
