...
class NeuroNewsAPIClient:
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if attempt == self.retry_attempts - 1:
                    raise APIError(f"API request failed after {self.retry_attempts} attempts: {e}")
    ...
    def get_breaking_news(_self, hours: int = 24, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            params = {
                "hours": hours,
                "limit": min(limit, DATA_CONFIG["max_events"])
            }
    ...
    def search_articles(self, query: str, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            params = {
                "q": query,
                "limit": min(limit, DATA_CONFIG["max_articles"])
            }
    ...
    def get_event_clusters(self, method: str = "kmeans", min_size: int = 3) -> List[Dict]:
...
class BatchAPIClient:
    ...
    async def fetch_multiple_entity_relationships(self, entity_ids: List[str]) -> Dict[str, Dict]:
            ...
            async def fetch_relationship(entity_id: str) -> tuple:
                async with semaphore:
                    try:
                        ...
                        async with session.get(url, timeout=self.api_client.timeout) as response:
    ...
    def get_multiple_entity_relationships(self, entity_ids: List[str]) -> Dict[str, Dict]:
        """Synchronous wrapper for fetching multiple entity relationships."""
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            return loop.run_until_complete(self.fetch_multiple_entity_relationships(entity_ids))
        except Exception as e:
