...
class CloudWatchLogger:
    ...
    async def _send_batch_metrics(self):
        ...
        try:
            ...
            successful_attempts = sum(1 for m in self.metrics_buffer if m.status == ScrapingStatus.SUCCESS)
            failed_attempts = total_attempts - successful_attempts
            avg_duration = sum(m.duration_ms for m in self.metrics_buffer) / total_attempts
