...
class TechCrunchSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        # Extract title
        item["title"] = (
            response.css("h1.article__title::text").get()
        ...
        content_paragraphs = (
            ...
            or response.css('div[class*="articleContent"] p::text').getall()
        )
        ...
        date_element = (
            response.css("time.full-date-time::attr(datetime)").get()
            or response.css(".byline time::attr(datetime)").get()
            or response.css(
        ...
        # Extract author
        author = (
            response.css('.byline a[rel="author"]::text').get()
