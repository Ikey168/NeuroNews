...
class PlaywrightNewsSpider(scrapy.Spider):
            ...
            yield Request(
                ...
                meta={
                    ...
                    "playwright_page_methods": [
                        PageMethod("wait_for_selector", "article, .article, .content, .news-item, .story"),
    ...
    async def parse(self, response):
        ...
        try:
            ...
            link_selectors = [
                "a.article-link", "a.headline", ".article a", ".news-item a", 
                "article a", ".story a", "a[href*='/article/']", "a[href*='/news/']"
            ]
            ...
            if not article_links:
                ...
                for link in all_links:
                    ...
                    if href:
                        ...
                        if any(keyword in href for keyword in ['/article/', '/news/', '/story/', '/post/']):
                            # Convert relative URLs to absolute
                            if href.startswith("/"):
                                href = response.urljoin(href)
                            article_links.append(href)
            ...
            for article_url in article_links[:10]:  # Limit to 10 articles per source for testing
                yield Request(
                    url=article_url,
                    ...
                    meta={
                        ...
                        "playwright_include_page": True,
                        "playwright_page_methods": [
                            PageMethod("wait_for_selector", "article, .article, .content, p"),
                            PageMethod("wait_for_timeout", 2000),
                        ],
                    }
                )
    ...
    async def parse_article(self, response):
        ...
        try:
            ...
            content_selectors = [
                ...
                "#main-content p", ".story-content p"
            ]
            ...
            for selector in date_selectors:
                ...
                if date_element:
                    if selector.startswith("meta"):
                        item['published_date'] = await date_element.get_attribute("content")
                    else:
            ...
            # Extract author
            author_selectors = [
                ".author", ".byline", "meta[name='author']", 
            ...
            # Extract category
            category_selectors = [
                ".category", ".section", "meta[property='article:section']",
            ...
            for selector in category_selectors:
                ...
                if category_element:
                    if selector.startswith("meta"):
                        item['category'] = await category_element.get_attribute("content")
                    else:
            ...
            if 'category' not in item or not item['category']:
                ...
                for category in ['news', 'tech', 'politics', 'business', 'sports', 'entertainment']:
