...
class BBCSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        # Extract title
        item['title'] = (
            response.css('h1[data-testid="headline"]::text').get() or
            response.css('h1.story-headline::text').get() or
        ...
        content_paragraphs = (
            response.css('div[data-component="text-block"] p::text').getall() or
            response.css('.story-body__inner p::text').getall() or
        ...
        date_element = (
            ...
            response.css('div[data-testid="timestamp"]::text').get() or
        ...
        # Extract author
        author = (
            response.css('div[data-testid="byline-name"]::text').get() or
    ...
    def _extract_category(self, url, response):
        ...
        if '/politics/' in url_lower:
            ...
            breadcrumb = response.css('nav[aria-label="Breadcrumb"] span::text').getall()
