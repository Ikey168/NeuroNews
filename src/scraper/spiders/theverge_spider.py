...
class VergeSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        # Extract title
        item['title'] = (
            response.css('h1.c-page-title::text').get() or
            response.css('h1[data-testid="ArticleTitle"]::text').get() or
        ...
        # Extract content
        content_paragraphs = (
            response.css('.c-entry-content p::text').getall() or
            response.css('div[data-testid="ArticleBody"] p::text').getall() or
        ...
        # Extract author
        author = (
            response.css('.c-byline__author-name::text').get() or
            response.css('span[data-testid="BylineAuthor"]::text').get() or
