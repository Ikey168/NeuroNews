...
class CNNSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in article_links:
            ...
            if '/index.html' in link or '/politics/' in link or '/business/' in link or '/tech/' in link:
    ...
    def parse_article(self, response):
        ...
        # Extract title
        item['title'] = (
            response.css('h1.headline__text::text').get() or
        ...
        content_paragraphs = response.css('.zn-body__paragraph::text, .zn-body__paragraph p::text').getall()
        if not content_paragraphs:
            content_paragraphs = response.css('div[data-component-name="ArticleBody"] p::text').getall()
        
        item['content'] = ' '.join(content_paragraphs).strip() if content_paragraphs else ''
        
        # Extract publication date
        date_element = (
            response.css('div.timestamp::text').get() or
            response.css('p.update-time::text').get() or
        ...
        # Extract author
        author = (
            response.css('.byline__name::text').get() or
            response.css('span.metadata__byline__author::text').get() or
