...
class GuardianSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        # Extract title
        item['title'] = (
            response.css('h1[data-gu-name="headline"]::text').get() or
            response.css('h1.content__headline::text').get() or
        ...
        # Extract content
        content_paragraphs = (
            response.css('div[data-gu-name="body"] p::text').getall() or
            response.css('.content__article-body p::text').getall() or
