...
class MultiSourceRunner:
    ...
    def run_spider(self, spider_name):
        ...
        if spider_name not in self.spiders:
            raise ValueError(f"Spider '{spider_name}' not found. Available: {list(self.spiders.keys())}")
    ...
    def run_all_spiders(self, exclude=None, include_only=None):
        ...
        if include_only:
            spiders_to_run = {k: v for k, v in self.spiders.items() if k in include_only}
        else:
            ...
            if filename.endswith('_articles.json'):
                ...
                try:
                    ...
                    report['sources'][source_name] = {
                        ...
                        'avg_content_length': sum(len(a.get('content', '')) for a in articles) / max(1, article_count),
                        'avg_validation_score': sum(a.get('validation_score', 0) for a in articles) / max(1, article_count),
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Multi-source news scraper for NeuroNews')
    ...
    parser.add_argument('--report', action='store_true', help='Generate scraping report')
