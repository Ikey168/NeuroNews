...
class MultiLanguagePipeline:
    ...
    def open_spider(self, spider):
        ...
        required_settings = [
            self.redshift_host, self.redshift_database,
    ...
    def process_item(self, item: NewsItem, spider) -> NewsItem:
        ...
        try:
            ...
            if result.get('errors'):
                ...
                logger.warning(f"Translation errors for {article_data['id']}: {result['errors']}")
            ...
            logger.info(f"Successfully processed article: {article_data['id']} "
    ...
    def _validate_item(self, item: NewsItem) -> bool:
        ...
        if len(content.strip()) < self.min_content_length:
            logger.warning(f"Content too short: {len(content)} < {self.min_content_length}")
            return False
