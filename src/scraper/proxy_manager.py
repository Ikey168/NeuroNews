...
class ProxyRotationManager:
    ...
    def __init__(self, config_path: Optional[str] = None, rotation_strategy: str = "round_robin"):
    ...
    def _load_config(self):
        ...
        try:
            ...
            self.health_check_interval = proxy_settings.get('health_check_interval', 300)
    ...
    def _is_proxy_healthy(self, proxy: ProxyConfig) -> bool:
        ...
        key = f"{proxy.host}:{proxy.port}"
        stats = self.proxy_stats.get(key, ProxyStats())
        return stats.is_healthy and self.active_connections[key] < proxy.concurrent_limit
    
    async def record_request(self, proxy: ProxyConfig, success: bool, response_time: float = 0.0):
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        self.logger.debug(f"Proxy {key} health check passed: {data.get('origin')}")
                        return True
                    else:
                        self.logger.warning(f"Proxy {key} returned status {response.status}")
                        return False
    ...
    async def start_health_monitor(self):
        ...
        if self.health_check_interval > 0:
            self.logger.info(f"Starting proxy health monitor (interval: {self.health_check_interval}s)")
            while True:
                try:
