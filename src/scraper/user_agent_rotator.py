...
class UserAgentRotator:
    ...
    def _rotate_profile(self):
        ...
        self.logger.info(f"Rotated to new browser profile: {self.current_profile.browser_name} {self.current_profile.version}")
    ...
    def _initialize_default_profiles(self):
        ...
        self.browser_profiles = (
            ...
            safari_profiles + edge_profiles + mobile_profiles
        )
        
        self.logger.info(f"Initialized {len(self.browser_profiles)} browser profiles")
    ...
    def load_config(self, config_file: str):
        ...
        try:
            ...
            if not config_path.exists():
                self.logger.warning(f"User-Agent config file not found: {config_file}, using defaults")
                self._initialize_default_profiles()
                return
            ...
            self.session_duration_range = tuple(settings.get('session_duration_range', [300, 1800]))
            self.max_profile_usage = settings.get('max_profile_usage', 100)
            
            self.logger.info(f"Loaded {len(self.browser_profiles)} browser profiles from {config_file}")
            
        except Exception as e:
            self.logger.error(f"Error loading user-agent config: {e}")
            self._initialize_default_profiles()
    ...
    def get_profile(self, force_new: bool = False) -> BrowserProfile:
        ...
        should_rotate = (
            ...
            self.profile_usage_count >= self.max_profile_usage or
            (self.session_start_time and 
             current_time - self.session_start_time > random.randint(*self.session_duration_range))
        )
    ...
    def _rotate_profile(self):
        ...
        self.logger.info(f"Rotated to new profile: {self.current_profile.browser_name} "
    ...
    def get_stats(self) -> Dict[str, any]:
        ...
        return {
            ...
            'current_profile': {
                'browser': self.current_profile.browser_name if self.current_profile else None,
                ...
                'is_mobile': self.current_profile.is_mobile if self.current_profile else False
            },
            'session_info': {
            ...
            'profile_distribution': {
                profile.browser_name: len([p for p in self.browser_profiles if p.browser_name == profile.browser_name])
