...
class FailedUrl:
    ...
    def from_dynamodb_item(cls, item: Dict[str, Any]) -> 'FailedUrl':
        ...
        return cls(
            ...
            is_permanent_failure=item.get('is_permanent_failure', {}).get('BOOL', False)
        )
...
class DynamoDBFailureManager:
    ...
    async def record_failure(self, 
                           ...
                           proxy_used: Optional[str] = None,
                           user_agent_used: Optional[str] = None,
                           response_code: Optional[int] = None) -> FailedUrl:
        """
        ...
        try:
            ...
            self.dynamodb.put_item(
                TableName=self.table_name,
                Item=failed_url.to_dynamodb_item()
            )
            
            self.logger.info(f"Recorded failure for URL: {url} (attempt {failed_url.retry_count})")
            return failed_url
    ...
    async def get_failure_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            permanent_failures = sum(1 for item in items if item.get('is_permanent_failure', {}).get('BOOL', False))
            pending_retries = sum(1 for item in items if not item.get('is_permanent_failure', {}).get('BOOL', False))
