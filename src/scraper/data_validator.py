...
class ScrapedDataValidator:
        ...
        for filename in os.listdir(self.sources_dir):
            if filename.endswith('_articles.json'):
                ...
                try:
                    ...
                    results[source_name] = self.validate_source_data(articles, source_name)
            ...
            if url:
                try:
                    ...
                    if parsed.scheme and parsed.netloc:
                        ...
                        if expected_domain.lower() in domain or domain in expected_domain.lower():
                            domain_matches += 1
                except:
                    pass
    ...
    def _check_duplicates(self, articles):
        ...
        return {
            ...
            'uniqueness_percentage': ((len(articles) - max(url_duplicates, title_duplicates)) / len(articles)) * 100
        }
    ...
    def _calculate_accuracy_score(self, validations):
        ...
        required_avg = sum(field['percentage'] for field in completeness.values() if field['required']) / 5
    ...
    def _generate_summary(self, results):
        ...
        total_articles = sum(results[source].get('total_articles', 0) for source in sources if 'error' not in results[source])
        avg_accuracy = sum(results[source].get('accuracy_score', 0) for source in sources if 'error' not in results[source]) / len(sources) if sources else 0
        
        best_source = max(sources, key=lambda s: results[s].get('accuracy_score', 0) if 'error' not in results[s] else 0)
        worst_source = min(sources, key=lambda s: results[s].get('accuracy_score', 0) if 'error' not in results[s] else 0)
...
def main():
    ...
    for source, data in results.items():
        if source != 'summary' and 'error' not in data:
            print(f"{source.upper()}: {data.get('accuracy_score', 0):.2f}% accuracy, {data.get('total_articles', 0)} articles")
