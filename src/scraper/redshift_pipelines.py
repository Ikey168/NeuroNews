    ...
    def open_spider(self, spider: Spider):
        ...
        try:
            ...
            try:
                ...
                spider.logger.warning(f"Schema initialization failed (may already exist): {e}")
    ...
    def process_item(self, item: Dict[str, Any], spider: Spider) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._is_validated_item(item):
                spider.logger.warning(f"Item missing validation metadata: {item.get('url', 'Unknown')}")
            ...
            spider.logger.debug(f"Article queued for Redshift storage: {item.get('url', 'Unknown')}")
            return item
...
""")
            ...
            spider.crawler.stats.set_value('redshift_processed_count', self.stats['processed_count'])
            spider.crawler.stats.set_value('redshift_stored_count', self.stats['stored_count'])
            spider.crawler.stats.set_value('redshift_failed_count', self.stats['failed_count'])
            spider.crawler.stats.set_value('redshift_success_rate', 
                                          (self.stats['stored_count'] / max(1, self.stats['processed_count'])) * 100)
...
class RedshiftAnalyticsPipeline:
    ...
    def from_crawler(cls, crawler):
        ...
        return cls(
            ...
            analytics_table=crawler.settings.get('REDSHIFT_ANALYTICS_TABLE', 'article_analytics')
        )
    
    def open_spider(self, spider: Spider):
        """Initialize analytics connection."""
        if not self.redshift_host:
            spider.logger.warning("REDSHIFT_HOST not configured, analytics pipeline disabled")
            return
    ...
    def _process_analytics_batch(self, spider: Spider, final: bool = False):
        ...
        try:
            ...
            spider.logger.info(f"Processing analytics batch: {len(self.analytics_batch)} records")
