...
from src.scraper.async_scraper_engine import AsyncNewsScraperEngine, ASYNC_NEWS_SOURCES, NewsSource
...
class AsyncScraperRunner:
    ...
    async def run_scraper(self, sources: list, test_mode: bool = False) -> list:
        ...
        self.logger.info(f"📊 Configuration: {self.config['max_concurrent']} concurrent, {self.config['max_threads']} threads")
    ...
    def print_summary(self, articles: list, stats: dict):
        ...
        for source, source_stats in stats['source_stats'].items():
            print(f"  {source}: {source_stats['articles']} articles, {source_stats['errors']} errors")
...
def main():
    ...
    parser.add_argument("--sources", "-s", help="Comma-separated list of sources to scrape")
