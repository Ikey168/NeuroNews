...
def run_spider(
    ...
    if s3_storage:
        ...
        pipelines["src.scraper.pipelines.s3_pipeline.S3StoragePipeline"] = settings.get(
            "S3_PIPELINE_PRIORITY", 400
        )
    ...
    if (
        cloudwatch_logging
        and cloudwatch_log_stream_prefix is None
        and "cloudwatch_logging" in aws_config
    ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NeuroNews Scraper - Multi-Source News Scraping"
    )
    ...
    source_group.add_argument(
        "--exclude", nargs="+", help="Exclude specific spiders from multi-source run"
    )
    source_group.add_argument(
        "--include", nargs="+", help="Include only specific spiders in multi-source run"
    )
    source_group.add_argument(
        "--validate", action="store_true", help="Run data validation after scraping"
    )
    source_group.add_argument(
    ...
    s3_group.add_argument(
        "--s3", "-s", action="store_true", help="Store articles in Amazon S3"
    )
