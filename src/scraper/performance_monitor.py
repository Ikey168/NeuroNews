...
class PerformanceDashboard:
    ...
    def _update_metrics_history(self):
        ...
        metrics = {
            ...
            'system': {
                'cpu_usage': list(self.system_metrics['cpu_usage'])[-1] if self.system_metrics['cpu_usage'] else 0,
                'memory_mb': list(self.system_metrics['memory_usage'])[-1] if self.system_metrics['memory_usage'] else 0
            },
    ...
    def get_current_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'system': {
                ...
                'avg_cpu': sum(self.system_metrics['cpu_usage']) / max(len(self.system_metrics['cpu_usage']), 1),
                'avg_memory': sum(self.system_metrics['memory_usage']) / max(len(self.system_metrics['memory_usage']), 1)
            },
    ...
    def _get_source_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        for source, metrics in self.source_metrics.items():
            ...
            stats[source] = {
                ...
                'success_rate': (
                    metrics['articles'] / 
                    max(metrics['articles'] + metrics['errors'], 1) * 100
                )
            }
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        for metrics in recent_metrics:
            trend_data['articles_per_second_trend'].append(metrics['articles_per_second'])
        ...
        return {
            ...
            'trends': trend_data,
            'insights': insights,
            'recommendations': self._generate_recommendations(current_metrics, insights)
        }
    
    def _generate_insights(self, metrics: Dict[str, Any], trends: Dict[str, List[float]]) -> List[str]:
        """Generate performance insights."""
        insights = []
        
        # Throughput analysis
        if metrics['articles_per_second'] > 5:
            insights.append("🚀 Excellent throughput: Scraper is performing at high speed")
        elif metrics['articles_per_second'] > 2:
            insights.append("⚡ Good throughput: Scraper is performing well")
        else:
            insights.append("🐌 Low throughput: Consider optimizing concurrent connections")
        ...
        if metrics['success_rate'] > 95:
            ...
            insights.append("⚠️ Low success rate: Check network conditions and rate limiting")
        
        # Memory usage analysis
        avg_memory = metrics['system']['avg_memory']
        if avg_memory > 1000:
            insights.append("🔥 High memory usage: Consider reducing concurrent connections")
        elif avg_memory > 500:
        ...
        if len(trends['articles_per_second_trend']) > 5:
            ...
            if recent_avg > earlier_avg * 1.1:
                ...
                insights.append("📉 Performance declining: Throughput trending downward")
    ...
    def _generate_recommendations(self, metrics: Dict[str, Any], insights: List[str]) -> List[str]:
        ...
        if metrics['success_rate'] < 90:
            ...
            recommendations.append("🌐 Check network connectivity and target site availability")
        
        # Based on throughput
        if metrics['articles_per_second'] < 2:
            recommendations.append("⚡ Increase concurrent connections if system resources allow")
            recommendations.append("🔄 Consider using more thread pool workers")
        
        # Based on memory usage
        if metrics['system']['avg_memory'] > 800:
            recommendations.append("💾 Reduce concurrent connections to lower memory usage")
        ...
        if problematic_sources:
            recommendations.append(f"🎯 Review configuration for problematic sources: {', '.join(problematic_sources)}")
        
        # Based on concurrent connections
        if metrics['counters']['concurrent_connections'] < 5:
            recommendations.append("🚀 Consider increasing concurrent connections for better throughput")
    ...
    def print_live_dashboard(self):
        ...
        for source, stats in metrics['sources'].items():
            print(f"   {source}: {stats['articles']} articles, {stats['success_rate']:.1f}% success")
