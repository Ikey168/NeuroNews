...
class AsyncPipelineProcessor:
    ...
    async def process_articles_async(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        self.logger.info(f"🔄 Processing {len(articles)} articles through async pipeline")
        ...
        for article in articles:
            task = asyncio.create_task(
                self.process_single_article(semaphore, article)
            )
            tasks.append(task)
        ...
        self.logger.info(f"✅ Pipeline processing completed: {len(valid_articles)} valid articles")
        self.logger.info(f"📊 Validation: {self.validation_stats['passed']} passed, {self.validation_stats['failed']} failed")
        self.logger.info(f"🔍 Duplicates: {self.duplicate_stats['unique']} unique, {self.duplicate_stats['duplicates']} duplicates")
        
        return valid_articles
    
    async def process_single_article(self, semaphore: asyncio.Semaphore, article: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        async with semaphore:
            try:
                ...
                self.logger.error(f"Error processing article {article.get('url', 'unknown')}: {e}")
                return None
    ...
    def extract_category(self, article: Dict[str, Any]) -> str:
        ...
        categories = {
            ...
            'Politics': ['politics', 'political', 'government', 'election', 'vote', 'congress', 'senate'],
            'Business': ['business', 'finance', 'economy', 'market', 'stock', 'economic', 'trade'],
            'Sports': ['sports', 'sport', 'football', 'basketball', 'baseball', 'soccer', 'tennis'],
            'Health': ['health', 'medical', 'medicine', 'doctor', 'hospital', 'covid', 'vaccine'],
            ...
            'World': ['world', 'international', 'global', 'foreign', 'country', 'nation']
        }
    ...
    def extract_keywords(self, content: str, max_keywords: int = 10) -> List[str]:
        ...
        stop_words = {
            ...
            'than', 'them', 'well', 'were', 'what', 'your', 'about', 'after', 'back',
            'could', 'first', 'into', 'most', 'only', 'other', 'right', 'should',
            'these', 'think', 'through', 'would', 'said', 'says', 'according'
        }
    ...
    def analyze_sentiment(self, content: str) -> str:
        ...
        positive_words = {
            ...
            'success', 'win', 'victory', 'progress', 'improve', 'better', 'best',
            'happy', 'pleased', 'excited', 'optimistic', 'hopeful'
        }
        
        negative_words = {
            'bad', 'terrible', 'awful', 'horrible', 'negative', 'fail', 'failure',
            'loss', 'decline', 'worse', 'worst', 'sad', 'angry', 'disappointed',
            'crisis', 'problem', 'issue', 'concern', 'worry', 'fear'
        }
    ...
    def extract_entities(self, content: str) -> Dict[str, List[str]]:
        ...
        org_pattern = r'\b[A-Z][a-z]+(?: [A-Z][a-z]+)*(?: Inc\.?| Corp\.?| Ltd\.?| LLC)\b'
    ...
    async def save_processed_articles(self, articles: List[Dict[str, Any]], output_path: str):
        """Save processed articles to file."""
        async with aiofiles.open(output_path, 'w') as f:
            await f.write(json.dumps(articles, indent=2, ensure_ascii=False))
    
    async def upload_to_s3_async(self, articles: List[Dict[str, Any]], bucket: str, key: str) -> bool:
        ...
        try:
            ...
            await loop.run_in_executor(
                self.thread_pool,
                self.upload_to_s3_sync,
                data, bucket, key
            )
            
            self.logger.info(f"✅ Uploaded {len(articles)} articles to s3://{bucket}/{key}")
            return True
