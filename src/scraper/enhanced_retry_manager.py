...
from .sns_alert_manager import SNSAlertManager, AlertType, AlertSeverity
...
class EnhancedRetryManager:
        ...
        for attempt in range(config.max_retries + 1):
            ...
            try:
                ...
                if attempt >= config.max_retries:
                    # Final failure
                    duration_ms = int((time.time() - start_time) * 1000)
                    await self._record_final_failure(url, str(e), attempt, duration_ms, context)
                ...
                # Log retry attempt
                self.logger.warning(f"Retry attempt {attempt + 1}/{config.max_retries} for {url} "
                                  f"after {delay:.2f}s delay. Reason: {retry_reason.value}")
                
                # Record retry metrics
                await self._record_retry_attempt(url, attempt, retry_reason, delay, str(e), context)
    ...
    def _determine_retry_reason(self, exception: Exception, context: Dict[str, Any]) -> RetryReason:
    ...
    def _is_permanent_failure(self, exception: Exception, context: Dict[str, Any]) -> bool:
    ...
    async def _record_success(self, url: str, duration_ms: int, retry_count: int, context: Dict[str, Any]):
        ...
        self.logger.info(f"Successfully scraped {url} after {retry_count} retries in {duration_ms}ms")
    
    async def _record_retry_attempt(self, url: str, attempt: int, reason: RetryReason, 
                                  delay: float, error: str, context: Dict[str, Any]):
    ...
    async def _record_final_failure(self, url: str, error: str, retry_count: int, 
                                   duration_ms: int, context: Dict[str, Any]):
        ...
        self.logger.error(f"Final failure for {url} after {retry_count} retries: {error}")
    
    async def _record_permanent_failure(self, url: str, error: str, attempt: int, context: Dict[str, Any]):
        ...
        try:
            ...
            for failed_url in failed_urls:
                try:
                    ...
                    self.logger.info(f"URL ready for retry: {failed_url.url} "
                                   f"(attempt {failed_url.retry_count + 1})")
                    processed_urls.append(failed_url.url)
                    
                except Exception as e:
                    self.logger.error(f"Error processing retry for {failed_url.url}: {e}")
