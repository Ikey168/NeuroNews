...
class EnhancedValidationPipeline:
    ...
    def process_item(self, item: Item, spider) -> Item:
        ...
        if result is None:
            ...
            spider.logger.info(f"Article dropped by validation pipeline: {article_dict.get('url', 'Unknown URL')}")
        ...
        # Log warnings if any
        if result.warnings:
            spider.logger.warning(f"Article validation warnings for {item.get('url', 'Unknown URL')}: {result.warnings}")
        
        # Log quality info
        quality = result.cleaned_data.get('content_quality', 'unknown')
        spider.logger.debug(f"Article validated - Quality: {quality}, Score: {result.score:.1f}")
...
==========================================
    """)
        ...
        spider.crawler.stats.set_value('validation_accepted_count', stats['accepted_count'])
        spider.crawler.stats.set_value('validation_rejected_count', stats['rejected_count'])
        spider.crawler.stats.set_value('validation_acceptance_rate', stats['acceptance_rate'])
...
class QualityFilterPipeline:
    ...
    def process_item(self, item: Item, spider) -> Item:
        ...
        if validation_score < self.min_score:
            ...
            spider.logger.info(f"Article filtered - low score ({validation_score:.1f}): {item.get('url', 'Unknown URL')}")
            raise DropItem(f"Article validation score ({validation_score:.1f}) below threshold ({self.min_score})")
        
        # Check content length
        content_length = item.get('content_length', 0)
        if content_length < self.min_content_length:
            self.filtered_count += 1
            spider.logger.info(f"Article filtered - short content ({content_length}): {item.get('url', 'Unknown URL')}")
            raise DropItem(f"Article content length ({content_length}) below threshold ({self.min_content_length})")
        
        # Check for critical validation flags
        validation_result = item.get('validation_result', {})
        issues = validation_result.get('issues', [])
        
        critical_issues = ['missing_title', 'missing_content', 'banned_domain', 'placeholder_content']
        for issue in critical_issues:
            if issue in issues:
                self.filtered_count += 1
                spider.logger.warning(f"Article filtered - critical issue ({issue}): {item.get('url', 'Unknown URL')}")
    ...
    def close_spider(self, spider):
        ...
        spider.logger.info(f"Quality filter removed {self.filtered_count} low-quality articles")
...
class SourceCredibilityPipeline:
    ...
    def process_item(self, item: Item, spider) -> Item:
        ...
        self.credibility_stats[credibility] = self.credibility_stats.get(credibility, 0) + 1
        
        # Block unreliable sources if configured
        if self.block_unreliable and credibility == 'unreliable':
            self.blocked_count += 1
            spider.logger.warning(f"Article blocked - unreliable source: {item.get('url', 'Unknown URL')}")
...
class DuplicateFilterPipeline:
    ...
    def process_item(self, item: Item, spider) -> Item:
        ...
        if is_duplicate:
            ...
            spider.logger.info(f"Duplicate article detected ({reason}): {item.get('url', 'Unknown URL')}")
...
class ValidationReportPipeline:
    ...
    def from_crawler(cls, crawler):
        """Create pipeline instance from crawler."""
        settings = crawler.settings
        report_file = settings.get('VALIDATION_REPORT_FILE', 'data/validation_report.json')
    ...
    def _generate_report(self, spider) -> Dict[str, Any]:
        ...
        best_source = max(source_avg_scores.items(), key=lambda x: x[1]) if source_avg_scores else ('unknown', 0)
        worst_source = min(source_avg_scores.items(), key=lambda x: x[1]) if source_avg_scores else ('unknown', 0)
