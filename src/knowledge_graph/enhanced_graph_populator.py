...
class EnhancedKnowledgeGraphPopulator:
    ...
    async def populate_from_article(self, 
                                   article_id: str,
                                   title: str,
                                   content: str,
        ...
        try:
            logger.info(f"Processing article {article_id} for enhanced knowledge graph population")
    ...
    async def populate_from_articles_batch(self, 
    ...
    async def _add_article_vertex(self, 
                                 article_id: str,
                                 title: str,
                                 content: str,
    ...
    async def _process_entities_batch(self, 
                                    ...
                                    article_id: str,
                                    article_vertex_id: str) -> Dict[str, Any]:
        ...
        for entity in entities:
            try:
                ...
                entity_vertex_id = existing_vertex_id or self.entity_registry.get(entity.entity_id)
                if entity_vertex_id:
                    await self._link_entity_to_article(entity, article_vertex_id, entity_vertex_id)
    ...
    async def _process_relationships_batch(self, 
        ...
        for relationship in relationships:
            try:
                ...
                source_vertex_id = self.entity_registry.get(relationship.source_entity.entity_id)
                target_vertex_id = self.entity_registry.get(relationship.target_entity.entity_id)
    ...
    async def _merge_entity_data(self, vertex_id: str, entity: EnhancedEntity):
        ...
        try:
            ...
            if entity.aliases:
                ...
                existing_aliases = json.loads(existing_vertex.get('aliases', '[]')) if existing_vertex else []
    ...
    async def _link_entity_to_article(self, 
                                     entity: EnhancedEntity,
                                     article_vertex_id: str,
    ...
    async def _link_to_historical_entities(self, 
                                          entities: List[EnhancedEntity],
                                          article_id: str) -> List[Dict[str, Any]]:
        ...
        for entity in entities:
            try:
                ...
                logger.error(f"Error linking entity {entity.text} to historical data: {e}")
                continue
    ...
    async def query_entity_relationships(self, 
                                       entity_name: str,
                                       max_depth: int = 2,
    ...
    async def execute_sparql_query(self, sparql_query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning("SPARQL queries not yet implemented - using Gremlin fallback")
    ...
    async def _update_vertex_properties(self, vertex_id: str, properties: Dict[str, Any]):
    ...
    async def validate_graph_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            entity_types = ['Person', 'Organization', 'Technology', 'Policy', 'Location', 'Entity']
            for entity_type in entity_types:
                ...
                validation_results['entity_counts'][entity_type] = count_results[0] if count_results else 0
            
            # Count relationships by type
            relationship_types = ['WORKS_FOR', 'PARTNERS_WITH', 'COMPETES_WITH', 'DEVELOPS', 'USES_TECHNOLOGY']
            for rel_type in relationship_types:
                count_query = self.graph_builder.g.E().hasLabel(rel_type).count()
                count_results = await self.graph_builder._execute_traversal(count_query)
                validation_results['relationship_counts'][rel_type] = count_results[0] if count_results else 0
            
            # Find orphaned entities (entities with no relationships)
            orphaned_query = self.graph_builder.g.V().where(__.bothE().count().is_(0)).count()
            orphaned_results = await self.graph_builder._execute_traversal(orphaned_query)
            validation_results['orphaned_entities'] = orphaned_results[0] if orphaned_results else 0
...
def create_enhanced_knowledge_graph_populator(neptune_endpoint: str) -> EnhancedKnowledgeGraphPopulator:
...
def create_high_performance_graph_populator(neptune_endpoint: str) -> EnhancedKnowledgeGraphPopulator:
...
def create_high_quality_graph_populator(neptune_endpoint: str) -> EnhancedKnowledgeGraphPopulator:
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            ...
            print(f"  • Average entities per article: {stats['entities_per_article']:.1f}")
            print(f"  • Average relationships per article: {stats['relationships_per_article']:.1f}")
