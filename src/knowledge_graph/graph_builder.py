...
class GraphBuilder:
    ...
    async def connect(self):
        ...
        try:
            ...
            self.g = traversal().withRemote(self.connection)
            logger.info("Successfully connected to Gremlin server and graph traversal source 'g' initialized.")
    ...
    async def add_vertex(self, label: str, properties: dict):
        ...
        if not self.g: await self.connect()
    ...
    async def add_relationship(self, from_vertex_id: str, to_vertex_id: str, rel_type: str, properties: dict = None):
        """Adds a relationship (edge) between two vertices."""
        if not self.g: await self.connect()
        
        t = self.g.V().has('id', from_vertex_id).addE(rel_type).to(__.V().has('id', to_vertex_id)) # Changed self.g to __
        if properties:
            for key, value in properties.items():
                t = t.property(key, value)
    ...
    async def close(self):
        ...
        if self.client: # Changed from self.connection to self.client
            try:
                ...
                logger.error(f"Error closing Gremlin client connection asynchronously: {e}")
            finally:
                self.client = None
                self.connection = None # Also nullify the DriverRemoteConnection if used by 'g'
                self.g = None
        else:
