...
class IngestionMetrics:
    ...
    def update_metrics(self, processing_time: float, success: bool, error_type: str = None):
        ...
        if self.total_articles_processed > 0:
            self.average_processing_time = self.total_processing_time / self.total_articles_processed
            self.throughput_articles_per_second = self.total_articles_processed / self.total_processing_time
    
    def get_summary(self) -> Dict[str, Any]:
        """Get a summary of ingestion metrics."""
        success_rate = (self.successful_articles / max(self.total_articles_processed, 1)) * 100
        ...
        return {
            ...
            'performance': {
                ...
                'throughput_articles_per_second': round(self.throughput_articles_per_second, 2),
        ...
        while not self._stop_monitoring:
            try:
                ...
                if self.current_usage > self.max_memory_mb * 0.8:
                    import gc
                    gc.collect()
                    logger.warning(f"High memory usage detected: {self.current_usage:.1f}MB, triggered GC")
...
class AdaptiveBatchProcessor:
    ...
    def __init__(self, initial_batch_size: int = 100, min_size: int = 10, max_size: int = 1000):
    ...
    def adjust_batch_size(self, processing_time: float, success_rate: float):
        ...
        if recent_avg > historical_avg * 1.1:
            ...
            new_size = min(self.max_size, int(self.current_batch_size * self.adjustment_factor))
        elif recent_avg < historical_avg * 0.9:
            # Performance is degrading, try smaller batches
            new_size = max(self.min_size, int(self.current_batch_size / self.adjustment_factor))
        else:
            # Performance is stable
            return
        
        logger.info(f"Adjusting batch size from {self.current_batch_size} to {new_size} "
...
class OptimizedIngestionPipeline:
    ...
    def __init__(self, config: OptimizationConfig = None):
        ...
        logger.info(f"Optimized ingestion pipeline initialized with config: {self.config}")
    
    async def process_articles_async(self, articles: List[Dict[str, Any]], 
                                   storage_backends: List[Callable] = None) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(f"Starting optimized ingestion pipeline for {len(articles)} articles")
            ...
            if total_time > 0:
                self.metrics.throughput_articles_per_second = len(processed_articles) / total_time
            
            logger.info(f"Pipeline processing completed: {len(processed_articles)} articles "
    ...
    def _create_adaptive_batches(self, articles: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
        ...
        for article in articles:
            ...
            if (current_batch_size + estimated_memory > self.config.max_memory_usage_mb * 1024 / 10 or
                len(current_batch) >= self.batch_processor.get_batch_size()):
    ...
    async def _process_batch_async(self, semaphore: asyncio.Semaphore, 
                                 ...
                                 batch_id: int,
                                 storage_backends: List[Callable] = None) -> List[Dict[str, Any]]:
        ...
        async with semaphore:
            ...
            try:
                ...
                for article in batch:
                    task = asyncio.create_task(self._process_single_article_async(article))
                    tasks.append(task)
                ...
                for result in article_results:
                    if isinstance(result, dict):
                        ...
                        logger.warning(f"Article processing failed in batch {batch_id}: {result}")
                ...
                logger.debug(f"Batch {batch_id} completed: {len(processed_articles)}/{len(batch)} "
    ...
    async def _process_single_article_async(self, article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Article processing failed for {article.get('url', 'unknown')}: {e}")
            return None
    ...
    async def _enhance_article_async(self, article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        enhanced['reading_time'] = max(1, enhanced['word_count'] // 200)  # Assume 200 WPM
    ...
    async def _store_batch_async(self, articles: List[Dict[str, Any]], 
                               storage_backends: List[Callable]):
        ...
        try:
            ...
            for backend in storage_backends:
                task = asyncio.create_task(self._execute_storage_backend(backend, articles))
    ...
    async def _execute_storage_backend(self, backend: Callable, articles: List[Dict[str, Any]]):
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'batch_processor': {
                ...
                'performance_history_length': len(self.batch_processor.performance_history)
            },
            'circuit_breaker': {
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            ...
            print(f"Throughput: {len(results['processed_articles']) / results['processing_time']:.2f} articles/sec")
            print("\nMetrics:")
