...
class KnowledgeGraphDemo:
    ...
    async def _demo_relationship_detection(self):
        ...
        try:
            ...
            for i, rel in enumerate(relationships[:5]):  # Show first 5 relationships
                print(f"\n  Relationship {i+1}:")
                print(f"    Source: {rel.source_entity.text} ({rel.source_entity.label})")
                print(f"    Target: {rel.target_entity.text} ({rel.target_entity.label})")
    ...
    async def _demo_graph_querying(self):
        ...
        try:
            ...
            for query_info in gremlin_queries:
                ...
                mock_results = await self._mock_execute_gremlin_query(query_info['query'])
            ...
            for query_info in sparql_queries:
                ...
                print(f"  Status: Mock execution (SPARQL endpoint needed for real queries)")
    ...
    async def _demo_validation_and_stats(self):
        ...
        try:
            ...
            print(f"  • Data quality score: {validation_results['data_quality_score']:.1%}")
            
            # Quality recommendations
            print(f"\n💡 Quality Recommendations:")
            if validation_results['orphaned_entities'] > 0:
                print(f"  • Review {validation_results['orphaned_entities']} orphaned entities")
            if validation_results['duplicate_entities'] > 0:
                print(f"  • Merge {validation_results['duplicate_entities']} duplicate entities")
        ...
        class MockEntityExtractor:
            async def extract_entities_from_article(self, article_id, title, content):
                ...
                if 'Microsoft' in content:
                    entities.append(self._create_mock_entity('Microsoft Corporation', 'ORGANIZATION', 0.95))
                    entities.append(self._create_mock_entity('Satya Nadella', 'PERSON', 0.92))
                if 'OpenAI' in content:
                    entities.append(self._create_mock_entity('OpenAI', 'ORGANIZATION', 0.94))
                    entities.append(self._create_mock_entity('Sam Altman', 'PERSON', 0.90))
                if 'Google' in content:
                    entities.append(self._create_mock_entity('Google LLC', 'ORGANIZATION', 0.96))
                    entities.append(self._create_mock_entity('Sundar Pichai', 'PERSON', 0.91))
                if 'Tesla' in content:
                    entities.append(self._create_mock_entity('Tesla Inc.', 'ORGANIZATION', 0.95))
                    entities.append(self._create_mock_entity('Elon Musk', 'PERSON', 0.93))
                
                # Add technology entities
                if 'artificial intelligence' in content.lower():
                    entities.append(self._create_mock_entity('Artificial Intelligence', 'TECHNOLOGY', 0.89))
                if 'quantum' in content.lower():
                    entities.append(self._create_mock_entity('Quantum Computing', 'TECHNOLOGY', 0.87))
                if 'battery' in content.lower():
                    entities.append(self._create_mock_entity('Lithium-ion Battery', 'TECHNOLOGY', 0.85))
                
                # Add policy entities
                if 'AI Act' in content:
                    entities.append(self._create_mock_entity('EU AI Act', 'POLICY', 0.88))
                if 'GDPR' in content:
                ...
                for i, source_entity in enumerate(entities):
                    for target_entity in entities[i+1:]:
                        if self._should_relate(source_entity, target_entity, content):
                            rel_type = self._determine_relationship_type(source_entity, target_entity)
                            relationships.append(self._create_mock_relationship(
    ...
    def _create_mock_entity(self, text, label, confidence):
        ...
        class MockEntity:
            def __init__(self, text, label, confidence):
                ...
                if label == 'PERSON':
                    if 'CEO' in text or any(ceo in text for ceo in ['Nadella', 'Altman', 'Pichai', 'Musk']):
    ...
    def _create_mock_relationship(self, source_entity, target_entity, rel_type, content):
    ...
    def _create_mock_graph_populator(self):
        ...
        class MockGraphPopulator:
            async def populate_from_article(self, article_id, title, content, published_date, metadata):
    ...
    async def _mock_execute_gremlin_query(self, query):
        ...
        if 'Microsoft' in query:
            ...
            return ['GPT-4', 'Azure OpenAI', 'Artificial Intelligence', 'Machine Learning']
        else:
