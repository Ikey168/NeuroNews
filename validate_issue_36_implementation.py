...
class Issue36ValidationSuite:
    ...
    async def _validate_component_availability(self):
        ...
        for component_name, file_path in components.items():
            try:
                if os.path.exists(file_path):
                    ...
                    self.validation_results['component_availability'][component_name] = True
                else:
                    print(f"❌ {component_name}: File not found")
                    self.validation_results['component_availability'][component_name] = False
            except Exception as e:
        ...
        for test_name, test_func in import_tests.items():
            try:
                ...
                print(f"{'✅' if success else '❌'} {test_name}: {'Available' if success else 'Import failed'}")
                self.validation_results['component_availability'][f"{test_name}_import"] = success
            except Exception as e:
                print(f"❌ {test_name}: Import error - {e}")
                self.validation_results['component_availability'][f"{test_name}_import"] = False
    ...
    async def _validate_issue_35_compatibility(self):
        ...
        for test_name, test_func in compatibility_tests.items():
            try:
                success = await test_func()
                print(f"{'✅' if success else '❌'} {test_name}: {'Compatible' if success else 'Incompatible'}")
    ...
    async def _test_performance_pipeline_reuse(self):
        ...
        try:
            ...
            optimized_pipeline_available = os.path.exists('src/nlp/optimized_nlp_pipeline.py')
            enhanced_extractor_available = os.path.exists('src/knowledge_graph/enhanced_entity_extractor.py')
    ...
    async def _validate_performance(self):
        ...
        for test_name, test_func in performance_tests.items():
            try:
                success = await test_func()
                print(f"{'✅' if success else '❌'} {test_name}: {'Acceptable' if success else 'Needs improvement'}")
    ...
    async def _test_graph_population_performance(self):
        ...
        try:
            ...
            return (entities_per_second >= min_entities_per_second and
                    relationships_per_second >= min_relationships_per_second)
        except Exception:
            return False
    ...
    async def _validate_requirements_compliance(self):
        ...
        for req_id, req_info in requirements.items():
            try:
                ...
                print(f"{'✅' if compliance else '❌'} {req_info['description']}: {status}")
    ...
    async def _test_named_entity_extraction_requirement(self):
        ...
        try:
            ...
            enhanced_extractor_exists = os.path.exists('src/knowledge_graph/enhanced_entity_extractor.py')
    ...
    async def _test_neptune_storage_requirement(self):
        ...
        try:
            ...
            graph_builder_exists = os.path.exists('src/knowledge_graph/graph_builder.py')
            enhanced_populator_exists = os.path.exists('src/knowledge_graph/enhanced_graph_populator.py')
    ...
    async def _test_graph_builder_requirement(self):
        ...
        try:
            ...
            existing_graph_builder = os.path.exists('src/knowledge_graph/graph_builder.py')
