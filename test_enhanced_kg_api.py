...
class TestEnhancedKnowledgeGraphAPI:
    ...
    def mock_populator(self):
        ...
        populator.query_entity_relationships = AsyncMock(return_value={
            'query_entity': 'Google',
            'related_entities': [
                {
                    'id': 'entity_001',
        ...
        mock_graph_builder._execute_traversal = AsyncMock(return_value=[
            {
                ...
                'content': ['Google has made significant advances in artificial intelligence...'],
        ...
        populator.execute_sparql_query = AsyncMock(return_value={
            'results': [
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_API_AVAILABLE, reason="Enhanced API components not available")
    ...
    async def test_get_related_entities_validation_error(self, client, mock_populator):
        ...
        try:
            ...
            assert "Entity name must be at least 2 characters long" in response.json()["detail"]
    ...
    async def test_get_entity_details_success(self, client, mock_populator):
        ...
        mock_populator.graph_builder._execute_traversal = AsyncMock(side_effect=[
            # Entity basic info
            [{
                'id': ['google_inc_001'],
            ...
            [{
                'type': 'COMPETES_WITH',
                'target': {'normalized_form': ['Microsoft Corporation'], 'entity_type': ['ORGANIZATION']},
                'properties': {'confidence': [0.9]}
            }],
            # Incoming relationships
            [{
                'type': 'CEO_OF',
                'source': {'normalized_form': ['Sundar Pichai'], 'entity_type': ['PERSON']},
    ...
    async def test_get_entity_details_not_found(self, client, mock_populator):
        ...
        try:
            response = client.get("/api/v1/knowledge-graph/entity_details/nonexistent_entity")
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_API_AVAILABLE, reason="Enhanced API components not available")
    async def test_advanced_graph_search_invalid_query_type(self, client, mock_populator):
    ...
    async def test_graph_analytics_success(self, client, mock_populator):
        ...
        mock_populator.graph_builder._execute_traversal = AsyncMock(side_effect=[
            [100],  # vertex count
            [250],  # edge count
            [{'ORGANIZATION': 45, 'PERSON': 30, 'TECHNOLOGY': 25}]  # type distribution
        ])
...
class TestAPIIntegration:
    ...
    async def test_end_to_end_workflow(self, mock_full_system):
        ...
        populator.graph_builder._execute_traversal.side_effect = [
            [{'id': ['entity_001'], 'normalized_form': ['Test Entity']}],  # Entity details
        ...
        # Step 3: SPARQL query
        sparql_result = await populator.execute_sparql_query("SELECT * WHERE { ?s ?p ?o }")
