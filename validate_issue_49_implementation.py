...
def validate_imports() -> Dict[str, bool]:
    ...
    try:
        ...
        print_test_result("QuickSight service imports", True, "All core classes imported successfully")
    ...
    try:
        ...
        print_test_result("QuickSight routes imported", True, "API router imported successfully")
    ...
    try:
        ...
        if quicksight_routes:
            print_test_result("FastAPI integration", True, f"Found {len(quicksight_routes)} dashboard routes")
            results['fastapi_integration'] = True
        else:
            print_test_result("FastAPI integration", False, "No dashboard routes found in FastAPI app")
...
def validate_service_functionality() -> Dict[str, bool]:
    ...
    try:
        ...
        try:
            ...
            print_test_result("QuickSight configuration", True, f"Config created for account {config.aws_account_id}")
        ...
        try:
            ...
            with unittest.mock.patch('boto3.client') as mock_boto3:
                ...
                print_test_result("Service initialization", True, "QuickSight service initialized with mocked AWS client")
        ...
        try:
            with unittest.mock.patch('boto3.client') as mock_boto3:
                ...
                print_test_result("Sentiment trends SQL", True, "SQL query generated with required fields")
                ...
                print_test_result("Entity relationships SQL", True, "SQL query generated with entity fields")
                ...
                print_test_result("Event timeline SQL", True, "SQL query generated with timeline fields")
        ...
        try:
            ...
            required_types = ['sentiment_trends', 'entity_relationships', 'event_timeline', 'comprehensive']
            
            for req_type in required_types:
                if req_type not in dashboard_types:
                    raise ValueError(f"Missing required dashboard type: {req_type}")
            
            print_test_result("Dashboard types", True, f"All required types available: {required_types}")
        ...
        try:
            with unittest.mock.patch('boto3.client') as mock_boto3:
                ...
                print_test_result("Dataset columns", True, f"Column definitions available for {len(datasets)} datasets")
...
def validate_api_models() -> Dict[str, bool]:
    ...
    try:
        from src.api.routes.quicksight_routes import (
        ...
        try:
            ...
            print_test_result("Setup request model", True, f"Account ID: {setup_request.aws_account_id}")
        ...
        try:
            layout_request = DashboardLayoutRequest(
                layout_type="sentiment_trends",
                refresh_schedule="hourly"
            )
            assert layout_request.layout_type == "sentiment_trends"
            print_test_result("Layout request model", True, f"Layout type: {layout_request.layout_type}")
        ...
        try:
            ...
            print_test_result("Response models", True, f"Setup response with {len(setup_response.datasets_created)} datasets")
...
def validate_api_endpoints() -> Dict[str, bool]:
    ...
    try:
        ...
        try:
            ...
            print_test_result("Health check endpoint", True, f"Status: {data['status']}")
        ...
        try:
            ...
            for expected in expected_routes:
                ...
                if any(expected.replace('{layout_type}', '') in route for route in dashboard_routes):
                    route_coverage += 1
            
            success = route_coverage >= len(expected_routes) - 1  # Allow for some flexibility
            print_test_result("Endpoint routing", success, f"Found {len(dashboard_routes)} dashboard routes")
...
def validate_issue_49_requirements() -> Dict[str, bool]:
    ...
    try:
        from src.dashboards.quicksight_service import QuickSightDashboardService, DashboardType
        ...
        try:
            ...
            with unittest.mock.patch('boto3.client') as mock_boto3:
                ...
                print_test_result("Requirement 1: QuickSight setup", True, "Method implemented")
        ...
        try:
            with unittest.mock.patch('boto3.client') as mock_boto3:
                ...
                print_test_result("Requirement 2: Dashboard layouts", True, f"All 3 required layouts supported")
        ...
        try:
            with unittest.mock.patch('boto3.client') as mock_boto3:
                ...
                required_filters = ['date_filter', 'sentiment_filter', 'category_filter']
                ...
                print_test_result("Requirement 3: Filtering", success, f"Found filters: {filters_found}")
        ...
        try:
            with unittest.mock.patch('boto3.client') as mock_boto3:
                ...
                print_test_result("Requirement 4: Real-time updates", success, "Redshift integration verified")
        ...
        print_test_result("Issue #49 requirements validation", False, f"Import error: {e}")
...
def validate_redshift_integration() -> Dict[str, bool]:
    ...
    try:
        ...
        try:
            ...
            print_test_result("Redshift loader available", True, "RedshiftETLProcessor imported successfully")
        ...
        try:
            ...
            with unittest.mock.patch('boto3.client') as mock_boto3:
                ...
                print_test_result("Redshift table references", success, "All SQL queries reference news_articles table")
        ...
        try:
            ...
            with unittest.mock.patch('boto3.client') as mock_boto3:
                ...
                print_test_result("Required columns", success, f"Found {len(columns_found)}/{len(required_columns)} columns")
...
async def main():
    ...
    test_categories = {
        "Import Tests": ["quicksight_service", "quicksight_routes", "fastapi_integration"],
        "Service Functionality": ["configuration", "service_init", "sql_generation", "dashboard_types", "dataset_columns"],
        "API Models": ["setup_request", "layout_request", "response_models"],
        "API Endpoints": ["health_check", "endpoint_routing"],
        "Issue #49 Requirements": ["req1_quicksight_setup", "req2_dashboard_layouts", "req3_filtering", "req4_realtime_updates"],
        "Redshift Integration": ["redshift_loader", "table_references", "required_columns"]
    }
    
    for category, test_keys in test_categories.items():
        category_results = [all_results.get(key, False) for key in test_keys if key in all_results]
        ...
        if category_total > 0:
            ...
            if category_passed < category_total:
                failed_tests = [key for key in test_keys if not all_results.get(key, False)]
    ...
    if success_rate >= 80:
        ...
        print("  • Dashboard layouts for sentiment trends, entity relationships, and event timelines")
