...
class TestOptimizedNLPPipeline:
        ...
        return [
            {
                ...
                'content': f'This is a test article about technology and innovation. ' * 50
            }
            for i in range(20)
        ]
    ...
    @pytest.mark.skipif(not OPTIMIZED_PIPELINE_AVAILABLE, reason="Optimized pipeline not available")
...
class TestNLPIntegration:
    ...
    @pytest.mark.skipif(not INTEGRATION_AVAILABLE, reason="NLP integration not available")
    @pytest.mark.asyncio
    ...
    async def test_optimized_event_clusterer(self, test_config, sample_articles):
        ...
        try:
            ...
            results = await clusterer.cluster_articles_async(test_articles, mock_embeddings)
    ...
    async def test_integrated_nlp_processor(self, test_config, sample_articles):
        ...
        try:
            ...
            results = await processor.process_articles_comprehensive(
                test_articles,
                operations=['keywords']  # Use keywords as it's simplest
            )
...
class TestPerformanceOptimizations:
    ...
    def test_memory_management_thresholds(self):
        ...
        config = NLPConfig(max_memory_usage_mb=64.0, gc_threshold=0.5)  # Very low for testing
    ...
    async def test_throughput_benchmark(self):
        ...
        try:
            ...
            articles = [
                {
                    'id': f'benchmark_{i}',
                    'content': f'Benchmark article content for performance testing. ' * 20
                }
                for i in range(50)
            ]
            ...
            logger.info(f"Benchmark: {len(articles)} articles in {processing_time:.2f}s")
