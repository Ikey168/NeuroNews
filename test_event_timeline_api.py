...
class TestEventTimelineService:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not EVENT_TIMELINE_AVAILABLE, reason="Event timeline components not available")
    async def test_track_historical_events(self, service, sample_events):
        ...
        assert events[0].event_type in ["announcement", "business_transaction", "regulation", "general"]
    ...
    async def test_generate_timeline_api_response(self, service, sample_events):
        ...
        with patch.object(service, 'track_historical_events', return_value=sample_events):
...
class TestEventTimelineAPI:
    ...
    def mock_service(self):
        ...
        service.store_event_relationships = AsyncMock(return_value={
            'events_stored': 1,
            'relationships_created': 2,
            'errors': []
        })
        service.generate_visualization_data = AsyncMock(return_value={
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not EVENT_TIMELINE_AVAILABLE, reason="Event timeline components not available")
    async def test_get_enhanced_event_timeline_date_validation(self, client, mock_service):
