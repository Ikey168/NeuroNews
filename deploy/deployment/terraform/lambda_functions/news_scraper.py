...
async def _run_async_scraper(
    scraper_runner, selected_sources, config: Dict[str, Any]
) -> Dict[str, Any]:
    """
...
def _send_cloudwatch_metrics(
    total_articles: int, sources_count: int, config: Dict[str, Any]
) -> None:
    """
...
def _store_articles_in_s3(articles: List[Dict[str, Any]], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        storage_results = asyncio.run(
            ingest_scraped_articles_to_s3(articles=articles, s3_config=s3_config)
        )
...
if __name__ == "__main__":
    ...
    test_event = {
        ...
        "scraper_config": {"concurrent_requests": 2},
    }
