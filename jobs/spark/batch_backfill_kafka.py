...
def read_kafka_batch_by_offsets(spark, topic, bootstrap_servers, start_offset, end_offset):
    ...
    return spark.read \
        ...
        .option("startingOffsets", '{{"{}":{{0:{}}}}}'.format(topic, start_offset)) \
        .option("endingOffsets", '{{"{}":{{0:{}}}}}'.format(topic, end_offset)) \
        .load()
...
def read_kafka_batch_by_timestamp(spark, topic, bootstrap_servers, start_timestamp, end_timestamp):
    ...
    return spark.read \
        ...
        .option("kafka.bootstrap.servers", bootstrap_servers) \
        .option("subscribe", topic) \
        .option("startingOffsets", '{{"{}":{{0:{}}}}}'.format(topic, start_ts)) \
        .option("endingOffsets", '{{"{}":{{0:{}}}}}'.format(topic, end_ts)) \
        .load()
