...
class NeuroNewsS3Integration:
    ...
    async def initialize(self):
        ...
        try:
            self.scraper = AsyncNewsScraperEngine(
                max_concurrent=5,
                enable_monitoring=True  # Use monitoring if available
            )
    ...
    async def process_and_store_articles(self, raw_article_keys: List[str]) -> Dict[str, Any]:
        """
        ...
        for key in raw_article_keys:
            try:
                ...
                processed_count += 1
                print(f"   ✅ Processed: {processed_article.get('title', 'Unknown')[:50]}...")
    ...
    async def get_storage_overview(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_raw = await self.s3_storage.list_articles_by_date(today, ArticleType.RAW, limit=10)
            recent_processed = await self.s3_storage.list_articles_by_date(today, ArticleType.PROCESSED, limit=10)
    ...
    try:
        ...
        if result.get('storage', {}).get('stored_keys'):
            ...
            if stored_keys:
                ...
                processing_result = await integration.process_and_store_articles(stored_keys[:2])
