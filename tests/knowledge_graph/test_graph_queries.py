...
def test_relationship_path_queries(graph_queries):
    ...
    graph_queries.g.V().hasLabel().as_().out().as_().in_().where().project().by().by().by().by().toList.return_value = mock_result
...
def test_complex_traversals(graph_queries):
    ...
    graph_queries.g.V().hasLabel().as_().where().project().by().by().by().toList.return_value = mock_result
