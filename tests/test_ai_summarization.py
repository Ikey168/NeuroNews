...
class TestIntegration:
    ...
    async def test_end_to_end_workflow(self, sample_articles):
        ...
        with patch('src.nlp.ai_summarizer.AutoTokenizer') as mock_tokenizer_class, \
             patch('src.nlp.ai_summarizer.AutoModelForSeq2SeqLM') as mock_model_class:
...
class TestPerformance:
    ...
    async def test_concurrent_summarization(self):
        ...
        with patch('src.nlp.ai_summarizer.AutoTokenizer'), \
             patch('src.nlp.ai_summarizer.AutoModelForSeq2SeqLM'):
            
            summarizer = AIArticleSummarizer(device='cpu')
            
            # Create multiple concurrent tasks
            texts = [
                "Test article content for summarization " * 20
                for _ in range(5)
            ]
