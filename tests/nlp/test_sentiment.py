...
def mock_hf_analyzer(mocker):
    ...
    ), "Patching 'src.nlp.sentiment_analysis.pipeline' did not result in the mock being used."
    return analyzer
...
def test_batch_processing_default_analyzer():
    ...
    texts_for_live_test = [
        ...
        if item["expected_label"] != "ERROR" and item["text"] and item["text"].strip()
    ]
...
@pytest.mark.parametrize(
    "text, expected_label, _score_not_used", EDGE_CASE_TEXTS_FOR_DEFAULT_ANALYZER
)
...
def test_hf_analyzer_label_mapping(mock_hf_analyzer: SentimentAnalyzer, test_case):
    ...
    if not text_to_analyze or not text_to_analyze.strip():
        ...
        mock_hf_analyzer.pipeline.return_value = [
            {"label": expected_label_from_fixture, "score": 0.99}
        ]
        result = mock_hf_analyzer.analyze(text_to_analyze)
