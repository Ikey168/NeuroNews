...
class TestAsyncScraperEngine:
    ...
    async def test_article_parsing(self, scraper_engine):
        ...
        article = scraper_engine.parse_article_html(html_content, "https://example.com/test", source)
...
class TestPerformanceMonitor:
    ...
    def test_get_stats(self, performance_monitor):
        ...
        for success, time in [(True, 1.0), (True, 1.5), (False, 0.0), (True, 2.0), (True, 1.2)]:
...
class TestAsyncPipelines:
    ...
    async def test_article_enhancement(self, pipeline_processor):
        ...
        enhanced_fields = ['category', 'keywords', 'sentiment', 'word_count', 'reading_time', 'processed_date', 'entities']
        has_enhancement = any(field in enhanced for field in enhanced_fields)
        assert has_enhancement, f"No enhancement fields found. Available fields: {list(enhanced.keys())}"
