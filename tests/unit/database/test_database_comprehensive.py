...
This module exercises the *current* database API in ``src.database.setup`` and
``src.nlp.summary_database`` with fully mocked drivers, so it runs anywhere
(CI, laptop, container) without a live PostgreSQL server.
...
``DatabaseSetupConfig`` classes (which never existed). Both imports failed, so
``DATABASE_AVAILABLE`` was always ``False`` and *zero* tests ran.

The real API is function-based:
    - ``get_db_config(testing=...)``
    - ``get_sync_connection(testing=...)``   -> ``psycopg2.connect(**config)``
    - ``get_async_connection(testing=...)``  -> ``await asyncpg.connect(**config)``
    - ``create_test_articles(count)``
plus the ``SummaryDatabase`` / ``SummaryRecord`` helpers.

``asyncpg`` is not installed in this environment, so we install a lightweight
module stub in ``sys.modules`` *before* importing ``src.database.setup`` (which
does ``import asyncpg`` at module import time). ``psycopg2`` and ``asyncpg`` are
then patched at the source module for the connection tests.
"""
...
# and asyncpg is not installed here. Provide a minimal module with an async
# `connect` so the import (and later patching) works.
# ---------------------------------------------------------------------------
if "asyncpg" not in sys.modules:
    _asyncpg_stub = types.ModuleType("asyncpg")
    _asyncpg_stub.connect = AsyncMock()  # replaced per-test where needed
...
class TestGetDbConfig:
    def test_testing_config_uses_test_defaults(self, monkeypatch):
        # Clear env so we observe the built-in defaults.
    ...
    def test_testing_env_var_forces_test_config(self, monkeypatch):
        ...
        # testing arg is False but TESTING env should still pick the test branch.
        config = db_setup.get_db_config(testing=False)
        assert config["database"] == "neuronews_test"
...
class TestCreateTestArticles:
    def test_inserts_requested_number_and_returns_ids(self):
        ...
        # Each fetchone returns a RealDictCursor-style row with an "id" key.
        cursor.fetchone.side_effect = [{"id": i} for i in range(1, 6)]
        # `with conn.cursor(...) as cur` -> context manager returning cursor.
...
class TestSummaryDatabase:
    def test_get_connection_uses_connection_params(self):
        ...
        # SummaryDatabase._get_connection calls psycopg2.connect directly.
