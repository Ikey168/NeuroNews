...
class TestNERProcessor(unittest.TestCase):
    ...
    def test_extract_entities_success(self):
        ...
        self.assertEqual(
            apple_entity["type"], "TECHNOLOGY_ORGANIZATION"
        )  # Enhanced type
    ...
    def test_duplicate_entity_filtering(self):
        ...
        entities = self.processor.extract_entities(
            "Apple and Apple again", "test_duplicates"
        )
    ...
    def test_create_ner_processor_factory(self):
        ...
        processor = create_ner_processor(
            model_name="custom-model", confidence_threshold=0.8
        )
...
class TestNERArticleProcessor(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures."""
        # Mock dependencies
        self.mock_sentiment_analyzer = patch(
            "src.nlp.sentiment_analysis.create_analyzer"
        )
        self.mock_sentiment_analyzer.start()

        self.mock_ner_processor = patch(
            "src.nlp.ner_article_processor.create_ner_processor"
        )
        ...
        self.mock_db_article.connect.return_value.__enter__.return_value = (
            self.mock_conn
        )
    ...
    def test_process_articles_with_ner(self, mock_parent_process):
        ...
        mock_sentiment_results = [
            {"article_id": "test_1", "sentiment": "positive", "confidence": 0.85}
        ]
    ...
    def test_search_entities(self):
        ...
        results = processor.search_entities(
            entity_text="Apple", entity_type="ORGANIZATION", min_confidence=0.8
        )
...
class TestNERIntegration(unittest.TestCase):
    ...
    def test_end_to_end_processing(
        ...
        # Mock parent class processing
        with patch.object(
            processor.__class__.__bases__[0], "process_articles"
        ) as mock_parent:
            mock_parent.return_value = [
                {"article_id": "ai_news_1", "sentiment": "positive", "confidence": 0.8}
            ]
