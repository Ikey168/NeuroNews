...
class TestGraphVisualizer:
    ...
    @pytest.mark.asyncio
    async def test_advanced_filtering(self, visualizer):
        """Test advanced filtering with range and list filters."""
        nodes = [
            {"id": "node1", "properties": {"score": 80, "type": "person", "tags": ["a", "b"]}},
            {"id": "node2", "properties": {"score": 60, "type": "organization", "tags": ["b", "c"]}},
            {"id": "node3", "properties": {"score": 90, "type": "person", "tags": ["c", "d"]}}
        ]
        
        visualization = await visualizer.create_visualization(nodes, [])
        
        # Test range filter
        range_filters = {"score": {"$gt": 70}}
        filtered_viz = await visualizer.apply_filters(visualization, range_filters)
        assert isinstance(filtered_viz, GraphVisualization)
        
        # Test list filter
        list_filters = {"type": ["person", "organization"]}
        filtered_viz2 = await visualizer.apply_filters(visualization, list_filters)
        assert isinstance(filtered_viz2, GraphVisualization)
        
        # Test exact match filter
        exact_filters = {"score": {"$eq": 80}}
        filtered_viz3 = await visualizer.apply_filters(visualization, exact_filters)
        assert isinstance(filtered_viz3, GraphVisualization)
        
        # Test less than filter
        lt_filters = {"score": {"$lt": 70}}
        filtered_viz4 = await visualizer.apply_filters(visualization, lt_filters)
        assert isinstance(filtered_viz4, GraphVisualization)
    
    @pytest.mark.asyncio
    async def test_hierarchical_layout_disconnected(self, visualizer):
        """Test hierarchical layout with disconnected components."""
        # Create disconnected components to trigger remaining nodes handling
        nodes = [
            {"id": "node1"}, {"id": "node2"}, {"id": "node3"},
            {"id": "node4"}, {"id": "node5"}
        ]
        # Create disconnected edges (two separate components)
        edges = [
            {"source": "node1", "target": "node2"},
            {"source": "node4", "target": "node5"}
        ]
        
        config = LayoutConfig(algorithm=LayoutAlgorithm.HIERARCHICAL)
        visualization = await visualizer.create_visualization(nodes, edges, config)
        
        assert isinstance(visualization, GraphVisualization)
        # All nodes should have positions despite being disconnected
    ...
    @pytest.mark.asyncio
    async def test_layout_algorithms_empty_nodes(self, visualizer):
        """Test layout algorithms with empty node lists."""
        for algorithm in LayoutAlgorithm:
            config = LayoutConfig(algorithm=algorithm)
            # Test with empty nodes to trigger early returns in layout methods
            visualization = await visualizer.create_visualization([], [])
    ...
    async def test_custom_node_styles(self, visualizer):
        ...
        # Check that node was created with custom style
        assert visualization.nodes[0].id == "styled_node"
    
    @pytest.mark.asyncio
    async def test_custom_edge_styles(self, visualizer, sample_nodes):
        ...
        visualization = await visualizer.create_visualization(sample_nodes, edges)
        
        assert isinstance(visualization, GraphVisualization)
        assert len(visualization.edges) == 1
        
        # Check that edge was created with custom style
        assert visualization.edges[0].id == "styled_edge"
    
    @pytest.mark.asyncio
    async def test_subgraph_extraction_no_expansion(self, visualizer):
        """Test subgraph extraction where no expansion occurs."""
        ...
        isolated_edges = []  # No edges, so no expansion possible
        
        visualization = await visualizer.create_visualization(isolated_nodes, isolated_edges)
        
        # Extract subgraph - should terminate early due to no expansion
        subgraph = await visualizer.extract_subgraph(visualization, "isolated1", max_distance=3)
        
        assert isinstance(subgraph, GraphVisualization)
        # Should only contain the center node since there are no connections
        assert len(subgraph.nodes) == 1
        assert subgraph.nodes[0].id == "isolated1"
    ...
    async def test_visualization_metadata(self, visualizer, sample_nodes, sample_edges):
        ...
        assert visualization.metadata["edge_count"] == len(sample_edges)
