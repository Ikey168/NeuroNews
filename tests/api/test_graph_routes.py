...
from src.api.routes.graph_routes import router as graph_api_router, lifespan as graph_api_lifespan
...
def mock_graph_builder_methods(app_for_test: FastAPI):
    ...
    methods_to_mock_on_traversal = [
        "hasLabel", "has", "limit", "bothE", "otherV", "simplePath", 
        "both", "dedup", "valueMap", "project", "by", "toList", 
        "addE", "to", "property", "values", "is", "filter", "V", "E", "addV", "id" 
    ]
    ...
    app_for_test.dependency_overrides[original_get_graph_dependency] = override_get_graph_for_this_test
...
def test_get_related_entities(app_for_test: FastAPI, mock_graph_builder_methods: AsyncMock):
    ...
    with TestClient(app_for_test) as client:
        response = client.get("/graph/related_entities?entity=TestCo&entity_type=Organization&max_depth=1")
...
def test_get_event_timeline(app_for_test: FastAPI, mock_graph_builder_methods: AsyncMock):
    mock_graph_builder_methods._execute_traversal.return_value = [
        {'event_name': 'AI Conf', 'date': datetime.now(timezone.utc).isoformat(), 'location': 'Online'}
    ]
    with TestClient(app_for_test) as client:
...
def test_event_timeline_date_filtering(app_for_test: FastAPI, mock_graph_builder_methods: AsyncMock):
    mock_graph_builder_methods._execute_traversal.return_value = [] 
    
    start_date_str = "2024-01-01T00:00:00Z"
    end_date_str = "2024-01-31T23:59:59Z"
    with TestClient(app_for_test) as client:
        response = client.get(f"/graph/event_timeline?topic=AI&start_date={start_date_str}&end_date={end_date_str}")
...
@pytest.mark.parametrize("neptune_connect_fails, expected_status, expected_detail_part", [
...
def test_error_handling(
    ...
    if neptune_connect_fails:
        mock_builder_instance_for_lifespan.connect = AsyncMock(side_effect=ConnectionError("Lifespan mock connection failed"))
    else: 
        ...
        mock_builder_instance_for_lifespan._execute_traversal = AsyncMock(return_value=[{"id":"healthy_vertex"}])
        mock_builder_instance_for_lifespan.client = AsyncMock(spec=Client) 
        mock_builder_instance_for_lifespan.client.submit_async = mock_builder_instance_for_lifespan._execute_traversal
    ...
    with patch('src.api.routes.graph_routes.GraphBuilder', return_value=mock_builder_instance_for_lifespan) as MockedGBClassInLifespan:
...
def test_health_check(app_for_test: FastAPI, mock_graph_builder_methods: AsyncMock):
    ...
    mock_graph_builder_methods._execute_traversal.side_effect = ConnectionError("Simulated connection error from get_graph mock")
    
    with TestClient(app_for_test) as client:
        response = client.get("/graph/health")
    assert response.status_code == 503, response.json()
    assert "Graph database connection error: Simulated connection error from get_graph mock" in response.json()["detail"]
