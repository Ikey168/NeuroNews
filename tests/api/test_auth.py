...
def test_db(monkeypatch):
    ...
    async def mock_execute_query(self, query, params=None):
        if "SELECT" in query and params:
            email = params[0]
            return [(users[email]["id"], users[email]["password_hash"], users[email]["role"])] if email in users else []
    ...
    monkeypatch.setattr("src.database.redshift_loader.RedshiftLoader.execute_query", mock_execute_query)
    return users
