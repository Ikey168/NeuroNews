...
class TestSentimentTrendAnalyzer:
    ...
    def test_calculate_trend_summary_empty_points(self, trend_analyzer):
        ...
        time_range = (datetime.now(timezone.utc) - timedelta(days=5), datetime.now(timezone.utc))
    ...
    async def test_detect_sentiment_alerts_significant_shift(self, trend_analyzer):
        ...
        assert shift_alert.change_magnitude >= trend_analyzer.config['alert_thresholds']['significant_shift']
        ...
        with patch.object(trend_analyzer, '_fetch_sentiment_data') as mock_fetch:
            with patch.object(trend_analyzer, '_store_alerts') as mock_store:
                ...
                for alert in alerts:
                    ...
                    assert alert.alert_type in ['significant_shift', 'trend_reversal', 'volatility_spike']
    
    @pytest.mark.asyncio
    async def test_get_active_alerts(self, trend_analyzer):
        """Test retrieving active alerts from database."""
        sample_alerts_data = [
            ('alert_1', 'technology', 'significant_shift', 'high', 0.8, 0.2, 0.6, 300.0, 
             0.9, '7d', datetime.now(timezone.utc), 'Test alert', ['article_1'], {}),
            ('alert_2', 'politics', 'trend_reversal', 'medium', -0.5, 0.3, 0.8, 266.7,
             0.85, '7d', datetime.now(timezone.utc), 'Test alert 2', ['article_2'], {})
        ]
