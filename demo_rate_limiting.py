...
class RateLimitingDemo:
        ...
        for i in range(15):  # Try 15 requests
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"   Reset in: {error_data.get('reset_in_seconds', 'unknown')} seconds")
                    break
                else:
    ...
    def demo_user_tiers(self):
        ...
        for tier, description in tiers:
            ...
            try:
                ...
                if response.status_code == 200:
                    tier_headers = {k: v for k, v in response.headers.items() 
                                  if k.startswith('X-RateLimit')}
    ...
    def demo_suspicious_activity_simulation(self):
        ...
        test_endpoints = [
            "/api/test", "/news/articles", "/graph/entities", 
            "/admin/users", "/api/secret", "/config/settings"
        ]
    ...
    def demo_health_check(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   Store backend: {health_data.get('store_backend', 'unknown')}")
...
def create_test_server():
    ...
    try:
        ...
        async def test_endpoint():
            return {"message": "Rate limiting test successful", "timestamp": datetime.now().isoformat()}
