...
class PerformanceValidator:
    ...
    def _generate_test_data(self, size: int = 1000) -> List[Dict[str, Any]]:
        ...
        for i in range(size):
            ...
            article = {
                ...
                'word_count': len(template['content_base'].split()) * content_multiplier
            }
    ...
    async def run_baseline_benchmark(self, articles: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def run_optimized_benchmark(
        ...
        try:
            ...
            return {
                ...
                'average_time_per_article': processing_time / len(results['processed_articles']),
    ...
    async def run_performance_comparison(self, dataset_sizes: List[int] = None) -> Dict[str, Any]:
    ...
    def _print_size_comparison(self, size: int, results: Dict[str, Any]):
        ...
        print(f"{'Configuration':<20} {'Time (s)':<10} {'Throughput (art/s)':<18} {'Speedup':<10}")
        print("-" * 70)
        ...
        for config_name, result in results.items():
            ...
            print(f"{config_name:<20} {time_taken:<10.3f} {throughput:<18.1f} {speedup:<10.2f}x")
        ...
        for count in article_counts:
            ...
            try:
                ...
                stress_results[count] = {
                    ...
                    'throughput': result['throughput'],
                    'memory_used': result['metrics']['performance'].get('peak_memory_usage', 0),
                    'success': True
                }
        ...
        for memory_limit, config_name in memory_configs:
            ...
            try:
                ...
                memory_results[memory_limit] = {
                    ...
                    'peak_memory': result['metrics']['performance'].get('peak_memory_usage', 0),
                    'success': True
                }
    ...
    def generate_performance_report(self, results: Dict[str, Any]) -> str:
        ...
        if 'stress_test' in results:
            ...
            for count, result in results['stress_test'].items():
                if result.get('success', False):
                    ...
                    report_lines.append(f"{count:>5} articles: FAILED - {result.get('error', 'Unknown error')}")
        ...
        if 'performance_comparison' in results:
            best_configs = self._analyze_best_configurations(results['performance_comparison'])
            
            for size, config in best_configs.items():
                report_lines.append(f"For {size} articles: Use {config['name']} configuration")
    ...
    def _analyze_best_configurations(self, comparison_results: Dict[str, Any]) -> Dict[str, Any]:
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Validate optimized pipeline performance')
    ...
    parser.add_argument('--memory-test', action='store_true',
                       help='Run memory efficiency validation')
