...
class SentimentTrendDemo:
    ...
    def _load_config(self) -> Dict:
        ...
        try:
            config_path = Path(__file__).parent.parent / 'config' / 'sentiment_trend_analysis_settings.json'
    ...
    def _generate_sample_data(self) -> List[Dict]:
            ...
            for topic in topics:
                ...
                for article_num in range(num_articles):
                    ...
                    if topic == 'technology':
                        ...
                        if days_ago > 20:
                            base_sentiment = 0.2 - (days_ago - 20) * 0.05
                        else:
                            base_sentiment = -0.3 + (20 - days_ago) * 0.04
                        ...
                        base_sentiment = 0.2 - (30 - days_ago) * 0.02
                        sentiment_noise = (article_num % 4 - 2) * 0.25
                    
                    sentiment_score = max(-1.0, min(1.0, base_sentiment + sentiment_noise))
        ...
        logger.info(f"Generated {len(sample_data)} sample articles across {len(topics)} topics")
        return sample_data
    ...
    async def demo_topic_specific_analysis(self, topic: str) -> Optional[TopicTrendSummary]:
    ...
    def demo_data_visualization(self, topic_summaries: Dict[str, TopicTrendSummary]) -> None:
        ...
        try:
            ...
            fig.suptitle('Historical Sentiment Trend Analysis - Demo Results', fontsize=16)
            ...
            avg_sentiments = [summary.average_sentiment for summary in topic_summaries.values()]
            colors = ['green' if s > 0.1 else 'red' if s < -0.1 else 'orange' for s in avg_sentiments]
            ...
            ax3 = axes[1, 0]
            volatilities = [summary.sentiment_volatility for summary in topic_summaries.values()]
            ...
            ax4 = axes[1, 1]
            article_counts = [summary.statistical_summary.get('total_articles', 0) 
            ...
            # Save the plot
            output_path = Path(__file__).parent.parent / 'demo' / 'sentiment_trend_analysis_demo.png'
    ...
    def _display_topic_summary(self, summary: TopicTrendSummary) -> None:
        ...
        print(f"   Time Range: {summary.time_range[0].date()} to {summary.time_range[1].date()}")
        ...
        print(f"   Total Articles: {summary.statistical_summary.get('total_articles', 'N/A')}")
    ...
    def _display_detailed_topic_analysis(self, summary: TopicTrendSummary) -> None:
        ...
        print("-" * 40)
        recent_points = summary.data_points[-5:] if len(summary.data_points) >= 5 else summary.data_points
        for point in recent_points:
            date_str = point.date.strftime('%Y-%m-%d')
            sentiment_emoji = "😊" if point.sentiment_score > 0.1 else "😞" if point.sentiment_score < -0.1 else "😐"
            print(f"   {date_str}: {point.sentiment_score:.3f} {sentiment_emoji} ({point.article_count} articles)")
...
class MockSentimentTrendAnalyzer:
    ...
    async def analyze_historical_trends(self, topic: Optional[str], start_date: datetime, 
                                      end_date: datetime, time_granularity: str = 'daily') -> List[TopicTrendSummary]:
        ...
        for topic_name, topic_data in grouped_data.items():
            if len(topic_data) >= 3:  # Minimum data requirement
                ...
                aggregated = analyzer._aggregate_by_time_granularity(topic_data, time_granularity)
                ...
                summary = analyzer._calculate_trend_summary(topic_name, trend_points, time_range)
                summaries.append(summary)
        
        return summaries
    
    async def generate_sentiment_alerts(self, topic: Optional[str] = None, 
        ...
        recent_data = [
            item for item in self.sample_data
            if item['publish_date'] >= cutoff_date
        ]
        ...
        for topic_name, topic_data in grouped_data.items():
            if topic is None or topic_name == topic:
                alerts = await analyzer._detect_sentiment_alerts(topic_name, topic_data, lookback_days)
    ...
    async def get_topic_trend_summary(self, topic: str, days: int = 30) -> Optional[TopicTrendSummary]:
        ...
        summaries = await self.analyze_historical_trends(topic, start_date, end_date, 'daily')
        return summaries[0] if summaries else None
    
    async def get_active_alerts(self, topic: Optional[str] = None, limit: int = 10) -> List[TrendAlert]:
