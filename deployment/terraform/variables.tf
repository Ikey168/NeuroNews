...
variable "neptune_master_username" {
  description = "Master username for the Neptune cluster (if using password authentication)"
  type        = string
  default     = "neptune_admin"
}

variable "neptune_master_password" {
  description = "Master password for the Neptune cluster. DO NOT set a default value here. Should be provided via AWS Secrets Manager or environment variable TF_VAR_neptune_master_password."
  type        = string
  sensitive   = true
  # No default value - must be provided externally
}
...
variable "trusted_account_id" {
  description = "AWS account ID that is trusted for cross-account access (replace with actual account ID in production)"
  type        = string
  default     = "123456789012"
  
  validation {
    condition     = can(regex("^[0-9]{12}$", var.trusted_account_id))
    error_message = "The trusted_account_id must be a 12-digit AWS account ID."
  }
}
