...
class DynamoDBMetadataDemo:
    ...
    async def run_comprehensive_demo(self):
        ...
        try:
            ...
            print("✅ Store article metadata (title, source, published_date, tags) in DynamoDB")
    ...
    async def _demo_initialization(self):
        ...
        print(f"🔍 Full-text search: {'Enabled' if self.config.enable_full_text_search else 'Disabled'}")
    ...
    async def _demo_query_api(self):
        ...
        date_result = await self.manager.get_articles_by_date_range(start_date, end_date)
        query_time = (time.time() - start_time) * 1000
    ...
    async def _demo_system_integration(self):
        ...
        start_time = time.time()
        redshift_success = await integrate_with_redshift_etl(redshift_record, self.manager)
        integration_time = (time.time() - start_time) * 1000
        ...
        print(f"📊 Articles synced: {sync_result['indexed_count']}/{sync_result['total_articles']}")
    ...
    async def _demo_performance_monitoring(self):
        ...
        print(f"  • Indexes: {'Enabled' if stats['table_info']['indexes_enabled'] else 'Disabled'}")
    ...
    async def _demo_health_monitoring(self):
        ...
        start_time = time.time()
        update_success = await self.manager.update_article_metadata(test_article_id, updates)
        ...
        if updated_article:
            print(f"✅ Verified updated sentiment score: {updated_article.sentiment_score}")
...
class MockDynamoDBManager:
    ...
    async def get_articles_by_date_range(self, start_date: str, end_date: str, **kwargs):
        """Mock get articles by date range."""
        items = [item for item in self.mock_data.values() 
    ...
    async def update_article_metadata(self, article_id: str, updates: Dict[str, Any]) -> bool:
