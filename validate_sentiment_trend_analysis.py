...
def test_data_structures():
    ...
    from nlp.sentiment_trend_analyzer import SentimentTrendPoint, TrendAlert, TopicTrendSummary
    ...
    class MockSentimentTrendAnalyzer:
        ...
        def _create_trend_points(self, topic, aggregated_data):
            ...
            for date_key, articles in aggregated_data.items():
                if len(articles) >= self.config['analysis_settings']['min_articles_per_data_point']:
        ...
        def _calculate_trend_summary(self, topic, trend_points, time_range):
            ...
            if len(scores) >= 2:
                ...
                slope, intercept, r_value, p_value, std_err = stats.linregress(date_ordinals, scores)
            ...
            statistical_summary = {
                ...
                'avg_articles_per_point': total_articles / len(trend_points) if trend_points else 0
            }
...
def test_configuration_loading():
    ...
    config_path = Path(__file__).parent / 'config' / 'sentiment_trend_analysis_settings.json'
    ...
    if config_path.exists():
        ...
        if 'sentiment_trend_analysis' in config:
            ...
            required_sections = ['analysis_settings', 'alert_configuration', 'performance_settings']
            for section in required_sections:
                if section == 'alert_configuration':
                    assert section in main_config, f"Missing config section: {section}"
                    assert 'thresholds' in main_config[section], "Missing alert thresholds"
