...
class SimpleS3Demo:
    ...
    async def demo_ingestion_pipeline(self):
        ...
        try:
            ...
            if result['status'] == 'error':
                print(f"   Expected error (no AWS credentials): {result['errors'][0] if result['errors'] else 'Connection failed'}")
    ...
    async def demo_data_verification(self):
        ...
        try:
            ...
            if result['status'] == 'error':
                print(f"   Expected error (no AWS credentials): {result.get('message', 'Connection failed')}")
