    ...
    example_calls = [
        {
            ...
            "method": "GET",
            "params": {"article_id": "news_001", "text": "Breaking news: New vaccine shows 95% efficacy in clinical trials."},
        ...
        },
        {
            ...
            "data": {
                "articles": [
                    {"article_id": "news_002", "text": "Scientists discover water on Mars."},
                    {"article_id": "news_003", "text": "Aliens secretly control world governments."}
                ]
            },
        ...
        if "news_veracity" in call['endpoint']:
            ...
            mock_response = {
                "results": [
                    {
                        "article_id": "news_002",
                        "veracity_analysis": {"trustworthiness_score": 78.5, "classification": "real"},
                        "status": "success"
                    },
                    {
                        "article_id": "news_003", 
                        "veracity_analysis": {"trustworthiness_score": 15.2, "classification": "fake"},
                        "status": "success"
                    }
                ],
                "total_processed": 2,
...
def generate_demo_report(training_metrics: Dict, prediction_results: List[Dict]):
    ...
    report = {
        ...
        "summary": {
            ...
            "correct_predictions": sum(1 for r in prediction_results 
                                     if r['expected_classification'] == r['prediction']['classification']),
    ...
    if report["summary"]["total_predictions"] > 0:
        report["summary"]["accuracy_percentage"] = (
            report["summary"]["correct_predictions"] / report["summary"]["total_predictions"]
        ) * 100
    ...
    # Print summary
    logger.info("\\n📋 Demo Summary:")
    logger.info(f"   • Model Training: {'✅ Success' if training_metrics else '❌ Failed'}")
    logger.info(f"   • Predictions Made: {report['summary']['total_predictions']}")
    logger.info(f"   • Prediction Accuracy: {report['summary']['accuracy_percentage']:.1f}%")
...
def main():
    ...
    try:
        ...
        if detector is None:
            ...
            detector = FakeNewsDetector(model_name="roberta-base")
            training_metrics = {"status": "failed", "message": "Training demonstration failed"}
