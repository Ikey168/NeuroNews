...
class EventDetectionDemo:
    ...
    async def demo_article_embedding(self):
        ...
        try:
            ...
            if embeddings:
                avg_quality = np.mean([emb['embedding_quality_score'] for emb in embeddings])
    ...
    async def demo_event_clustering(self):
        ...
        try:
            from src.nlp.event_clusterer import EventClusterer, get_redshift_connection_params
            ...
            if events:
                ...
                for i, event in enumerate(events[:5], 1):  # Show top 5
                    ...
                    print(f"   🌍 Geographic Focus: {', '.join(event['geographic_focus'][:3])}")
    ...
    async def demo_breaking_news_api(self):
        ...
        try:
            ...
            for category, stats in sorted(category_stats.items(), 
                ...
                print(f"   {category}: {stats['count']} events (avg score: {stats['avg_score']:.2f})")
            ...
            api_responses = {
                ...
                '/breaking_news?category=Technology': {
                    'events': len([e for e in breaking_events if e['category'] == 'Technology']),
                    'response_time': '0.12s',
                    'cached': False
                },
                '/events/clusters': {
            ...
            for endpoint, metrics in api_responses.items():
                print(f"   {endpoint}")
                print(f"     📊 Results: {metrics['events'] if 'events' in metrics else metrics['clusters']}")
    ...
    async def demo_event_significance(self):
        ...
        try:
            ...
            sorted_events = sorted(self.detected_events, 
            ...
            self.demo_results['significance'] = {
                ...
                'max_significance': max(significance_scores) if significance_scores else 0,
                'distribution': {
    ...
    async def demo_database_integration(self):
        ...
        try:
            ...
            print(f"   📝 Events to store: {len(self.detected_events) if hasattr(self, 'detected_events') else 0}")
            print(f"   🔗 Article assignments: {sum(len(e['articles']) for e in self.detected_events) if hasattr(self, 'detected_events') else 0}")
            print(f"   🧠 Embeddings cached: {len(self.embeddings_data) if hasattr(self, 'embeddings_data') else 0}")
            
            # Schema information
            print(f"\n🏗️ Database Schema:")
            print(f"   📋 event_clusters table")
            print(f"   🔗 article_cluster_assignments table")
            print(f"   🧠 article_embeddings table")
            print(f"   📊 Views: active_breaking_news, trending_events_by_category")
            
            # Performance estimates
            print(f"\n⚡ Performance Estimates:")
            print(f"   💾 Storage per event: ~2KB")
            print(f"   🧠 Embedding storage: ~1.5KB per article")
            print(f"   🔍 Query response time: <100ms")
            print(f"   📈 Indexing: Optimized for time-based queries")
            
            # Store results
            self.demo_results['database'] = {
                'events_to_store': len(self.detected_events) if hasattr(self, 'detected_events') else 0,
                'embeddings_cached': len(self.embeddings_data) if hasattr(self, 'embeddings_data') else 0,
                'schema_tables': ['event_clusters', 'article_cluster_assignments', 'article_embeddings'],
    ...
    async def demo_performance_metrics(self):
        ...
        try:
            ...
            total_events = len(self.detected_events) if hasattr(self, 'detected_events') else 0
            total_embeddings = len(self.embeddings_data) if hasattr(self, 'embeddings_data') else 0
            ...
            if hasattr(self, 'detected_events') and self.detected_events:
                avg_cluster_size = np.mean([e['cluster_size'] for e in self.detected_events])
                avg_silhouette = np.mean([e['silhouette_score'] for e in self.detected_events])
                avg_cohesion = np.mean([e['cohesion_score'] for e in self.detected_events])
            ...
            # System efficiency
            total_processing_time = avg_embedding_time * total_articles + clustering_time
            throughput = total_articles / total_processing_time if total_processing_time > 0 else 0
            
            print(f"\n⚡ System Efficiency:")
            print(f"   🚀 Total processing time: {total_processing_time:.2f}s")
            print(f"   📊 Throughput: {throughput:.1f} articles/second")
            print(f"   💾 Memory efficient: Batch processing enabled")
            print(f"   🔄 Scalable: Async processing architecture")
            ...
            self.demo_results['performance'] = {
                ...
                'quality_metrics': {
                    'avg_cluster_size': avg_cluster_size if hasattr(self, 'detected_events') and self.detected_events else 0,
                    'avg_silhouette_score': avg_silhouette if hasattr(self, 'detected_events') and self.detected_events else 0,
                    'avg_cohesion_score': avg_cohesion if hasattr(self, 'detected_events') and self.detected_events else 0
                }
            }
    ...
    async def save_demo_results(self):
        ...
        try:
            ...
            # Save to file
            output_file = f"event_detection_demo_results.json"
