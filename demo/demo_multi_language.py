...
from src.nlp.language_processor import LanguageDetector, TranslationQualityChecker
...
def demo_language_detection():
    ...
    # Summary
    correct = sum(1 for lang, result in results.items() 
                 if result['language'] == lang.lower()[:2])
    total = len(results)
...
def demo_configuration():
    ...
    try:
        ...
        if config_path.exists():
            ...
            print(f"  Supported Languages: {len(config['multi_language']['supported_languages'])}")
            print(f"  Translation Enabled: {config['multi_language']['translation_enabled']}")
            print(f"  Quality Threshold: {config['multi_language']['quality_threshold']}")
            print(f"  AWS Region: {config['multi_language']['aws_translate']['region']}")
