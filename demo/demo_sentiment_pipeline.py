...
class SentimentPipelineDemo:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"✅ Sentiment analyzer initialized with model: {self.analyzer.model_name}")
            return True
    ...
    async def analyze_single_article(self, article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if title_result['label'] == content_result['label']:
            ...
            overall_score = (title_result['score'] * title_weight + 
        ...
        # Log results
        logger.info(f"📈 Title Sentiment: {title_result['label']} ({title_result['score']:.3f})")
        logger.info(f"📄 Content Sentiment: {content_result['label']} ({content_result['score']:.3f})")
    ...
    def analyze_sentiment_trends(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            if topic not in topic_sentiments:
                topic_sentiments[topic] = {'POSITIVE': 0, 'NEGATIVE': 0, 'NEUTRAL': 0, 'scores': []}
        ...
        for topic, sentiments in topic_sentiments.items():
            total_topic = sum(sentiments[label] for label in ['POSITIVE', 'NEGATIVE', 'NEUTRAL'])
            avg_score = sum(sentiments['scores']) / len(sentiments['scores'])
            dominant = max(sentiments, key=lambda x: sentiments[x] if x != 'scores' else 0)
            logger.info(f"  📂 {topic}: {dominant} dominant (avg score: {avg_score:.3f})")
    ...
    def generate_summary_report(self, results: List[Dict[str, Any]], trends: Dict[str, Any]):
        ...
        most_positive = max(results, key=lambda x: x['overall_sentiment']['score'] 
                           if x['overall_sentiment']['label'] == 'POSITIVE' else 0)
        ...
        most_negative = max(results, key=lambda x: x['overall_sentiment']['score']
                           if x['overall_sentiment']['label'] == 'NEGATIVE' else 0)
