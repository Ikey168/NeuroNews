...
try:
    from nlp.nlp_integration import (
...
class OptimizedNLPDemo:
    ...
    async def demonstrate_concurrent_processing(self):
        ...
        try:
            ...
            concurrent_tasks = [
                pipeline.process_articles_async(batch1, operations=['sentiment', 'keywords']),
                pipeline.process_articles_async(batch2, operations=['sentiment', 'keywords'])
            ]
            ...
            start_time = time.time()
            seq_result1 = await pipeline.process_articles_async(batch1, operations=['sentiment', 'keywords'])
            seq_result2 = await pipeline.process_articles_async(batch2, operations=['sentiment', 'keywords'])
            ...
            efficiency_gain = sequential_time / concurrent_time if concurrent_time > 0 else 1.0
            
            print(f"🏃 Sequential processing: {sequential_time:.2f}s ({sequential_throughput:.2f} articles/sec)")
            print(f"🚀 Concurrent processing: {concurrent_time:.2f}s ({concurrent_throughput:.2f} articles/sec)")
    ...
    def demonstrate_aws_sagemaker_readiness(self):
        ...
        try:
            ...
            print(f"  ⚡ Model quantization: {sagemaker_config.enable_model_quantization}")
            print(f"  🔄 Batch transform: {sagemaker_config.enable_sagemaker_batch_transform}")
