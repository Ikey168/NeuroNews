...
class TestQuickSightService:
    ...
    def mock_quicksight_client(self):
        ...
        mock_client.describe_data_source.side_effect = mock_client.exceptions.ResourceNotFoundException()
        ...
        mock_client.describe_data_set.side_effect = mock_client.exceptions.ResourceNotFoundException()
        ...
        mock_client.describe_analysis.side_effect = mock_client.exceptions.ResourceNotFoundException()
        ...
        mock_client.describe_dashboard.side_effect = mock_client.exceptions.ResourceNotFoundException()
...
class TestIntegration:
    ...
    async def test_issue_49_requirements_coverage(self):
        ...
        service_methods = [method for method in dir(QuickSightDashboardService) 
                          if not method.startswith('_') or method in ['_get_dashboard_filters']]
        
        for requirement in requirements_covered:
            assert requirement in service_methods or hasattr(QuickSightDashboardService, requirement)
        
        # Check that all dashboard types are supported
        supported_layouts = [layout.value for layout in DashboardType]
        required_layouts = ['sentiment_trends', 'entity_relationships', 'event_timeline']
