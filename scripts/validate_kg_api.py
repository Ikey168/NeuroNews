...
class KnowledgeGraphAPIValidator:
        ...
        def decorator(func):
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    duration_ms = (time.time() - start_time) * 1000
                    self.add_result(test_name, False, duration_ms, f"Exception: {str(e)}")
                    return False
    ...
    def test_api_deployment_status(self):
        ...
        try:
            result = subprocess.run([
                "kubectl", "get", "deployment", "knowledge-graph-api",
            ...
            if ready_replicas >= 1 and ready_replicas == replicas:
                return True, f"API deployment ready ({ready_replicas}/{replicas} replicas)", {
                    "ready_replicas": ready_replicas,
                    "total_replicas": replicas
                }
            else:
                return False, f"API deployment not ready ({ready_replicas}/{replicas} replicas)"
    ...
    def test_redis_deployment_status(self):
        ...
        try:
            result = subprocess.run([
                "kubectl", "get", "deployment", "kg-api-redis",
                "-n", self.namespace, "-o", "json"
            ], capture_output=True, text=True, timeout=10)
            
            if result.returncode != 0:
                return True, "Redis deployment not found (may be skipped)"  # Non-critical
            ...
            if ready_replicas >= 1 and ready_replicas == replicas:
                return True, f"Redis deployment ready ({ready_replicas}/{replicas} replicas)"
            else:
                return False, f"Redis deployment not ready ({ready_replicas}/{replicas} replicas)"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not missing_keys:
                    ...
                    return True, "API stats endpoint working", {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not missing_keys:
                    ...
                    return True, f"Entity search working (found {total_results} results)", {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not missing_keys:
                    ...
                    return True, f"Related entities query working (found {total_related} related)", {
                        "entity": entities_data.get("entity"),
                        "total_related": total_related
                    }
                else:
                    return False, f"Related entities response missing keys: {missing_keys}"
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not missing_keys:
                    ...
                    return True, f"Event timeline working (found {total_events} events)", {
                        "topic": timeline_data.get("topic"),
                        "total_events": total_events
                    }
                else:
                    return False, f"Event timeline response missing keys: {missing_keys}"
            else:
    ...
    def test_cache_performance(self):
        ...
        try:
            ...
            if response1.status_code != 200:
                return False, f"First cache test request failed: HTTP {response1.status_code}"
            ...
            if response2.status_code != 200:
                return False, f"Second cache test request failed: HTTP {response2.status_code}"
            ...
            if data1 == data2:
                ...
                return True, f"Cache working correctly (performance improvement: {performance_improvement:.1f}%)", {
        ...
        try:
            ...
            for endpoint, params in test_endpoints:
                ...
                if response.status_code == 200:
                    ...
                    return False, f"Performance test failed on {endpoint}: HTTP {response.status_code}"
            
            avg_response_time = sum(response_times) / len(response_times)
            max_response_time = max(response_times)
            
            # Define performance thresholds
            if avg_response_time <= 1000 and max_response_time <= 2000:  # 1s avg, 2s max
                return True, f"Response time performance good (avg: {avg_response_time:.1f}ms, max: {max_response_time:.1f}ms)", {
                    "average_response_time_ms": avg_response_time,
                    "max_response_time_ms": max_response_time,
                    "response_times": response_times
                }
            else:
                return False, f"Response time performance poor (avg: {avg_response_time:.1f}ms, max: {max_response_time:.1f}ms)"
    ...
    def test_service_discovery(self):
        ...
        try:
            ...
            if len(existing_services) >= 1:  # At least API service should exist
                return True, f"Service discovery working ({len(existing_services)}/{len(services_to_check)} services found)", {
    ...
    def test_monitoring_integration(self):
        ...
        try:
            ...
            result = subprocess.run([
                "kubectl", "get", "servicemonitor", "knowledge-graph-api-monitor",
                "-n", self.namespace
            ], capture_output=True, text=True, timeout=10)
            ...
            if result.returncode == 0:
                ...
                return True, "ServiceMonitor not found (monitoring may not be deployed)"  # Non-critical
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Validate Knowledge Graph API deployment')
    parser.add_argument('--api-url', default='http://localhost:8080', 
                       help='Base URL for the API (default: http://localhost:8080)')
    parser.add_argument('--namespace', default='neuronews',
    ...
    if args.port_forward:
        ...
        try:
            port_forward_process = subprocess.Popen([
                ...
                "service/knowledge-graph-api-service", "8080:80",
                "-n", args.namespace
