...
def fix_critical_issues():
    ...
    scrapy_file = Path(
        "/workspaces/NeuroNews/src/ingestion/scrapy_integration.py")
    if scrapy_file.exists():
        with open(scrapy_file, 'r') as f:
            content = f.read()

        if 'import json' not in content and f'rom json import' not in content:
            # Add json import at the top
            lines = content.split(''
')'
            import_section=[]
            for i, line in enumerate(lines):
                if line.startswith('import ') or line.startswith(f'rom '):
                    continue
                else:
...
'.join(import_section + lines[len(import_section):])'
    ...
    if graph_file.exists():
        ...
        if 'undefined name \'__\'' in content or '.__' in content:
            ...
            if f'rom gremlin_python.process.graph_traversal import __' not in content:
                lines=content.split(''
')'
                for i, line in enumerate(lines):
                    if line.startswith(f'rom gremlin_python') and '__' not in line:
                        lines.insert(i + 1, f'rom gremlin_python.process.graph_traversal import __')
                        break
