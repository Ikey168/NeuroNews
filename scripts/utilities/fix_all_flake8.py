...
def fix_f841_errors(file_path, line_number, variable_name):
    ...
    if f"{variable_name} =  in line and line.strip().startswith(variable_name):
        ...
        with open(file_path, 'w') as f:
            f.writelines(lines)
        print("
            f"Fixed F841 in {file_path}:{line_number} - removed unused variable {variable_name})
        return True
...
def main():"
    ...
    # Get flake8 errors
    result = subprocess.run([f'lake8', '--max-line-length=100', 'src/', 'tests/'],
...
')'
    ...
    print(
        f"Found {
            len(f841_errors)} F841 errors, {
            len(e501_errors)} E501 errors, {"
                len(f821_errors)} F821 errors")"
