...
class NeptuneFailoverManager:
    ...
    def save_original_config(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"✅ Original configuration saved to {self.original_config_file}")
            return True
    ...
    def create_failover_config(self, 
                              ...
                              mode: str = "replica") -> bool:
        """Create disaster recovery configuration."""
        try:
            dr_config = {
                "disaster_recovery": {
                    "mode": mode,
                    "current_endpoint": replica_endpoint if mode == "replica" else primary_endpoint,
                    "endpoints": {
                        "primary": {
            ...
            self.logger.info(f"✅ Disaster recovery config created: {self.dr_config_file}")
            return True
    ...
    def switch_to_replica(self, replica_endpoint: str) -> bool:
        ...
        try:
            ...
            if replica_endpoint.startswith('ws://'):
                ...
                host_port = replica_endpoint.replace('ws://', '').replace('/gremlin', '')
                if ':' in host_port:
    ...
    def switch_to_primary(self) -> bool:
        ...
        try:
            ...
            self.logger.info("🔄 Application should be restarted to use primary endpoint")
    ...
    def test_endpoint(self, endpoint: str) -> bool:
        ...
        try:
            ...
            self.logger.warn("Gremlin dependencies not available for connection testing")
            return False
        except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Neptune Failover Configuration Manager')
    parser.add_argument('action', choices=['switch-to-replica', 'switch-to-primary', 'status', 'test'],
                       help='Action to perform')
    parser.add_argument('--replica-endpoint', 
                       help='Replica endpoint (required for switch-to-replica)')
    parser.add_argument('--primary-endpoint',
                       help='Primary endpoint (optional, for configuration)')
    ...
    if args.action == 'switch-to-replica':
        ...
        if manager.switch_to_replica(args.replica_endpoint):
            if args.primary_endpoint:
                manager.create_failover_config(args.primary_endpoint, args.replica_endpoint, "replica")
        ...
        if not endpoint:
            print("❌ No endpoint to test (specify --test-endpoint or set NEPTUNE_ENDPOINT)")
            return 1
