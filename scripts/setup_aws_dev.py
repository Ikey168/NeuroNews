...
def create_dev_config(profile_name, s3_bucket, s3_prefix, cloudwatch_log_group, 
                     cloudwatch_log_stream_prefix, cloudwatch_log_level):
...
def check_aws_resources(profile_name, s3_bucket, cloudwatch_log_group):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            create_bucket = input(f"Do you want to create the S3 bucket '{s3_bucket}'? (y/n): ")
            if create_bucket.lower() == "y":
                region_param = ["--region", os.environ.get("AWS_DEFAULT_REGION", DEFAULT_REGION)]
                result = subprocess.run(
                    ["aws", "s3api", "create-bucket", "--bucket", s3_bucket] + region_param,
                    capture_output=True,
                    text=True
                )
                if result.returncode == 0:
    ...
    try:
        result = subprocess.run(
            ["aws", "logs", "describe-log-groups", "--log-group-name-prefix", cloudwatch_log_group],
            capture_output=True,
            text=True
        )
        if result.returncode == 0:
            log_groups = json.loads(result.stdout).get("logGroups", [])
            log_group_exists = any(lg["logGroupName"] == cloudwatch_log_group for lg in log_groups)
            
            if log_group_exists:
                print(f"CloudWatch log group '{cloudwatch_log_group}' exists and is accessible")
            else:
                print(f"CloudWatch log group '{cloudwatch_log_group}' does not exist")
                
                # Try to create the log group
                create_log_group = input(f"Do you want to create the CloudWatch log group '{cloudwatch_log_group}'? (y/n): ")
                if create_log_group.lower() == "y":
                    ...
                    if result.returncode == 0:
                        print(f"CloudWatch log group '{cloudwatch_log_group}' created successfully")
                    else:
                        print(f"Failed to create CloudWatch log group '{cloudwatch_log_group}'")
                        print(f"Error: {result.stderr}")
                        return False
        else:
...
def generate_run_commands(profile_name, s3_bucket, s3_prefix, cloudwatch_log_group, 
                         cloudwatch_log_stream_prefix, cloudwatch_log_level):
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Setup AWS credentials and dev environment for NeuroNews scraper")
    ...
    # CloudWatch options
    parser.add_argument("--cloudwatch-log-group", default=DEFAULT_CLOUDWATCH_LOG_GROUP, help="CloudWatch log group name")
    parser.add_argument("--cloudwatch-log-stream-prefix", default=DEFAULT_CLOUDWATCH_LOG_STREAM_PREFIX, help="CloudWatch log stream prefix")
    parser.add_argument("--cloudwatch-log-level", default=DEFAULT_CLOUDWATCH_LOG_LEVEL, choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"], help="CloudWatch log level")
    
    args = parser.parse_args()
    
    # Set up AWS credentials
    if not setup_aws_credentials(args.profile, args.access_key, args.secret_key, args.region):
