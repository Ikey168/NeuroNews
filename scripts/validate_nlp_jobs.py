...
class NLPJobsValidator:
        ...
        try:
            ...
            for pc in expected_classes:
                result = self.run_kubectl_command(
                    ["get", "priorityclass", pc], check=False
                )
                if result.returncode != 0:
    ...
    def test_rbac_resources(self) -> bool:
        ...
        try:
            ...
            result = self.run_kubectl_command(
                ["get", "sa", "nlp-processor", "-n", self.namespace], check=False
            )
            if result.returncode != 0:
            ...
            # Check Role
            result = self.run_kubectl_command(
                ["get", "role", "nlp-processor", "-n", self.namespace], check=False
            )
            if result.returncode != 0:
            ...
            # Check RoleBinding
            result = self.run_kubectl_command(
                ["get", "rolebinding", "nlp-processor", "-n", self.namespace],
                check=False,
            )
            if result.returncode != 0:
            ...
            # Check ClusterRole
            result = self.run_kubectl_command(
                ["get", "clusterrole", "nlp-gpu-access"], check=False
            )
            if result.returncode != 0:
    ...
    def test_storage_resources(self) -> bool:
        ...
        try:
            ...
            unbound_pvcs = [
                pvc for pvc, status in pvc_status.items() if status != "Bound"
            ]
    ...
    def test_configmap(self) -> bool:
        ...
        try:
            result = self.run_kubectl_command(
                ["get", "cm", "nlp-config", "-n", self.namespace, "-o", "json"],
                check=False,
            )
            if result.returncode != 0:
                self.validation_results["tests"][test_name]["status"] = "failed"
            ...
            if not missing_keys:
                ...
                self.validation_results["tests"][test_name]["details"] = {
                    "keys": list(data_keys)
                }
                ...
                self.validation_results["tests"][test_name][
                    "message"
                ] = f"Missing ConfigMap keys: {missing_keys}"
                self.validation_results["tests"][test_name]["details"] = {
    ...
    def test_gpu_availability(self) -> bool:
        ...
        try:
            # Check for GPU nodes
            result = self.run_kubectl_command(
                ["get", "nodes", "-l", "nvidia.com/gpu.present=true", "--no-headers"],
                check=False,
            )
    ...
    def test_nvidia_device_plugin(self) -> bool:
        ...
        try:
            ...
            result = self.run_kubectl_command(
                ["get", "ds", "nvidia-device-plugin-daemonset", "-n", "gpu-operator"],
                check=False,
            )
            if result.returncode != 0:
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    "⚠ NVIDIA device plugin not found - GPU scheduling may not work"
                )
                return False
    ...
    def test_monitoring_resources(self) -> bool:
        ...
        try:
            ...
            result = self.run_kubectl_command(
                ["get", "servicemonitor", "nlp-jobs-monitor", "-n", self.namespace],
                check=False,
            )
            ...
            result = self.run_kubectl_command(
                ["get", "prometheusrule", "nlp-jobs-alerts", "-n", self.namespace],
                check=False,
            )
            ...
            result = self.run_kubectl_command(
                ["get", "cm", "nlp-grafana-dashboard", "-n", self.namespace],
                check=False,
            )
            ...
            existing_components = [
                name for name, exists in monitoring_components.items() if exists
            ]
            missing_components = [
                name for name, exists in monitoring_components.items() if not exists
            ]
            ...
            if len(existing_components) == len(monitoring_components):
                ...
                self.validation_results["tests"][test_name][
                    "message"
                ] = "All monitoring resources exist"
                ...
                self.validation_results["tests"][test_name][
                    "message"
                ] = f"Some monitoring resources missing: {missing_components}"
                self.validation_results["tests"][test_name][
                    "details"
                ] = monitoring_components
                logger.warning(
    ...
    def test_job_creation(self) -> bool:
        ...
        try:
            ...
            for job in expected_jobs:
                result = self.run_kubectl_command(
                    ["get", "job", job, "-n", self.namespace, "-o", "json"], check=False
                )
                if result.returncode == 0:
            ...
            existing_jobs = [
                job for job, status in job_status.items() if status["exists"]
            ]
            missing_jobs = [
                job for job, status in job_status.items() if not status["exists"]
            ]
            ...
            if len(existing_jobs) == len(expected_jobs):
                ...
                self.validation_results["tests"][test_name][
                    "message"
                ] = f"Some jobs missing: {missing_jobs}"
                self.validation_results["tests"][test_name]["details"] = job_status
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate NLP Jobs deployment in Kubernetes"
    )
