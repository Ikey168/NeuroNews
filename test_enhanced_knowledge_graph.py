...
class TestEnhancedEntityExtractor:
    ...
    def mock_nlp_pipeline(self):
        ...
        mock_pipeline.process_text = AsyncMock(return_value={
            'entities': [
                ...
                {'text': 'Tim Cook', 'label': 'PERSON', 'start': 20, 'end': 28, 'confidence': 0.92},
                {'text': 'artificial intelligence', 'label': 'TECH', 'start': 50, 'end': 73, 'confidence': 0.88}
            ],
            'sentences': ['Apple Inc. CEO Tim Cook announced new artificial intelligence initiatives.'],
            'dependencies': [
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_KG_AVAILABLE, reason="Enhanced KG components not available")
    async def test_entity_extraction_basic(self, mock_nlp_pipeline, sample_article_content):
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_KG_AVAILABLE, reason="Enhanced KG components not available")
    async def test_relationship_extraction(self, mock_nlp_pipeline, sample_article_content):
    ...
    async def test_entity_normalization(self, mock_nlp_pipeline):
        ...
        entities = await extractor.extract_entities_from_article(
            article_id="test_003",
            title="Entity Normalization Test",
            content=content
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_KG_AVAILABLE, reason="Enhanced KG components not available")
    async def test_confidence_thresholding(self, mock_nlp_pipeline, sample_article_content):
    ...
    async def test_entity_properties_extraction(self, mock_nlp_pipeline):
        ...
        sarah_entity = next((e for e in person_entities if 'sarah' in e.text.lower()), None)
        if sarah_entity:
            assert 'title' in sarah_entity.properties
            assert 'cto' in sarah_entity.properties['title'].lower() or 'chief' in sarah_entity.properties['title'].lower()
...
class TestEnhancedKnowledgeGraphPopulator:
    ...
    def mock_graph_builder(self):
        ...
        mock_builder.get_vertex_by_id = AsyncMock(return_value={'id': 'vertex_123', 'name': 'test'})
    ...
    def mock_entity_extractor(self):
        ...
        mock_extractor.extract_entities_from_article = AsyncMock(return_value=mock_entities)
        mock_extractor.extract_relationships = AsyncMock(return_value=mock_relationships)
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_KG_AVAILABLE, reason="Enhanced KG components not available")
    async def test_article_processing(self, mock_graph_builder, mock_entity_extractor, sample_article_data):
        """Test complete article processing workflow."""
        with patch('src.knowledge_graph.enhanced_graph_populator.GraphBuilder', return_value=mock_graph_builder):
            ...
            assert mock_graph_builder.add_relationship.call_count >= 1  # Entity relationships + mentions
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_KG_AVAILABLE, reason="Enhanced KG components not available")
    async def test_entity_deduplication(self, mock_graph_builder, mock_entity_extractor):
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_KG_AVAILABLE, reason="Enhanced KG components not available")
    async def test_relationship_querying(self, mock_graph_builder, mock_entity_extractor):
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_KG_AVAILABLE, reason="Enhanced KG components not available")
    async def test_processing_statistics(self, mock_graph_builder, mock_entity_extractor, sample_article_data):
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not ENHANCED_KG_AVAILABLE, reason="Enhanced KG components not available")
    async def test_confidence_filtering(self, mock_graph_builder, mock_entity_extractor):
        ...
        mock_entity_extractor.extract_entities_from_article.return_value = [low_confidence_entity]
...
class TestKnowledgeGraphIntegration:
    ...
    async def test_end_to_end_workflow(self):
        ...
        mock_nlp_pipeline.process_text = AsyncMock(return_value={
            'entities': [
                {'text': 'Apple Inc.', 'label': 'ORG', 'start': 0, 'end': 10, 'confidence': 0.95},
                {'text': 'Tim Cook', 'label': 'PERSON', 'start': 20, 'end': 28, 'confidence': 0.92}
            ],
    ...
    async def test_error_handling_robustness(self):
        ...
        failing_extractor.extract_entities_from_article = AsyncMock(side_effect=Exception("Extraction failed"))
