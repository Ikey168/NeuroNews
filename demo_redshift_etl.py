...
def demonstrate_etl_pipeline(use_mock: bool = True, batch_size: int = 100):
    ...
    try:
        if use_mock:
            ...
            load_stats = etl_processor.process_validation_pipeline_output(validated_articles)
        ...
        print(f"   • Processing Time: {load_stats['processing_time_seconds']:.2f} seconds")
    ...
    try:
        ...
        if db_stats.get('avg_validation_score'):
            ...
            print(f"   • Score Range: {avg_score['minimum']:.1f} - {avg_score['maximum']:.1f}")
        
        print("\n📊 Articles by Source Credibility:")
        for item in db_stats.get('by_source_credibility', []):
            print(f"   • {item['source_credibility'].title()}: {item['count']} articles")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Demo Redshift ETL Pipeline - Issue #22')
